...
There should be four data quality values: `1`, `2`, `3`, or `NA`
...
county_pop_births_metric_all <- county_pop_births_metric_all |>
  mutate(
    quality = if_else(
      unidentified_county==0, 1, quality
    )
  )
```

- `2`:
  + If the numerator (lbw births) is 10-29
  + If the denominator (all births) is 101-200
  + If the number of unidentified counties is between (0, 5]
  + If the number of counties in the state is <=15 and the ratio of unidentified counties is < 0.5
...
- `3`:
  + If the denominator (all births) is 30-100
  + For all unidentified counties
  
```{r}
county_pop_births_metric_all <- county_pop_births_metric_all |>
  mutate(
    quality = if_else(
      births_all < 100 & births_all > 30 | 
      unidentified_county==1,
        3, quality
    )
  )
...
- `NA`: 
  + If the denominator (all births) is less than 30
  + If the share of low birthweight births is missing
...
```{r}
county_pop_births_metric_all |> count(quality)
```
