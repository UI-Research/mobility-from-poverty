...
The county-level ratio of population to primary care physicians ranges from 0 to 46,784. 6.7 percent of observations do not have a primary care physician.
...
quantile(
  ...
  probs = c(0, .067, .069, 0.2, 0.4, 0.6, 0.8, 0.9, 0.99, 1),
  na.rm = TRUE
)
...
Note that County Health Rankings suppresses data for counties with population greater than 2,000 and 0 primary care physicians. They report a missing value for these observations. It is not entirely clear to me why they suppress these data. One potential explanation is that they assume "0 PCPs" could be a reporting error for counties with a decent-sized population. 

In the last update to this metric when we used data directly from County Health Rankings, their suppression made it so that counties with population greater than 2,000 and 0 PCPs had a ratio of `0` and a quality of `NA`, while Counties with population less than 2,000 and 0 PCPs had a ratio of `0` and a quality of `1`. We do not apply this suppression rule, and any county with 0 reported PCPs is assigned a quality value of `NA`. 

Future updates may want to think more about how to handle the quality indicator for counties that actually have 0 PCPs and those that have 0 reported PCPs due to a reporting error in the data.
