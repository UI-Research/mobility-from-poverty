...
joined_data <- joined_data %>%
  mutate(
    ...
    plus1_diff = round(abs(as.numeric(antecedent) - plus1), digits = 3),
    pct_change = round(abs((antecedent - plus1) / antecedent), digits = 4) * 100
  )
...
quantile(test$pct_change, probs = c(0, 0.2, 0.4, 0.6, 0.75, 0.8, 0.9, 1), na.rm = TRUE)
...
joined_data %>%
  ...
  ggplot(aes(number_pc_phys, pct_change)) +
  geom_point(alpha = 0.2) +
  labs(title = "Counties with the lowest number of PC physicians have the highest percentage point change",
       subtitle = "") +
  scatter_grid()
...
joined_data <- joined_data %>%
  mutate(pc_phys_quality = case_when(pct_change < 20 ~ 1,
                             pct_change >= 20 ~ 2)
  )
...
joined_data <- joined_data %>%
  mutate(year = 2019) %>%
  select(year, state, county, pc_phys_ratio, population, pc_phys_quality) %>%
