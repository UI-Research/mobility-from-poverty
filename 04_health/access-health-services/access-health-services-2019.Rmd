...
joined_data <- joined_data %>%
  mutate(
    ...
    diff_ratio = abs(antecedent - check_antecedent)
  )
```

Any differences in the ratio are likely due to rounding. 
```{r}
test <- joined_data %>%
  filter(number_pc_phys != 0)
  
quantile(test$diff_ratio, probs = seq(0, 1, 0.25), na.rm = TRUE)
...
quantile(test$plus1_diff, probs = seq(0, 1, 0.25), na.rm = TRUE)
