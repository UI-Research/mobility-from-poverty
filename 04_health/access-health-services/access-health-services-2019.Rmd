...
physicians <- read_excel(here::here('04_health/access-health-services/data/Alabama.xlsx'),
                         sheet = 'Ranked Measure Data',
                         skip = 1
) %>%
  select(fips = 'FIPS',
...
physicians <- physicians %>%
  ...
  # select(year, state, county, fips, pc_phys_ratio, number_pc_phys)
...
```{r}
alabama <- left_join(physicians, pop, by = "fips")

alabama <- alabama %>%
  mutate(check = population / number_pc_phys,
         diff = abs(as.numeric(antecedent) - check),
         plus1 = (population + 1) / number_pc_phys,
         minus1 = (population - 1) / number_pc_phys,
         plus1_diff = abs(as.numeric(antecedent) - plus1),
         minus1_diff = abs(as.numeric(antecedent) - minus1)
         )
