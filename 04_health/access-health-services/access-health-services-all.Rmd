...
rm(ahrf_long, ahrf_clean, all_counties)
...
The 2023 release of [County Health Rankings]() data includes this same metric. We check our calculations against theirs.
...
The CHR data also has Valdez-Cordova, AK listed for 2020 instead of Chugach and Copper River. We remove all three of these observations for this exercise. We also remove the 9 Connecticut counties from 2021 that are in our data but not the CHR data for the purposes of this exercise.
```{r clean-chr-test}
chr_test <- chr_test |>
  mutate(GEOID = str_c(state, county)) |>
...
rm(chr2023, chr_test, chr_test_suppressed, chr_pop, chr_test_2)
...
joined_data <- joined_data |>
  mutate(
    ...
    # We only want to calculate the sensitivity for observations with at least 1 PCP. This will also ensure that the quality metric constructed later is NA for observations with no PCPs
    sensitivity = ifelse(
...
Most counties are of high quality
```{r check-quality}
joined_data |>
...
final_data <- joined_data |>
  select(year, state, county, 
         ratio_population_pc_physician, ratio_population_pc_physician_quality) |>
