...
joined_data <- chr2023 %>%
  mutate(year = "2020") %>%
  select(year, state, county, pc_physicians_chr, ratio_population_pc_physician_chr, consequent, rate_pc_physicians_chr) %>%
  full_join(y = ahrf %>%
              filter(year == 2020) %>%
              select(year, state, county, pc_physicians, ratio_population_pc_physician, pop),
...
stopifnot(test$pc_physicians == test$pc_physicians_chr)
...
County Health Rankings uses 2021 Census Population Estimates for their ratio, even though the measure of primary care physicians is from 2020.
```{r}
joined_data <- chr2023 %>%
...
pop19_pep <- tidycensus::get_estimates("county", year = 2019, variables = "POP") %>%
  select(GEOID,
         population = value)
