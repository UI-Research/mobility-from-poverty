...
```{r list-files}
# List files
paths <- list.files(here::here("04_health", "exposure-to-trauma", "data"),
...
expected_files <- c(
  here::here(paste0("04_health/exposure-to-trauma/data/injury_deaths_",
                    expected_years$year,
                    ".txt"))
)
...
30 observations are in our universe of counties but not in the CDC data (from Alaska, Connecticut, and South Dakota). 

Two counties (Kusilvak Census Area, AK and Oglala Lakota County, SD) are missing for 2014-2020. The other two counties (Chugach Census Area, AK and Copper River Census Area, AK) are only missing for 2020.
...
After addressing the counties above, 19 observations (9 counties) are in the CDC data but not our universe of counties. Eight counties (Connecticut) are missing for 2022-2023. The other county (Valdez-Cordova Census Area, AK) is missing for 2021-2023.
...
```{r save-death-rate}
death_rate_vc21 <- deaths |>
  filter((GEOID == "02261" & year==2021)) |>
  pull(rate_injury_death)

death_rate_vc22 <- deaths |>
  filter((GEOID == "02261" & year==2022)) |>
  pull(rate_injury_death)

death_rate_vc23 <- deaths |>
  filter((GEOID == "02261" & year==2023)) |>
  pull(rate_injury_death)

```

Remove Valdez-Cordova Census Area, AK in 2021-2023 because it is not in the Urban universe of counties
```{r remove-vc}
deaths <- deaths |>
  filter(!(GEOID == "02261"))
...
# Remove obsolete objects
rm(all_counties, deaths, death_rate_vc21, death_rate_vc22, death_rate_vc23)
```
...
The county-level rate of injury deaths per 100,000 people ranges from 27.7 to 354.7.
...
There are more missing injury death rates in 2021 (323) than in 2022 (276) or 2023 (266).
...
In the last update of the mobility metrics (2023-2024), we compared our latest year of data (2020) to the version of this metric created by County Health Rankings. We cannot make that comparison this year. 

The most recent version of this metric created by County Health Rankings uses pooled mortality data from 2017-2021 via the National Center for Health Statistics [(NCHS)](https://www.cdc.gov/nchs/nvss/deaths.htm). The public use version of these data does not provide county information if the decedent's county has a population of less than 100,000 (see [2023 File Documentation](https://www.cdc.gov/nchs/nvss/mortality_public_use_data.htm)). Users can apply for the  [Restricted-Use Vital Statistics Data](https://www.cdc.gov/nchs/nvss/nvss-restricted-data.htm), which is what County Health Rankings must have done to get county-level information with these data. We want our code and data to be replicable, which is why we use data from CDC WONDER instead.

However, the Underlying Cause of Death by Single-Race Categories are only available for 2018-2023. This means that for 2021, we can only get a 4-year pool (2018-2021) instead of the 5-year pool used by CHR that includes 2017.
...
Only 6 observations have a data quality value of 3 (Chugach Census Area, AK and Copper River Census Area, AK in each of the three years). 865 observations are missing data quality.
```{r}
joined_data |>
...
Note that the 865 counties with missing quality values are those that CDC WONDER suppressed due to fewer than 10 deaths, marked as "unreliable" due to fewer than 20 deaths, or marked as "not applicable" because the population denominator figure was unavailable.
```{r}
joined_data |>
  count(rate_injury_death_quality)
