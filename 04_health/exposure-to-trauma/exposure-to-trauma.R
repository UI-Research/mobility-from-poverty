...
if (!file.exists(paste0("04_health/exposure-to-trauma/data/", out_state, ".xlsx"))) {
  download.file(all_URLs, destfile = paste0("04_health/exposure-to-trauma/data/", out_state, ".xlsx"))
}

# Read in, clean, and append each Excel sheet
my_data <- map_df(out_state,
                  ~{readxl::read_excel(path = paste0("04_health/exposure-to-trauma/data/", .x, ".xlsx"),
                                       sheet = "Ranked Measure Data",
                                       skip = 1
                  ) %>%
                      select(`FIPS`, `# Injury Deaths`, `Injury Death Rate`) %>%
                      rename(fips = "FIPS",
                             # state_name = "State",
                             # county_name = "County",
                      ...
                      mutate(injury_death_rate = round(injury_death_rate, 2))
                      # drop_na(county_name)
                    }
)
...
writexl::write_xlsx(my_data,
                    path = "04_health/exposure-to-trauma/data/injury_mortality_rate.xlsx")
