...
For more information on how VEST collected election result data and precinct shapefiles, please see their documentation [here](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/NH5S2I).
...
### TO DELETE
temp_file <- tempfile()
unzip(states$url[23], exdir = temp_file)

df <- st_read(temp_file)
```
...
**Because KY and NJ precinct files had naming conventions that differed from other states, we read in the place shapefiles for these states separately and then append both files.**
...
mit <- read_csv(here::here("05_local-governance", 
   ...
   mutate(FIPS = case_when(county == "District Of Columbia"  ~ 11001,
                           TRUE ~ FIPS)) %>%
   tidylog::filter(!(state == 'Rhode Island' & county == 'Federal Precinct')) %>%
...
# # Interpolate to county and check values against MIT values
 st_crs(precincts)
 st_crs(counties)
 # Update code to change CRS to 5070 for counties for compatibility with precincts 
...
# QUESTION (Ridhi): I am not sure what to do about the comments below
