...
This script calculates four measures of descriptive represntation for the Urban Institute Mobility Metrics project. The metrics are constructed using the Municipal Form of Government 2018 Survey and 5-year ACS data. 
...
analysis <- analysis %>%
  mutate(
    council_asian_other = 
      ifelse(council_members == 0, NA, council_asian_other / council_members),
    council_black_nonhispanic = 
      ifelse(council_members == 0, NA, council_black_nonhispanic / council_members),
    council_hispanic = 
      ifelse(council_members == 0, NA, council_hispanic / council_members),
    council_white_nonhispanic = 
      ifelse(council_members == 0, NA, council_white_nonhispanic / council_members)
  )
...
geographies <- geographies %>%
  ...
  select(state, county, place, county_subdivision, people, asian_other, black_nonhispanic, hispanic, white_nonhispanic, geography)
...
trouble <- filter(analysis, is.na(geography)) %>%
...
count(joined_data, state_name, county_name, sort = TRUE)

test <- filter(joined_data, state_name == "IL", county_name == "Cook")
```

### 4. Calculate the metrics

Find the ratio of the proportions. 

```{r}
joined_data <- joined_data %>%
  mutate(
    asian_other = if_else(asian_other == 0, 0, council_asian_other / asian_other), 
    black_nonhispanic = if_else(black_nonhispanic == 0, 0, council_black_nonhispanic / black_nonhispanic), 
    hispanic = if_else(hispanic == 0, 0, council_hispanic / hispanic),
    white_nonhispanic = if_else(white_nonhispanic == 0, 0, council_white_nonhispanic / white_nonhispanic)
  ) %>%
...
```{r}
missing <- filter(joined_data, is.nan(black_nonhispanic))
...
joined_data <- joined_data %>%
  filter(council_members > 0) %>%
  filter(geography != "trouble")
...
all_counties <- all_counties %>%
  mutate(year = 2018) %>%
  select(year, state, county, asian_other, asian_other_quality, black_nonhispanic, black_nonhispanic_quality, hispanic, hispanic_quality, white_nonhispanic, white_nonhispanic_quality)

write_csv(all_counties, here::here("05_local-governance", "descriptive-representation", "descriptive-representation.csv"))
