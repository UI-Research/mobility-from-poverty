...
This denominator is the population count at the county and city (e.g. Census Place) level overall and by race/ethnicity groups  
 Final Data:  [gitfolder]/05_local-governance/descriptive-representation/data
...
#Set year of update 
update_years = c(2014, 2016, 2018, 2021)
...
Place population file does not have 2014 so we will create a duplicate of 2015 data, change the year to 2014 and recombine the file. The population data from the county geography file is not used but we still need the list of places from 2015 to select our target places in 2014.

```{r}

places_pop_14 <- places_pop %>% 
  filter(year == 2015) %>% 
  mutate(year = 2014, population = NA)

places_pop <- places_pop %>% 
  bind_rows(places_pop_14) %>% 
  arrange(year, state_name, place_name)
...
wide_county_demo %>% 
  ...
  facet_wrap(~subgroup) +
  labs(
    title = "Distribution of population share, by county (most recent 5-year ACS data)",
    y = "Density",
...
wide_places_demo %>% 
  ...
  facet_wrap(~subgroup) +
  labs(
    title = "Distribution of population share, by place (most recent 5-year ACS data)",
    y = "Density",
    x = "Population (1,000's)"
  ) +
  theme_minimal()
