...
vacant_affordability = tidylog::left_join(
    vacant_crosswalk %>% mutate(year = as.character(year)), 
    crosswalked_income_limits %>% mutate(year = as.character(year)), 
    by = c("statefip" = "state", geography, "year"),
    relationship = "many-to-one") %>%
  mutate(
    ## binary indicators 
...
# population_listing %>%
#   arrange(geoid) %>%
#   select(geoid, year, matches("name")) %>%
#   left_join(
#     housing_subgroups %>%
#       transmute(
#         year,
...
#       distinct(),
#     by = c("geoid", "year")) %>%
#   filter(is.na(flag), year %in% county_years)

## This fails because of Valdez-Cordova, AK (2014-2019)
## and because of each of the new CT planning regions (2022-2023)
## for counties
...
We anticipate the the distributions for a given metric will be similar across years, and that the peaks of distributions will be progressively lower for lower AMI levels.
...
#   select(
#     metro_area, 
#     ratio_available_30ami = at_or_below_eli_2, 
#     ratio_available_50ami = at_or_below_50_percent_ami_2,
#     ratio_available_80ami = at_or_below_80_percent_ami) %>%
#   mutate(
