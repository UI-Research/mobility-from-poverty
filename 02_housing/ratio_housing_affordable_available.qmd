...
-   We aggregate our income limits to the county level. For most counties, this doesn't require any work, because there's only a single income limit for the county. For counties with sub-county-level income limits, we take the mean of these limits. For revised CT counties (beginning in 2022), we draw in a special
crosswalk to go from sub-counties to counties.
...
## this script:
data_form_path = here("02_housing", "housing_affordability_tenure_final_data_evaluation_form.csv")
read_csv(data_form_path) %>%
  mutate(
    row_number = row_number(),
    `...3` = if_else(row_number > 3, years %>% str_replace_all("_", ";"), `...3`)) %>%
  write_csv(data_form_path)

test_that(
  "The final data passes standardized QC checks", {
    expect_no_error(
      evaluate_final_data(
        exp_form_path = data_form_path,
