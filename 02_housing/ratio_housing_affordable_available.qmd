...
params: 
  ...
  geography: "county"
  county_years: !expr c(2014:2019, 2021:2023)
  place_years: !expr c(2014:2019, 2021:2023)
format: 
  html:
    toc: true
...
housing_microdata = list.files(here(temporary_data_path), full.names = TRUE) %>%
  ...
  map_dfr(
    ~ read_csv(.x) %>% 
        ...
        select(
          -c(PERWT, PERNUM, SAMPLE, SERIAL, GQ, products, afact2, sum_products, 
...
if (as.numeric(max(c(county_years, place_years)) >= 2022)) {
  ...
  fmr_income_limits_2 = fmr_income_limits_1 %>%
    ...
    bind_rows(ct_county_income_limits) } else { 
...
The base AMI level is for a four-person household living in a three-bedroom unit (technically, 2.67 bedrooms, because it is assumed that each bedroom houses 1.5 people). We adjust AMI levels based on the number of bedrooms in each unit and the number of people in each household.

This means that for a household with fewer members, the AMI level is lower. For example, if the base level of 50% AMI is \$35,000, then the modified 50% AMI level for a three-person household would be \$31,500 (discounted by 10%).

This means that for a unit with fewer bedrooms, the AMI level is lower. For example, if the base level of 50% AMI is \$35,000, then the modified 50% AMI level for a two-bedroom unit would be \$31,500 (discounted by 10%).

These adjustments are applied for each unit and household. For a three-person household in a five-bedroom unit, a base AMI of \$35,000 would be adjusted down by 10% and up by 16%, respectively, resulting in an AMI of \$36,540.
...
household_affordability = tidylog::left_join(
  ...
  mutate(
    ## adjusting AMI levels to account for household size, per Worst
    ## Case Housing Needs Report (pg. 84): https://www.huduser.gov/portal//portal/sites/default/files/pdf/Worst-Case-Housing-Needs-2023.pdf
    ## See Table 3: https://www.huduser.gov/datasets/hads/hads_doc.pdf 
    # across(
    #   .cols = matches("level_[0-9]{2}ami"),
    #   .fns = ~ case_when(
    #     household_size == 1 ~ .x * .7,
    #     household_size == 2 ~ .x * .8,
    #     household_size == 3 ~ .x * .9,
    #     household_size == 4 ~ .x,
    #     ## for all household sizes greater than four, we add 8% of the AMI threshold per
    #     ## household member above four
    #     household_size > 4 ~ .x * (1 + ((household_size - 4) * .08)))),
    # ## adjusting AMI levels to account for the number of bedrooms per unit, per Worst
    # ## Case Housing Needs Report (pg. 84): https://www.huduser.gov/portal//portal/sites/default/files/pdf/Worst-Case-Housing-Needs-2023.pdf
    # ## See Table 5: https://www.huduser.gov/datasets/hads/hads_doc.pdf 
    # across(
    #   .cols = matches("level_[0-9]{2}ami"),
    #   .fns = ~ case_when(
    #     BEDROOMS == 0 ~ .x * .7,
    ...
    #     BEDROOMS == 3  ~ .x * 1.04,
    #     BEDROOMS == 4  ~ .x * 1.16,
    #     ## for all bedrooms greater than four, we add 12% of the AMI threshold per
    #     ## household member above four
    #     BEDROOMS >= 5 ~ .x * (1.16 + ((BEDROOMS - 4) * .12)))),
...
vacant_affordability = tidylog::left_join(
    ...
    relationship = "many-to-one") %>%
  mutate(
    ## adjusting AMI levels to account for household size 
    ## interpolating household size based on number of bedrooms and efficiency of 1.5 people per bedroom 
    ## See Table 3: https://www.huduser.gov/datasets/hads/hads_doc.pdf 
    # across(
    #   .cols = matches("level_[0-9]{2}ami"),
    #   .fns = ~ case_when(
    #     BEDROOMS == 0 ~ .x * .7,    
    #     BEDROOMS == 1  ~ .x * .75,  # 1.5 people 
    #     BEDROOMS == 2  ~ .x * .9,   # 3 people  
    #     BEDROOMS == 3  ~ .x * 1.04, # 4.5 people 
    #     BEDROOMS == 4  ~ .x * 1.16, # 6 people 
    #     ## for all household sizes greater than four, we add 8% of the AMI threshold per
    #     ## household member above four
    #     BEDROOMS > 4 ~ .x * (1 + ((BEDROOMS - 4) * .08)))),
    # ## adjusting AMI levels to account for the number of bedrooms per unit, per Worst
    # # Case Housing Needs Report (pg. 84): https://www.huduser.gov/portal//portal/sites/default/files/pdf/Worst-Case-Housing-Needs-2023.pdf
    # across(
    #   .cols = matches("level_[0-9]{2}ami"),
    ...
    #     ## for all household sizes greater than four, we add 8% of the AMI threshold per
    #     ## household member above four
    #     BEDROOMS > 4 ~ .x * (1.16 + ((BEDROOMS - 4) * .12)))),
    ## binary indicators 
        ...
        ~ evaluate_final_data(
            ...
            data = housing_subgroups %>%
              { if (str_detect(.x, "availability")) 
                  select(., -matches("affordable")) 
                else select(., -matches("available"))} %>%
              rename_with(
                .cols = everything(),
                .fn = ~ str_replace(.x, "ratio", "share")),
            geography = geography,
...
if (geography == "county") {
  ...
  ## this file doesn't have place-level data for 2014, so we duplicate records
  ## from 2015, and assign population values to NA (we don't use these, but...
  ## just in case)
  population_listing = population_listing1 %>%
    bind_rows(
      population_listing1 %>%
...
warning("This geography universe listing is modified here because, at the time
        of writing, it had not been updated to include the eight CT county-equivalents
        that were used in years preceding 2022. However, in future updates, the original
        universe file should be updated, and this manual modification should be removed.")
  ...
  {if (geography == "county") 
    mutate(.,
      geography_count = case_when(
        year < 2022 ~ geography_count + 8,
        TRUE ~ geography_count)) 
    else mutate(.,
      ## we don't have a valid crosswalk for South Fulton, GA for 2018-2021
      ## accordingly, we intentionally don't produce estimates for this geography
      ## during those years
      geography_count = case_when(
...
test_that(
  ...
  expect_true(
    housing_subgroups %>%
      filter(subgroup == "All") %>%
      select(c(year, state, !!geography, matches("ratio"), -matches("quality"))) %>%
      group_by(state, year) %>%
      summarize(
        across(
...
```{r}
housing_subgroups %>% 
  select(year, c(matches("ratio"), -matches("quality"))) %>%
  group_by(year) %>%
  summarize(
    across(
      .cols = everything(), 
...
housing_subgroups %>% 
  ...
  mutate(name = name %>% str_replace_all(c("_" = " ", "ratio" = "", "affordable" = "aff.", "available" = "avail.")) %>% str_to_sentence()) %>%
  ggplot() +
