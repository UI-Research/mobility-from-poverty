...
households_2022 <- read_csv("data/temp/households_2022_county.csv")
...
vacant_summed_2022 <- read_csv("data/temp/vacant_summed_2022_county.csv")
...
#         This is calculated in step 5 of the 1b_housing_county.R file (variables: Affordable80AMI_all, Affordable80AMI_renter, Affordable80AMI_owner)
#    -  demand is number of households in each income bracket 
#         This is calculated in step 5 of the 1b_housing_county. file (variables: below80AMI, below50AMI, below30AMI)
...
available_2022_final <- available_2022 %>% 
  ...
  left_join(vacant_summed_2022, by = c("state", "county")) %>% 
  mutate(
...
available_2022_final <- left_join(available_2022_final, county_puma, by=c("state","county"))

# Generate the quality var (naming it housing_quality to match Kevin's notation from 2018)
available_2022_final <- available_2022_final %>% 
  mutate(housing_quality = case_when(county_size_flag==0 & puma_flag==1 ~ 1, # 579 obs
                                     county_size_flag==0 & puma_flag==2 ~ 2, # 454 obs
                                     county_size_flag==0 & puma_flag==3 ~ 3, # 2110 obs
...
write_csv(available_2022_subgroup, "02_housing/data/available_2022_subgroups_county.csv")  
...
# export our file as a .csv
write_csv(available_2022_overall, "02_housing/data/available_2022_subgroups_county.csv")  
...
available_2022_subgroup %>% 
  ggplot(aes(rate_affordable_available_50_ami))+
  geom_histogram()+
  facet_wrap(~subgroup)

# share affordable and available at 80 AMI histogram
available_2022_subgroup %>% 
  ggplot(aes(rate_affordable_available_80_ami))+
  geom_histogram()+
  facet_wrap(~subgroup)
