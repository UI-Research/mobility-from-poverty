---
title: "Affordable Housing - Place"
format: html
editor: visual
---
...
ACS Code: Housing metric Amy Rogin (2023-2024) Using IPUMS extract for ACS 2022 Based on processes developed by Paul Johnson and Kevin Werner in SAS and code by Tina Chelidze in R for 2022-2023 Process: (1) Housekeeping (2) Import microdata (PUMA Place combination already done) (2a) Create a vacant unit specific file (3) Create a Vacant unit dataframe (vacant units will not be accounted for when we isolate households in Steps 4 & 5) Note that to get vacant unit data, need to pull a separate extract from IPUMS; see instructions below. (3a) Calculate the monthly payment for the vacant units for a given first-time homebuyer: (3b) Add PMI, taxes, and insurance estimates, to get total monthly cost of vacant units for ownership This "total_monthly_cost" variable will be used to calculate affordability in Step 6 (3c) Now create accurate gross rent variable for vacant units for rent: (3d) For all microdata where PERNUM=1 and OWNERSHP=2, generate avg ratio of monthly cost vs advertised price of renting. e.g. ratio = RENTGRS/RENT (calculate per place). (3e) In the vacant file, update RENTGRS to be more representative of what actual cost would be (RENTGRS = RENT\*ratio). This "RENTGRS" variable will be used to calculate affordability in Step 6 (4) Import HUD county Income Levels for each FMR and population for FMR (population will be used for weighting) (4a) Merge the 2 files (4b) Bring in county_place crosswalk (4c) Merge FMR file with crosswalk on county (4d) Create place_level_income_limits (weight by FMR population in collapse) (5) Generate households_2022: 30%, 50% and 80% AMI (indicator of HH affordable at each of these levels) (6) Merge Vacant with place_level_income_limits (6a) create same 30%, 50%, and 80% AMI affordability indicators (7) Create the housing metric (7a) Summarize households_2021 and vacant both by place (7b) Merge them by place (7c) Calculate share_affordable_30/50/80AMI (8) Create Data Quality marker (9) Clean and export (10) Quality Checks and Visualizations (10a) Histograms (10b) Summaries (10c) Check against last years values
...
Run "0_housing_microdata.qmd"

```{r}
# this one should already match the PUMAs to places
# create one file with 2022 and 2021 data
acs <- read_csv(here::here("02_housing","data","temp","place_microdata_2022.csv")) %>% 
  bind_rows(read_csv(here::here("02_housing","data","temp","place_microdata_2021.csv")))

```

For HH side: isolate original microdata to only GQ under 3 (only want households) see here for more information: https://usa.ipums.org/usa-action/variables/GQ#codes_section
...
## (3) Create a Vacant units dataframe

(vacant units will not be accounted for when we isolate households in Steps 4 & 5)

Vacancy = 1 (for rent) Vacancy = 2 (for sale) Vacancy = 3 (rented or sold but not yet occupied)

Choosing only 1-3 excludes seasonal, occasional, and migratory units drop all missing VALUEH (value of housing units) obs: https://usa.ipums.org/usa-action/variables/VALUEH#codes_section
...
rent_ratio <- acs_clean %>% 
  select(RENT, RENTGRS, HHINCOME, HHWT, PERNUM, OWNERSHP, statefip, place, year) %>% 
  # Keep one observation per household (PERNUM=1), and only rented ones (OWNERSHP=2)
  tidylog::filter(PERNUM == 1,
                  OWNERSHP == 2)
```

### (3d) For all microdata where PERNUM=1 and OWNERSHP=2, generate avg ratio of monthly cost vs advertised price of renting. e.g. ratio = RENTGRS/RENT (calculate per place)
...
rent_ratio <- rent_ratio %>%
  ...
  dplyr::group_by(statefip, place, year) %>% 
...
puma_place <- read_csv(here::here("geographic-crosswalks/data/crosswalk_puma_to_place.csv")) %>% 
  ...
  mutate(crosswalk_period = ifelse(crosswalk_period == "pre-2022", 2021, crosswalk_period) %>% as.numeric())
```
...
```{r}
# Merge in places
vacant_places  <- left_join(vacant, puma_place, by=c("statefip","puma", "YEAR" = "crosswalk_period"))
...
vacant_final<- left_join(vacant_places, rent_ratio, by = c("statefip", "place", "YEAR" = "year"))
...
# download and combine 2022 and 2021 data into one file
...
FMR_Income_Levels <- tidylog::left_join(FMR_Income_Levels, FMR_pop, by=c("fips2010", "year"))
```

### (4b) Bring in county_place crosswalk

```{r}
county_place <- read_csv(here::here("geographic-crosswalks/data/geocorr2022_county_place.csv")) %>% 
...
FMR <- left_join(FMR_Income_Levels, county_place, by=c( "state", "county")) 
```

### (4d) Create place_level_income_limits (weight by FMR population in collapse)

Most FMRs have a one-to-one correspondence with places because most places fall into whole counties. However, some counties (mainly in New England) contain multiple FMRs. For these counties, replace the multiple FMR records with just one county record, using the weighted average value of the income levels, weighted by the FMR population

```{r}
place_income_limits <- FMR %>%
  dplyr::group_by(state, place, year) %>%
...
place_income_limits$GEOID <- paste(place_income_limits$statefip,place_income_limits$place, sep = "")
# limit only to places of interest
place_income_limits <- place_income_limits %>%
  filter(GEOID %in% puma_place$GEOID)
# 972 obs
```

## (5) Generate households_2022: 30%, 50% and 80% AMI (indicator of HH affordable at each of these levels) - overall and for renters and owner subgroups
...
First, filter microdata to where PERNUM == 1, so only one HH per observation

```{r}
microdata_housing <- acs_clean %>%
  tidylog::filter(PERNUM == 1)
#filter: removed 2,223,287 rows (60%), 1,497,043 rows remaining
```

Create new dataset called "households" to merge microdata & place income limits (place_income_limits) by state and place

```{r}
households <- left_join(microdata_housing, place_income_limits, by=c("statefip","place", "year"))
```

Create variables called Affordable80AMI, Affordable50AMI, Affordable30AMI Read more about the AMI vars methodology here: https://www.huduser.gov/portal/datasets/il//il18/IncomeLimitsMethodology-FY18.pdf

-   l50 is 50% of median rent: Very low-income
-   ELI is 30% of median rent: Extremely low-income
-   l80 is 80% of median rent: Low-income

For owners, use the housing cost, and for renters, use the gross rent. Also create variable for total population at 80% AMI, 50% AMI, and 30% AMI for renter and owner subgroups, this is just the population of renters or owners at each income level
...
write_csv(households, here::here("02_housing/data/temp/households.csv"))

# Data is 1/3 renters 2/3 owners 
#skimr::skim(households)
```

## (6) Merge Vacant with place_level_income_limits (FMR)

Merge on the % AMI income levels and determine which vacant units are also affordable for a family of 4 at %s of AMI (regardless of actual unit size). If there is a non-zero value for gross rent (RENTGRS), use that for the cost. Otherwise, if there is a valid house value, use the housing cost that was calculated and prepared above in the "vacant" df.

NOTE: 1 row doesn't merge - place 0672016 which didn't have any values with VACANCY = 1, 2, or 3

```{r}
vacant <- left_join(vacant_final, place_income_limits, by=c("statefip","place", "YEAR" = "year"))
# 35,631 obs 
```
...
# save file to use for affordability measure in 2a_affordable_available_place.qmd
write_csv(vacant_new, here::here("02_housing/data/temp/vacant.csv"))
...
# from 'vacant', grouped by statefip and place, and weighted by HHWT
...
# save csv for avaiablity calculation in 2a_affordable_available_place.qmd
write_csv(vacant_summed, here::here("02_housing/data/temp/vacant_summed.csv"))

```

### (7b) Merge them by place

NOTE: 1 place doesn't have vacancy data - place 0672016 which didn't have any values with VACANCY = 1, 2, or 3

```{r}
housing <- left_join(households_summed, vacant_summed, by=c("state","place", "year"))
```
...
place_puma <- read_csv(here::here("02_housing/data/temp/place_puma_2021.csv")) %>% 
  mutate(year = 2021) %>% 
  bind_rows(read_csv(here::here("02_housing/data/temp/place_puma_2022.csv")) %>% mutate(year = 2022)) %>% 
  rename("state" = "statefip")

# Merge the PUMA flag in & create the final data quality metric based on both size and puma flags
housing <- left_join(housing, place_puma, by=c("state","place", "year"))
...
write_csv(housing_overall, here::here("02_housing/data/housing_city.csv"))
```
...
# export our file as a .csv
write_csv(housing_subgroup_final, here::here("02_housing/data/housing_subgroups_city.csv"))
```
...
Download 2021 mobility metrics at the place level: https://datacatalog.urban.org/dataset/boosting-upward-mobility-metrics-inform-local-action-10 and save in Downloads folder as "mobility_metrics_place.csv"
...
NLIHC has an Out of Reach tool that is a good benchmark for comparing results to: https://nlihc.org/oor it's not a 1:1 equivalent measure, but gives a sense of the cost of renting in each state and by zip code
