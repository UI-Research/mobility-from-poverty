---
title: "ACS Code: Housing metric, non-subgroup"
author: "Amy Rogin (2023-2024)"
date: today 
format:
  html:
    toc: true
...
Either run "0_housing_microdata.qmd" OR: Import the already prepared microdata file. This one should already match the PUMAs to places.
...
## (3d) For all microdata where PERNUM=1 and OWNERSHP=2

Generate avg ratio of monthly cost vs advertised price of renting. e.g. ratio = RENTGRS/RENT (calculate per place)
...
However, some counties (mainly in New England) contain multiple FMRs. For these counties, replace the multiple FMR records with just one county record, using the weighted average value of the income levels, weighted by the FMR population
