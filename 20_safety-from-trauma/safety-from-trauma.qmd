...
+ CDC WONDER changed how it reports counties with zero deaths starting with the 2024 data. In prior years, counties with zero deaths had their crude rate marked as "Unreliable," which the code converted to NA. In the 2024 data, these counties instead have a literal crude rate of 0.0. We now explicitly set the rate (and confidence interval bounds) to NA when a county has zero deaths, consistent with how prior years were handled. This affects 3 counties in the "all" file (Kalawao County, HI; Clark County, ID; Kenedy County, TX) and many more in the subgroup files where small counties frequently have zero deaths for specific race/ethnicity groups.
...
1. 2018-2024: Underlying Cause of Death by Single-Race Categories
...
+ Other subgroups available for construction through CDC WONDER include age groups (ten-year age groups, five-year age groups, and single-year age groups), sex, and education. The next round of updates may want to consider including additional subgroups, and will have to consider CDC WONDER's data suppression criteria for any subgroup.
...
+ The quality values are based on the coefficient of variation (CV) for each estimate, but the range of CVs is relatively narrow. Future updates may want to consider assigning quality based on different CV values.


# 1. Manually download mortality data from CDC WONDER

This section explains the manual download process to create data extracts for these data from CDC WONDER. All data used in this file are available on Box [here](https://urbanorg.box.com/s/lqoem3k7kc6nlhcqukudmo8nzw1cz3gn). Data stored on Box are only available to Urban Institute researchers.
...
-   Move the `.txt` file from your Downloads folder to the `mobility-from-poverty\20_safety-from-trauma\data\bridged-race` directory.
...
### 2.2 Manually download Underlying Cause of Death by Single-Race Categories (2021-2024)
#### 2.2a Navigate to extract page

1.  Navigate to the Mortality Data homepage on CDC WONDER [here](https://wonder.cdc.gov/Deaths-by-Underlying-Cause.html).

2.  Select [2018-2024: Underlying Cause of Death by Single-Race Categories](https://wonder.cdc.gov/ucd-icd10-expanded.html)

3.  Select "I Agree" at the bottom of the page.

#### 2.2b Create extracts for each year 2021-2024

Create your extract for each year according to the specifications below. Note that the specific extract created by these specifications only creates the data file for 2024. You will need to modify and re-submit the query 3 times to get data files for 2021-2023. Instructions for how to do so are explained after the 2024 extract specifications.

1.  Organize table layout:
  + Group Results By: County
  + Measures: Deaths, Population, Crude Rate, 95% Confidence Interval (Crude Rate), Standard Error (Crude Rate)
  + Title: `injury_deaths_all_YYYY` where `YYYY` represents the most recent year in the five-year pool (e.g., if the pool includes years 2020, 2021, 2022, 2023, and 2024 name the file `injury_deaths_all_2024`)
...
4.  Select year and month:
  + Year/Month: 2020, 2021, 2022, 2023, 2024 (select all at once)
...
-   Move the `.txt` file from your Downloads folder to the `mobility-from-poverty\20_safety-from-trauma\data\single-race\all` directory.

Note that to recreate this metric for all years, the user will need to submit the extract 4 total times. The only thing that changes with each submit is the combination of years chosen. Each data year includes the current year and the previous four years to create a five-year pool. After exporting your resulting `.txt` file for the first extract, navigate back to the `Request Form` by clicking the `Request Form` tab on the top left of the screen. The only thing you need to change is the combination of years under `Year/Month` in the `4. Select year and month:` section. Select:

* 2019, 2020, 2021, 2022, and 2023 and then click `Send` at the bottom of the screen to create the 2023 data file
...
#### 2.2c Create extracts for each year 2021-2024 - race/ethnicity subgroups
...
Move all of the resulting `.txt` files from your Downloads folder to the `mobility-from-poverty\20_safety-from-trauma\data\single-race\subgroups` directory.



# 2. Read in and clean mortality data
...
## 2.1 Bridged Race (2014-2020), All
List all the files in the `bridged-race` folder
```{r create-paths-all-br}
paths <- list.files(here::here("20_safety-from-trauma", "data", "bridged-race"),
                    pattern = "[.]txt$", full.names = TRUE)
```

Check that all files have been properly downloaded
```{r check-files-all-br}
...
expected_files <- c(
  here::here(paste0("20_safety-from-trauma/data/bridged-race/injury_deaths_all_",
                    expected_years$year,
                    ".txt"))
)
...
```{r read-deaths-all-br, warning=FALSE}
deaths_br_raw <- paths |> 
...
```{r}
if (any(as.numeric(deaths_br_raw$Deaths) %in% 1:9)) {

  stop("Do not use death counts in 1:9")

}

```

The raw `.txt` files which we read in from CDC WONDER contains a `Notes` column which we drop. We retain all information for the crude death rate (rate, confidence interval, and standard error). 
```{r clean-data-all-br, warning=FALSE}
# Rename variables, remove `Notes` column
...
## 2.2 Single Race (2021-2024), All
```{r create-paths-all-sr}
paths <- list.files(here::here("20_safety-from-trauma", "data", "single-race", "all"),
                    pattern = "[.]txt$", full.names = TRUE)

```

Check that all files have been properly downloaded
```{r check-files-all-sr}
# Create list of expected years
expected_years <- tribble(
  ~year,
  "2021",
...
# Use years above to create vector of expected files
expected_files <- c(
  here::here(paste0("20_safety-from-trauma/data/single-race/all/injury_deaths_all_",
                    expected_years$year,
                    ".txt"))
)
...
```{r read-deaths-all-sr, warning=FALSE}
deaths_sr_raw <- paths |> 
  set_names(basename(paths)) |>  # Name elements of paths by their filenames
  map_dfr(
    ~ utils::read.delim(.x) |> 
...
```{r}
if (any(as.numeric(deaths_sr_raw$Deaths) %in% 1:9)) {
...
```{r clean-data-all-sr, warning=FALSE}
# Rename variables, remove `Notes` column
deaths_sr_clean <- deaths_sr_raw |>
  ...
  # Change values from character to numeric
  mutate(across(.cols = c(starts_with("rate"), deaths), .fns = as.numeric)) |>
  # Set rate to NA when deaths == 0 (CDC WONDER 2024+ reports 0.0 instead of "Unreliable")
  mutate(across(.cols = starts_with("rate"), ~if_else(deaths == 0, NA, .))) |>
  # Edit GEOID
  mutate(GEOID = str_pad(GEOID, 5, pad = "0")) |>
  mutate(subgroup = "all")
...
## 2.3 Single Race (2021-2024), Subgroups
```{r create-paths-sub-sr}
paths <- list.files(here::here("20_safety-from-trauma", "data", "single-race", "subgroups"),
                    pattern = "[.]txt$", full.names = TRUE)

```

Check that all files have been properly downloaded
```{r check-files-sub-sr}
# Create list of expected years
expected_years <- tribble(
  ~year,
  "nhwhite_2021",
...
# Use years above to create vector of expected files
expected_files <- c(
  here::here(paste0("20_safety-from-trauma/data/single-race/subgroups/injury_deaths_raceth_",
                    expected_years$year,
                    ".txt"))
)
...
```{r read-deaths-sub-sr, warning=FALSE}
...
```{r clean-data-sub-sr, warning=FALSE}
# Rename variables, remove `Notes` column
...
CDC WONDER reports rates as `unreliable` when the death count is less than 20, and as `suppressed` when the death count is less than 10. We changed these rates to `NA` in our data. We also want to change the lower bound, upper bound, and standard error to missing for these observations as well. Note that standard error isn't included in the final file, but the confidence interval bounds are. There are also instances where the upper and lower bounds are 0 because there are 0 deaths, but the rate was `Unreliable`; these also get replaced as `NA` in the code below.
```{r replace-missings}
...
# 3. Join mortality data onto universe of Urban counties

This section reads in the Urban Institute's universe of counties and joins the CDC injury death data to those counties.

## 3.1 Read in Urban county file for 2014-2020

```{r load-counties}
urban_counties <- readr::read_csv(here::here("geographic-crosswalks", "data", "county-populations.csv")) |>
...
### 3.2 Identify counties in Urban universe but not in CDC WONDER

Some observations are in our universe of counties but not in the CDC data (from Alaska, Connecticut, and South Dakota).

+ Chugach Census Area, AK is missing for 2020-2024.
+ Copper River Census Area, AK is missing for 2020-2024.
+ Kusilvak Census Area, AK is missing for 2015-2024.
+ All 9 of Connecticut's planning regions are missing for 2022-2024.
+ Oglala Lakota County, SD is missing for 2015-2024
...
### 3.3 Identify counties in CDC WONDER but not in Urban universe

After addressing the counties above, some observations are in the CDC data but not our universe of counties.
...
+ Valdez-Cordova Census Area, AK is missing for 2020-2024
+ All 8 counties from Connecticut are missing for 2022-2024
...
### 3.4 Address the split of Valdez-Cordova Census Area, AK

Effective 2019-01-02, Valdez-Cordova Census Area, AK (02261) split to form Chugach Census Area, AK (02063) and Copper River Census Area, AK (02066). This change is not reflected in the CDC deaths data. In the 2024 round of metric updates, we looked into using the `interpolate_pw()` [function](https://rdrr.io/cran/tidycensus/man/interpolate_pw.html) from the `library(tidycensus)` package, which uses block-level population estimates to weight the interpolation of death rates across geographies, to get estimates of the age-adjusted rates for each of the new counties. However, because the measure we wanted to interpolate (age-adjusted death rates) is population-standardized, we just ended up with the original rate we started with for both of the interpolated geometries. Instead, we applied the same death rate for both of these new county equivalents and later set the data quality flag to low for these two observations. We repeat this process in the 2026 round of metric updates even though we have switched to crude death rates. Future updates may want to consider a more sophisticated imputation strategy.
```{r check-ak-change}
death_rates_wide |>
  filter(GEOID == "02261")
...
```{r save-death-rate-vc}
# 2020 all
death_rate_vc20 <- death_rates_wide |>
  filter((GEOID == "02261" & year==2020)) |>
  pull(rate_injury_death_all)

death_rate_lb_vc20 <- death_rates_wide |>
  filter((GEOID == "02261" & year==2020)) |>
  pull(rate_injury_death_lb_all)

death_rate_ub_vc20 <- death_rates_wide |>
  filter((GEOID == "02261" & year==2020)) |>
  pull(rate_injury_death_ub_all)
...
death_rate_lb_vc21 <- death_rates_wide |>
  filter((GEOID == "02261" & year==2021)) |>
  pull(rate_injury_death_lb_all)

death_rate_ub_vc21 <- death_rates_wide |>
  filter((GEOID == "02261" & year==2021)) |>
  pull(rate_injury_death_ub_all)
...
death_rate_lb_vc22 <- death_rates_wide |>
  filter((GEOID == "02261" & year==2022)) |>
  pull(rate_injury_death_lb_all)

death_rate_ub_vc22 <- death_rates_wide |>
  filter((GEOID == "02261" & year==2022)) |>
  pull(rate_injury_death_ub_all)
...
death_rate_lb_vc23 <- death_rates_wide |>
  filter((GEOID == "02261" & year==2023)) |>
  pull(rate_injury_death_lb_all)

death_rate_ub_vc23 <- death_rates_wide |>
  filter((GEOID == "02261" & year==2023)) |>
  pull(rate_injury_death_ub_all)

# 2024 all
death_rate_vc24 <- death_rates_wide |>
  filter((GEOID == "02261" & year==2024)) |>
  pull(rate_injury_death_all)

death_rate_lb_vc24 <- death_rates_wide |>
  filter((GEOID == "02261" & year==2024)) |>
  pull(rate_injury_death_lb_all)

death_rate_ub_vc24 <- death_rates_wide |>
  filter((GEOID == "02261" & year==2024)) |>
  pull(rate_injury_death_ub_all)

# 2021 Non-Hispanic White
death_rate_vc21_nhw <- death_rates_wide |>
  filter((GEOID == "02261" & year==2021)) |>
  pull(rate_injury_death_nhwhite)

death_rate_lb_vc21_nhw <- death_rates_wide |>
  filter((GEOID == "02261" & year==2021)) |>
  pull(rate_injury_death_lb_nhwhite)

death_rate_ub_vc21_nhw <- death_rates_wide |>
  filter((GEOID == "02261" & year==2021)) |>
  pull(rate_injury_death_ub_nhwhite)

# 2022 Non-Hispanic White
death_rate_vc22_nhw <- death_rates_wide |>
  filter((GEOID == "02261" & year==2022)) |>
  pull(rate_injury_death_nhwhite)

death_rate_lb_vc22_nhw <- death_rates_wide |>
  filter((GEOID == "02261" & year==2022)) |>
  pull(rate_injury_death_lb_nhwhite)

death_rate_ub_vc22_nhw <- death_rates_wide |>
  filter((GEOID == "02261" & year==2022)) |>
  pull(rate_injury_death_ub_nhwhite)

# 2023 Non-Hispanic White
death_rate_vc23_nhw <- death_rates_wide |>
  filter((GEOID == "02261" & year==2023)) |>
  pull(rate_injury_death_nhwhite)

death_rate_lb_vc23_nhw <- death_rates_wide |>
  filter((GEOID == "02261" & year==2023)) |>
  pull(rate_injury_death_lb_nhwhite)

death_rate_ub_vc23_nhw <- death_rates_wide |>
  filter((GEOID == "02261" & year==2023)) |>
  pull(rate_injury_death_ub_nhwhite)

# 2024 Non-Hispanic White
death_rate_vc24_nhw <- death_rates_wide |>
  filter((GEOID == "02261" & year==2024)) |>
  pull(rate_injury_death_nhwhite)

death_rate_lb_vc24_nhw <- death_rates_wide |>
  filter((GEOID == "02261" & year==2024)) |>
  pull(rate_injury_death_lb_nhwhite)

death_rate_ub_vc24_nhw <- death_rates_wide |>
  filter((GEOID == "02261" & year==2024)) |>
  pull(rate_injury_death_ub_nhwhite)

# 2023 Non-Hispanic Other
death_rate_vc23_nho <- death_rates_wide |>
  filter((GEOID == "02261" & year==2023)) |>
  pull(rate_injury_death_nhother)

death_rate_lb_vc23_nho <- death_rates_wide |>
  filter((GEOID == "02261" & year==2023)) |>
  pull(rate_injury_death_lb_nhother)

death_rate_ub_vc23_nho <- death_rates_wide |>
  filter((GEOID == "02261" & year==2023)) |>
  pull(rate_injury_death_ub_nhother)

# 2024 Non-Hispanic Other
death_rate_vc24_nho <- death_rates_wide |>
  filter((GEOID == "02261" & year==2024)) |>
  pull(rate_injury_death_nhother)

death_rate_lb_vc24_nho <- death_rates_wide |>
  filter((GEOID == "02261" & year==2024)) |>
  pull(rate_injury_death_lb_nhother)

death_rate_ub_vc24_nho <- death_rates_wide |>
  filter((GEOID == "02261" & year==2024)) |>
  pull(rate_injury_death_ub_nhother)
...
```{r remove-vc}
death_rates_wide <- death_rates_wide |>
  filter(!(GEOID == "02261" & year %in% c(2020, 2021, 2022, 2023, 2024)))
...
Join county-level death rates from CDC WONDER for 2014-2024 onto Urban's county universe file

The Connecticut counties still do not match in 2022-2024 between the two files.
...
# All records in the Urban data but not in the death_rates_wide data are from Connecticut or Chugach Census Area and Copper River Census Area in Alaska
connecticut_fips = "09"

# Prove all records in urban_counties and not in CDC data are from Chugach Census Area, Copper River Census Area, or Connecticut
urban_antijoin <- tidylog::anti_join(
  x = urban_counties,
  y = death_rates_wide,
  by = c("year", "GEOID")
  ) |>
  filter(
    # Remove Chugach Census Area and Copper River Census Area
    GEOID %in% c("02063", "02066"),
    # Remove Connecticut planning areas
    str_sub(GEOID, 1, 2) == connecticut_fips & year %in% c(2022, 2023, 2024)
    )

stopifnot(urban_antijoin |> nrow() == 0)

# All records in death_rates_wide but not in urban_counties are from Connecticut
death_rates_wide_anti_join <- tidylog::anti_join(
  x = death_rates_wide,
  y = urban_counties,
  by = c("year", "GEOID")
  ) |>
...
stopifnot(death_rates_wide_anti_join |> nrow() == 0) 
  
```

Apply the CDC WONDER death rate from Valdez-Cordova Census Area in 2020-2024 to these two new county equivalents.
...
joined_data <- joined_data |>
  mutate(
    rate_injury_death_all = case_when(
      ...
      GEOID %in% c("02063", "02066") & year==2024 ~ death_rate_vc24,
      TRUE ~ rate_injury_death_all
    ),
    rate_injury_death_lb_all = case_when(
      GEOID %in% c("02063", "02066") & year==2020 ~ death_rate_lb_vc20,
      GEOID %in% c("02063", "02066") & year==2021 ~ death_rate_lb_vc21,
      GEOID %in% c("02063", "02066") & year==2022 ~ death_rate_lb_vc22,
      GEOID %in% c("02063", "02066") & year==2023 ~ death_rate_lb_vc23,
      GEOID %in% c("02063", "02066") & year==2024 ~ death_rate_lb_vc24,
      TRUE ~ rate_injury_death_lb_all
    ),
    rate_injury_death_ub_all = case_when(
      GEOID %in% c("02063", "02066") & year==2020 ~ death_rate_ub_vc20,
      GEOID %in% c("02063", "02066") & year==2021 ~ death_rate_ub_vc21,
      GEOID %in% c("02063", "02066") & year==2022 ~ death_rate_ub_vc22,
      GEOID %in% c("02063", "02066") & year==2023 ~ death_rate_ub_vc23,
      GEOID %in% c("02063", "02066") & year==2024 ~ death_rate_ub_vc24,
      TRUE ~ rate_injury_death_ub_all
    ),
    rate_injury_death_nhwhite = case_when(
      GEOID %in% c("02063", "02066") & year==2021 ~ death_rate_vc21_nhw,
      GEOID %in% c("02063", "02066") & year==2022 ~ death_rate_vc22_nhw,
      GEOID %in% c("02063", "02066") & year==2023 ~ death_rate_vc23_nhw,
      GEOID %in% c("02063", "02066") & year==2024 ~ death_rate_vc24_nhw,
      TRUE ~ rate_injury_death_nhwhite
    ),
    rate_injury_death_lb_nhwhite = case_when(
      GEOID %in% c("02063", "02066") & year==2021 ~ death_rate_lb_vc21_nhw,
      GEOID %in% c("02063", "02066") & year==2022 ~ death_rate_lb_vc22_nhw,
      GEOID %in% c("02063", "02066") & year==2023 ~ death_rate_lb_vc23_nhw,
      GEOID %in% c("02063", "02066") & year==2024 ~ death_rate_lb_vc24_nhw,
      TRUE ~ rate_injury_death_lb_nhwhite
    ),
    rate_injury_death_ub_nhwhite = case_when(
      GEOID %in% c("02063", "02066") & year==2021 ~ death_rate_ub_vc21_nhw,
      GEOID %in% c("02063", "02066") & year==2022 ~ death_rate_ub_vc22_nhw,
      GEOID %in% c("02063", "02066") & year==2023 ~ death_rate_ub_vc23_nhw,
      GEOID %in% c("02063", "02066") & year==2024 ~ death_rate_ub_vc24_nhw,
      TRUE ~ rate_injury_death_ub_nhwhite
    ),
    rate_injury_death_nhother = case_when(
      GEOID %in% c("02063", "02066") & year==2023 ~ death_rate_vc23_nho,
      GEOID %in% c("02063", "02066") & year==2024 ~ death_rate_vc24_nho,
      TRUE ~ rate_injury_death_nhother
    ),
    rate_injury_death_lb_nhother = case_when(
      GEOID %in% c("02063", "02066") & year==2023 ~ death_rate_lb_vc23_nho,
      GEOID %in% c("02063", "02066") & year==2024 ~ death_rate_lb_vc24_nho,
      TRUE ~ rate_injury_death_lb_nhother
    ),
    rate_injury_death_ub_nhother = case_when(
      GEOID %in% c("02063", "02066") & year==2023 ~ death_rate_ub_vc23_nho,
      GEOID %in% c("02063", "02066") & year==2024 ~ death_rate_ub_vc24_nho,
      TRUE ~ rate_injury_death_ub_nhother
    )
  )
...
joined_data <- joined_data |>
  pivot_longer(
    cols = starts_with(c("rate_injury_death_", "deaths_")), 
  ...
  filter((year >= 2014 & year <= 2020 & subgroup == "all") | (year >= 2021 & year <= 2024))
...
# Remove obsolete objects
rm(death_rate_vc20, death_rate_vc21, death_rate_vc22, death_rate_vc23, death_rate_vc24)
```
...
### 4.1 Table of injury death rates
...
wide <- joined_data |> 
  pivot_wider(names_from = "subgroup", 
              values_from = c("rate_injury_death"), 
              id_cols = c(year, state, county, GEOID, state_name, county_name), 
              names_prefix = "rate_injury_death_") |> 
  select( county_name, GEOID, year, 
...
### 4.2 Distribution of injury death rates and missingness

The county-level rate of injury deaths per 100,000 people has increased from 2014-2024, but because these are crude rates instead of age-adjusted, it could be that the underlying age distribution of the population has changed.
...
The rate of missing injury death rates is about the same for each year. It is higher in 2021 than in other years, but this makes sense because our data for 2021 is a 4-year pool while other years are 5-year pools. All race subgroups except Non-Hispanic white have high rates of missing.
...
joined_data |>
  filter(!is.na(rate_injury_death) & subgroup == "all") |>
  ggplot(aes(x = rate_injury_death, fill = factor(year), color = factor(year))) +
  geom_density(alpha = 0.5) +  
  labs(title = "", fill = "Year", color = "Year") +
  theme_minimal()

```

```{r plot-distribution-subgroup}
joined_data |>
  filter(!is.na(rate_injury_death) & subgroup!="all") |>
  ggplot(aes(rate_injury_death)) +
  geom_histogram(binwidth = 5) +
...
```{r check-cv-distribution}
joined_data |>
  group_by(year) |>
  summarise(
    quantiles = list(quantile(cv, 
                              probs = c(0, 0.2, 0.4, 0.6, 0.8, 0.9, 0.99, 1), 
                              na.rm = TRUE))
  ) |>
  unnest_wider(quantiles)
...
```
Ensure that all observations with missing rate also have missing quality
```{r}
joined_data |> 
  filter(is.na(rate_injury_death) & !is.na(rate_injury_death_quality)) |> 
  count()

joined_data <- joined_data |>
  mutate(
    rate_injury_death_quality = case_when(
      is.na(rate_injury_death) ~ NA,
      TRUE ~ rate_injury_death_quality
    )
  )
...
Only a handful of observations have a data quality value of 3 (Chugach Census Area, AK and Copper River Census Area, AK in each year 2020-2024).
```{r check-quality3}
joined_data |>
  group_by(subgroup) |>
...
Note that the 26,458 county-year observations with missing quality values are those that CDC WONDER suppressed due to fewer than 10 deaths, marked as "unreliable" due to fewer than 20 deaths, or marked as "not applicable" because the population denominator figure was unavailable.
```{r}
joined_data |>
  group_by(subgroup) |>
  count(rate_injury_death_quality)
...
## 6. Save and write out data

```{r}
testthat::test_that("Check that if the rate is missing, CIs and quality are also missing", {
  
  missing_condition <- is.na(joined_data$rate_injury_death) & 
                       (!is.na(joined_data$rate_injury_death_lb) | 
                        !is.na(joined_data$rate_injury_death_ub) | 
                        !is.na(joined_data$rate_injury_death_quality))
  
  expect_false(any(missing_condition))
  
})
```
```{r}
testthat::test_that("Check that if the rate is not missing, CIs and quality are also not missing", {
...
```{r evaluate-all}
evaluate_final_data(
  exp_form_path = "10a_final-evaluation/evaluation_form_injury_death_overall_county.csv",
  data = final_all,
  geography = "county",
  subgroups = FALSE, 
...
```{r create-final-subgroup}
# Remove unnecessary variables and sort by year, state, county, and subgroup
final_subgroup <- joined_data |>
  # Subgroup observations only apply for years 2021 and later
...
```{r evaluate-subgroup}
evaluate_final_data(
  exp_form_path = "10a_final-evaluation/evaluation_form_injury_death_subgroup_county.csv",
  data = final_subgroup,
  geography = "county",
  subgroups = TRUE, 
  confidence_intervals = TRUE
...
write_csv(x = final_subgroup,
          file = here::here(
            ...
            "data",
            "final",
            "rate_injury_deaths_metric_subgroup_county.csv"
          )
)
