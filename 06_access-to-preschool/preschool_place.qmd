...
source(here::here("06_access-to-preschool", "R", "finalize_metric.R"))
source(here::here("06_access-to-preschool", "R", "calc_survey_mean.R"))
source(here::here("06_access-to-preschool", "R", "calc_survey_mean_subgroup.R"))
```
...
if(isTRUE(bucket_exists(bucket = my_bucket))){
  ...
  write_csv(metrics_education_all, paste0(here::here("06_access-to-preschool", "data", "intermediate"), 
                  "/", "metrics_education_all_", params$year, ".csv"))
  }
```
...
To save time when processing the metric, calculate the effective sample count for each income group in each place, defined as the sum of the `afact` variable. This is used to create a cutoff for data quality based on the actual number of survey records being used in the calculation. We will not include places with less than 30 effective samples.

```{r}
#| label: income-effective-samples

acs_income <- acs_crosswalked %>%
  group_by(year, sample, crosswalk_period, statefip, place, subgroup_income) %>%
  mutate(effective_sample = sum(afact)) %>% 
  ungroup()
...
```{r}
expanded_place_list_income <- place_list %>% 
  left_join(results_income, by = c("statefip", "place"), relationship = "many-to-many") %>%
  mutate(geoid = paste0(statefip, place)) %>% 
  expand(year, geoid, subgroup_income) %>% 
  mutate(statefip = str_sub(geoid, 1, 2),
         place = str_sub(geoid, 3, 7)) 

results_income_expanded <- expanded_place_list_income %>% 
  left_join(results_income, by = c("year", "statefip", "place", "subgroup_income")) %>% 
...
stopifnot(nrow(metrics_education_sub) == 486 * n_distinct(results_all_expand$year) * 7)
...
```{r}
 if(isTRUE(bucket_exists(bucket = my_bucket))){
  ...
  write_csv(metrics_education_sub, paste0(here::here("06_access-to-preschool", "data", "intermediate"), 
                  "/", "metrics_education_sub_", params$year, ".csv"))
  }
...
read_intermediate <- function(year, file_type){
  ...
  df <- read_csv(paste0(here::here("06_access-to-preschool", "data", "intermediate", "preschool"), 
...
Look at distribution of pre-K attendance share by place for all and for subgroups in latest year (set by the program parameter).
...
metrics_education_all %>% 
  ...
  theme_minimal() +
  ggtitle(paste0("Distribution of share prek_attendance, by place ", params$year, "(5-year data)")) +
  ylab("Density")
...
metrics_education_sub %>% 
  ...
  geom_density(alpha = 0.15) +
  theme_minimal() +
  ggtitle(paste0("Distribution of share prek_attendance, subgroups, by place ", params$year, "(5-year data)")) +
  ylab("Density")
...
metrics_education_sub %>% 
  ...
  facet_wrap(~subgroup) +
  labs(
    title = paste0("Distribution of share prek_attendance, subgroups, by place ", params$year, "(5-year data)"),
    y = "Density"
  ) +
  theme_minimal()
...
evaluate_final_data(data = metrics_education_all, exp_form_path = here::here("99a_final-evaluation", "evaluation_form_preschool_overall_place.csv"), geography = "place", subgroups = FALSE, confidence_intervals = TRUE)
...
evaluate_final_data(data = metrics_education_race, exp_form_path = here::here("99a_final-evaluation", "evaluation_form_preschool_race_eth_place.csv"), geography = "place", subgroups = TRUE, confidence_intervals = TRUE)
...
evaluate_final_data(data = metrics_education_income, exp_form_path = here::here("99a_final-evaluation", "evaluation_form_preschool_income_place.csv"), geography = "place", subgroups = TRUE, confidence_intervals = TRUE)
...
write_csv(metrics_education_all, 
          here::here("06_access-to-preschool", "data", "final", "metrics_preschool_place_all_longitudinal_all.csv")
          )

metrics_education_race %>% 
  head(10)

write_csv(metrics_education_race, here::here("06_access-to-preschool", "data", "final",
                                             "metrics_preschool_place_race-ethnicity_longitudinal.csv"))

metrics_education_income %>% 
  head(10)

write_csv(metrics_education_income, here::here("06_access-to-preschool", "data", "final",
                                               "metrics_preschool_place_income_longitudinal.csv"))
```
