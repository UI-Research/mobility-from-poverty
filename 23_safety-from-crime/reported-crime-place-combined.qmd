...
This metric represents the place-level rate of reported violent crimes per 100,000 people and reported property crimes per 100,000 people for years 2021-2024.
...
4. Aggregate offenses from agency to place-level
5. Calculate metric
6. Construct quality indicators
7. Validation
8. Save and write out data

All data used for the creation of metrics in this program are available on Box [here](https://urbanorg.box.com/s/zs4kyrrzcd16jbtuv5bxj2qekwl71k5t). Data stored on Box are only available to Urban Institute researchers.
...
For data years 2021-2023, the Offense Segment is available on Harvard Dataverse [here](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/TODXCL) as individual per-year `.rds` files. As of April 2026, 2024 data was not yet on Harvard Dataverse.
...
codes <- tribble(
  ...
  "sex offenses - criminal sexual contact (formerly: fondling (indecent liberties/child molest))",	        "person",
...
```{r read-offense-function}
clean_offense_data <- function(year) {
  # Construct the file name based on year
  file_name <- here::here("23_safety-from-crime", "data", "offense-segment", paste0("nibrs_offense_segment_", year, ".rds"))
...
offenses_24_agency <- clean_offense_data(year = 2024)
```
...
rm(codes, violent, property, clean_offense_data, offenses_21_agency, offenses_22_agency, offenses_23_agency, offenses_24_agency)
...
For the 2026 metric update, we used the ICPSR version (`batch_header_rds_1991_2024.zip` from V11) which includes 2024. Download and unzip, then move `nibrs_batch_header_1991_2024.rds` to the `mobility-from-poverty/23_safety-from-crime/data/batch-header-segment-and-agencies` directory.

## 4.2 Load and clean NIBRS Batch Header Files
Load Batch Header File
```{r load-bhf, message=FALSE}
batch_header_file_full <- readRDS(here::here("23_safety-from-crime", "data", "batch-header-segment-and-agencies", "nibrs_batch_header_1991_2024.rds")) |>
...
Accessing these data requires an ICPSR account. Manually downloading these data is a five-step process, but you will only need to do each of these steps once for the creation of this metric:
  ...
  5. This will download a zip file titled `ICPSR_35158-V2`. Unzip the file and navigate to `ICPSR_35158\DS0001\35158-0001-Data.tsv`. Move this file to the `mobility-from-poverty\23_safety-from-crime\data\batch-header-segment-and-agencies` directory after you have cloned the repository for this project from GitHub.
...
```{r load-leaic}
# Read in crosswalk
leaic <- read_tsv(here::here("23_safety-from-crime", "data", "batch-header-segment-and-agencies", "35158-0001-Data.tsv"))
...
Join the LEAIC to the 2021-2024 NIBRS Batch Header Files by agency.
...
```{r load-geocorr-xw}
# The original geocorr2022_county_place.csv was replaced with a longitudinal
# file that handles the Connecticut county-to-planning-region transition.
# Filter to our data years so 2021 uses old CT counties and 2022+ uses
# planning regions automatically.
county_place_xw <- read_csv(here::here("geographic-crosswalks", "data", "county_place_crosswalk_longitudinal.csv")) |>
  filter(year %in% c(2021, 2022, 2023, 2024))
...
county_place_xw <- county_place_xw |>
  mutate(
    ...
    county_fips = str_sub(as.character(str_pad(countyfip, 5, pad = "0")), 3, 5),
...
joined_ba_cpxw <- left_join(
  x = ba_mis_urb,
  y = county_place_xw,
  by = c("year", "state_fips", "county_fips")
) |>
  select(-c(GEOID, n))
```
...
The only variable we use from the ACS in this metric is `B01003_001` (total population) which we pull at the place-level to get population denominators for the crime rates.

Load ACS variable for our first and last years. Manually explore each file and spot check several observations. The naming conventions of ACS variables do not seem to change during our time period. Note that if they did we would need to split up the code that reads in the years below.
...
# 6. Construct quality indicators
This section constructs an indicator used to assess the quality of data for each place. 
...
Places with 100% of agencies reporting are coded as 1; places with 80% or more of agencies reporting OR 100% of core agencies reporting are coded as 2; places with less than 80% of agencies reporting are coded as 3; places with 0% of agencies reporting (or no agencies in the place) are coded as `NA`.
...
Check the distribution of rates of property crimes for 2021-2024.
```{r}
rates_place |>
  group_by(year) |>
  summarise(
...
Check the distribution of rates of violent crimes for 2021-2024.
```{r}
rates_place |>
  group_by(year) |>
  summarise(
...
# 8. Save and write out data
This section saves and write out the final data file. This file is long by place and year (2021-2024). It contains our place-level estimates of reported violent crimes per 100,000 people and reported property crimes per 100,000 people for each year.
...
```{r evaluate-final}
evaluate_final_data(
  exp_form_path = "10a_final-evaluation/evaluation_form_rate_crime_overall_place.csv",
  data = rates_crime_place_all,
  geography = "place",
  subgroups = FALSE, 
...
write_csv(
  ...
  file = here::here("23_safety-from-crime", "data", "final", "rates_crime_place_all.csv")
)
