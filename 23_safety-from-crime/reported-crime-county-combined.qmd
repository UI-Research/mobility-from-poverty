...
This metric represents the county-level rate of reported violent crimes per 100,000 people and reported property crimes per 100,000 people for years 2021-2024.
...
## 0.4 Suggestions for future updates
...
```{r read-offense}
offenses_24 <- readRDS(here::here("23_safety-from-crime", "data", "offense-segment", "nibrs_offense_segment_2024.rds"))

offenses_24_agency <- offenses_24 |>
...
offenses_23 <- readRDS(here::here("23_safety-from-crime", "data", "offense-segment", "nibrs_offense_segment_2023.rds"))
...
offenses_22 <- readRDS(here::here("23_safety-from-crime", "data", "offense-segment", "nibrs_offense_segment_2022.rds"))
...
offenses_21 <- readRDS(here::here("23_safety-from-crime", "data", "offense-segment", "nibrs_offense_segment_2021.rds"))
...
rm(codes, violent, property, offenses_21, offenses_21_agency, offenses_22, offenses_22_agency, offenses_23, offenses_23_agency, offenses_24, offenses_24_agency)
...
batch_header_file_long <- batch_header_file |>
  ...
  # Remove observations that are not relevant to more than one county
  drop_na(county_fips) |>
  select(-c(name))
...
### 4.4f Join offense counts onto the weight file and multiply arrest variables by population weight
...
Loving County, TX (48301) has a population less than 100 in all years with really high crime rates and Kenedy County, TX (48261) has a population less than 100 in 2023 with a really high crime rate. We change the rates for these counties to missing in all years for Loving County and in 2023 for Kenedy County.
...
# 8. Save and write out data
This section saves and write out the final data file. This file is long by county and year (2021-2024). It contains our county-level estimates of reported violent crimes per 100,000 people and reported property crimes per 100,000 people for each year.
...
```{r evaluate-final}
evaluate_final_data(
  exp_form_path = "99a_final-evaluation/evaluation_form_rate_crime_overall_county.csv",
...
write_csv(
  ...
  file = here::here("23_safety-from-crime", "data", "final", "rates_crime_county_all.csv")
)
