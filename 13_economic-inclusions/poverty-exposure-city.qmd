...
if (!file.exists(here::here("13_economic-inclusions/data/tracts.csv"))) {
  tracts_raw <- map(years, ~ get_poverty_status_data(.x, geography = "tract")) |>
    list_rbind() |>
    relocate(year, .before = everything())

  if (!dir.exists(here::here("13_economic-inclusions/data"))) {
    dir.create(here::here("13_economic-inclusions/data"))
  }
  write_csv(tracts_raw, here("13_economic-inclusions/data/tracts.csv"))
}
```
...
tracts_raw <- read_csv(here::here("13_economic-inclusions/data/tracts.csv"))
```
...
if (!file.exists(here::here("13_economic-inclusions/data/places.csv"))) {
  places_test <-
    map(years, ~ get_poverty_status_data(.x, "place")) |>
    list_rbind() |>
    relocate(year, .before = everything())

  write_csv(places_test, here("13_economic-inclusions/data/places.csv"))
}
```

```{r}
places_test <- read_csv(here::here("13_economic-inclusions/data/places.csv"))
...
```{r}
final_city_data_subgroups <- read_csv(here::here("13_economic-inclusions/final/poverty-exposure_race-ethnicity_city.csv"), show_col_types = FALSE)
