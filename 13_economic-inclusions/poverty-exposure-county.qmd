...
if (!file.exists(here::here("13_economic-inclusions/data/counties.csv"))) {
  counties_test <-
    map(years, ~ get_poverty_status_data(.x, "county")) |>
    list_rbind() |>
    relocate(year, .before = everything())

  write_csv(counties_test, here("13_economic-inclusions/data/counties.csv"))
}
```

```{r}
counties_test <- read_csv(here::here("13_economic-inclusions/data/counties.csv"))
...
```{r}
final_county_data_subgroups <- read_csv(here::here("13_economic-inclusions/final/poverty-exposure_race-ethnicity_county.csv"), show_col_types = FALSE)
