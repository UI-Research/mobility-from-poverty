...
get_ipums_extract <- function(acs_vars, data_collection, data_description, sample) {
  ...
  data_path <- ipumsr::download_extract(
   acs_ext_complete, 
   download_dir = here("10_digital-access/data/raw/ipums")
   )
...
The following code chunk downloads the required IPUMS data only if no files exist in the "10_digital-access/data/raw/ipums" folder. If the required data is already downloaded, then please manually set the file
paths to point to the downloaded `xml` files for the program to read the data. Otherwise remove all files from folder "10_digital-access/data/raw/ipums" to re-download the data sets.
...
fs::dir_create(here("10_digital-access/data/raw/ipums"), recurse = TRUE)

# Download the IPUMS extracts for years 2018, 2021 and 2023 and get xml file paths

if ( length( list.files(here("10_digital-access/data/raw/ipums")) ) == 0 ) {
...
# different value for whoever re-runs this code file
  ...
  ipums_xml_2018 <- here("10_digital-access/data/raw/ipums/usa_00022.xml")
  ipums_xml_2021 <- here("10_digital-access/data/raw/ipums/usa_00023.xml")
  ipums_xml_2023 <- here("10_digital-access/data/raw/ipums/usa_00024.xml")
}
...
evaluate_final_data(exp_form_path = "99a_final-evaluation/evaluation_form_digital_access_overall_county.csv",
    data = final_share_dig_acc_county, geography = "county",
    subgroups = FALSE, confidence_intervals = FALSE)

evaluate_final_data(exp_form_path = "99a_final-evaluation/evaluation_form_digital_access_race_ethnicity_county.csv",
    data = final_share_dig_acc_county_race_ethnicity, geography = "county",
    subgroups = TRUE, confidence_intervals = FALSE)

evaluate_final_data(exp_form_path = "99a_final-evaluation/evaluation_form_digital_access_income_county.csv",
...
evaluate_final_data(exp_form_path = "99a_final-evaluation/evaluation_form_digital_access_overall_place.csv",
    data = final_share_dig_acc_place, geography = "place",
    subgroups = FALSE, confidence_intervals = FALSE)

evaluate_final_data(exp_form_path = "99a_final-evaluation/evaluation_form_digital_access_race_ethnicity_place.csv",
    data = final_share_dig_acc_place_race_ethnicity, geography = "place",
    subgroups = TRUE, confidence_intervals = FALSE)

evaluate_final_data(exp_form_path = "99a_final-evaluation/evaluation_form_digital_access_income_place.csv",
...
Lastly, after successful evaluation we save the final data files in folder `10_digital-access/data/final`.
...
save_final_data <- function(data, subgroups, subgroup_var = NA, geo) {
  ...
  file_path <- here::here("10_digital-access", "data", "final", file_name)
