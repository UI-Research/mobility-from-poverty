...
Check the distribution of the cancer metric by year.There are some significant outliers in the data where tracts have a very high cancer hazard risk. 
The second graph removes outliers and shows that after 2014 the EPA started rounding this statistic to the nearest 10th (not sure if this was intentional or an unintended consequence of using the xlsx format).
```{r}
cancer_data %>% 
...
The data contains some results that are not at the tract levle. These rows can be identified because the tract ID will be entirely zeros. Count the number of these rows.
...
Remove these rows from the data. This leaves us with approximately 73,450 tracts per year (note 2014 has one additioanl tract).
...
```{r}
enviro_index_all <- enviro_all_clean %>% 
  group_by(year) %>% 
...
Visualize the distribution to see how it looks. Note that 2018 and 2019 have significant bunching due to the rounding of the variables to the nearest 10th.
```{r}
haz_index_all %>% 
  mutate(year = factor(year)) %>% 
...
Add tract-level subgroup information which breaks out the racial composition and poverty level of each census tract. First we will read in tract level population data. Total population and total white, non-Hispanic. Then total population in poverty.
```{r}
...
Use ACS data to create a variable for percent of each tract that are people of color and the percent in poverty. Remove the estimate indicator "E" from value names.
```{r}
...
```{r}
haz_index_county_pov_expand  <- haz_index_acs  %>%
...
haz_by_poverty <- haz_index_county_pov_expand  %>% 
  left_join(haz_index_sub_poverty %>%
                                     select(year, county_id, state, county, poverty_type, index_air_hazard, na_perc),
                                   by=c("year", 
                                        "county_id",
                                        "poverty_type")) %>%
...
Expand dataset for every year, county and race/ethnicity tract designation.
```{r}
haz_index_county_sub_expand_race <- haz_index_acs %>%
  expand(year, county_id, race_ind)
```

Join to expanded, add geo variables, and add subgroup variables.
```{r}
haz_by_race <- haz_index_county_sub_expand_race %>% 
...
crosswalk_place <- read_csv(here::here("geographic-crosswalks", "data", "geocorr2018_tract_to_place.csv")) %>% 
...
```{r}
crosswalk_place <- crosswalk_place %>% 
...
```{r}
tract_place_hazard <- crosswalk_place %>% 
...
anti_join(crosswalk_place, haz_index_all, by = c("year", "tract")) %>% 
  mutate(state = str_sub(tract, start = 1, end = 2)) %>% 
  group_by(state, year) %>%
  count() %>% 
  arrange(desc(n))
...
hazard_place_sub_pov <- hazard_place_pov %>%
  ...
  summarise(index_air_hazard = weighted.mean(index_air_hazard, total_pop*afact, na.rm = TRUE), #Multiplied weight by % in Tract
...
```{r}
hazard_place_pov_expand <- haz_index_acs  %>%
  expand(year, place_id, poverty_type)
```

Join poverty data set.
```{r}
hazard_by_pov_sub_place <- hazard_place_pov_expand %>% 
...
```{r}
hazard_place_race_expand <- haz_index_acs  %>%
  expand(year, place_id, race_ind)
```

Join to expanded, add GEO variables, and add subgroup variables
```{r}
hazard_by_race_sub_place <- hazard_place_race_expand %>% 
...
write_csv(final_data_county_sub, here::here("06_neighborhoods", "environment", "data", "final", "environment_county_race_poverty_longitudinal.csv"))
```
...
write_csv(final_data_place, here::here("06_neighborhoods", "environment", "data", "final", "environment_place_race_poverty_longitudinal.csv"))
```
