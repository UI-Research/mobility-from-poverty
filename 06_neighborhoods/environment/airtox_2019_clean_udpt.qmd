...
Downloaded data is available in the Air Quality folder inside the Metrics_2024_round 
[box folder](https://urbanorg.box.com/s/n82wl2bv8n2nmrdcgwa07q1dyn5bldvc).
...
Combine the years by variable of interest. 
First select only cases where total_cancer_risk is filled in and join they years.
```{r}
...
Create the Environemtnal Health Indicator using the statistics calculated above. First calcualte the components, which is the individual health indicator for a tract minus the mean of that indicator and divided by the standard deviation.
...
Add tract-level subgroup information which breaks out the racial composition of the census tract.
...
Use ACS data to create a variable for percent POC and percent in poverty, by tract.
```{r}
...
haz_index_county <- crosswalk_cnty %>% 
  left_join(haz_index_all, by = "tract")
...
### All
Join onto the environmental hazard index data. Calculate the number of population from tracts where the index data is missing.
```{r}
...
Calculate the average of county level hazard index for each year. Weight the mean by the total population in tracts inside the county.
```{r}
...
Create weighting indicator for the poverty categorization of each tract.
```{r}
...
```{r}
haz_index_county_sub_expand  <- haz_index_county_sub  %>%
  expand(year, geoid, poverty_type)
```

Make dataset of unique state/county pairs to join to
```{r}
state_county <- haz_index_county_sub %>%
  transmute(year, geoid = str_c(state, county), state, county) %>%
  distinct()
```
