...
**Check:** Are there exactly 306 missing values?

```{r}
stopifnot(
  sum(is.na(merged_sa$count_membership_associations_per_10k)) == 306
)
```
...
merged_sa %>%
  ...
  write_csv(here("06_neighborhoods/social-capital/data/social_associations_2021_county.csv"))
```
