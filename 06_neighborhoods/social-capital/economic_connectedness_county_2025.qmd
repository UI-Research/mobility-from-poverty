...
merged_ec <- county_pop |>
                  ...
                  # update the year to 2022 as the Social Capital Atlas was published in 2022
                  mutate(year = 2022)
...
# update economic connectedness scaled variable by multiplying it with 50% to get the percentage values
# standard error for economic connectedness also multiplied by 50% 

merged_ec <- merged_ec |>
              mutate(economic_connectedness = ec_county * 50,
                     ec_se_county_updated = ec_se_county * 50)
...
#| label: visualize-updated-economic-connectedness
 ...
 merged_ec |>
    drop_na() |>
    ggplot(aes(x = economic_connectedness)) +
    geom_histogram(color = "blue", fill = alpha("blue", 0.3), binwidth = 0.8) +
    geom_density(aes(y = ..count.. * 0.8), color = "red", size = 1, alpha = 0.2) +
    labs(
      title = paste0("Economic Connectedness, County-Level"),
      y = "Count",
...
#| label: visualize-updated-standard-error-economic-connectedness
 ...
 merged_ec |>
    ...
    geom_histogram(color = "blue", fill = alpha("blue", 0.3), binwidth = 0.03) +
    geom_density(aes(y = ..count.. * 0.03), color = "red", size = 1, alpha = 0.2) +
    labs(
      title = paste0("Standard Error in Economic Connectedness, County-Level"),
      y = "Count",
...
merged_ec <- merged_ec |>
    mutate(
      ...
      economic_connectedness_quality = case_when(economic_connectedness >= 0 ~ 1),
      # calculate upper bound and lower bound from standard error
      economic_connectedness_lb = economic_connectedness - qnorm(0.975) * ec_se_county_updated,
      economic_connectedness_ub = economic_connectedness + qnorm(0.975) * ec_se_county_updated
           )
...
merged_ec |>
    drop_na() |>
    ggplot(aes(x = economic_connectedness)) +
    geom_histogram(color = "blue", fill = alpha("blue", 0.2), binwidth = 1) +
    geom_density(aes(y = ..count.. * 1, color="Metric Value"), size = 1, alpha = 0.2) +
    geom_density(
      aes(x = economic_connectedness_lb,y = ..count.. * 1, color = "Lower Bound"), 
       size = 1, alpha = 0.2
      ) +
    geom_density(
      aes(x = economic_connectedness_ub,y = ..count.. * 1, color = "Upper Bound"), 
      size = 1, alpha = 0.2
      ) +
  scale_color_manual(
    name = "Density",
    ...
    labs(
      title = paste0("Economic Connectedness with Lower & Upper Bounds"),
      y = "Count",
      x = "Economic Connectedness (Updated)"
    ) +
    theme_minimal()
...
ec_final_data <- merged_ec |> 
              select(year, state, county, economic_connectedness, economic_connectedness_lb,
                     economic_connectedness_ub, economic_connectedness_quality)
