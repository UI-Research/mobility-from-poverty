...
Create the merged file where the SA numerator (`count_orgs`) is averaged per Place, weighted by the % area of the ZCTA in that Place (`avg_orgs_per_place_weighted_by_area`) and also include total ZCTAs in Place (`zip_total`) & how many of those fully fall within the Place (`zips_in`)
