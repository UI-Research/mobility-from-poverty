...
Import and clean the 2010 ZCTA -\> 2021 Census Place crosswalk file to prepare for merge
...
**Check:** Are there exactly 4,565 missing values in the `new_est_zip` variable?

```{r}
stopifnot(
  sum(is.na(merged_sa_zip_city$avg_orgs_per_place_weighted_by_area)) == 4565
)
```
...
We can use this ratio to create a data quality tag for the final file. In particular, these values are:
...
sa_city_data %>% 
  ...
  write_csv(here("06_neighborhoods/social-capital/data/social_associations_all_city.csv"))

sa_city_data %>% 
  filter(year == 2021) %>% 
  select(-year) %>% 
  arrange(state, place) %>%
  write_csv(here("06_neighborhoods/social-capital/data/social_associations_2021_city.csv"))
```
