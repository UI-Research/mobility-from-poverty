...
```{r}
ggplot(sa_city_data, aes(x = count_membership_associations_per_10k)) +
...
```{r}
ggplot(sa_city_data, aes(y = count_membership_associations_per_10k)) +
  geom_boxplot() +
  labs(
...
gg_miss_var(sa_city_data) +
  labs(title = "Missing Values by Variable")
```

```{r}
yearly_avg <- sa_city_data %>%
  group_by(year) %>%
...
evaluate_final_data(here("06_neighborhoods/social-capital/data/final_data_evaluation_form.csv"), sa_city_data, "place", subgroups = FALSE, confidence_intervals = FALSE)
```
