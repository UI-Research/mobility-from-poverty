...
cbp_zip <-
  ...
  summarize(count_orgs = sum(orgs), .by = "zip_code") %>%
  # 28,599 observations to 16,644 observations
  mutate(year = 2021)
```

## Merge with the 2010 ZCTA -\> 2021 Census Place crosswalk

Import the 2010 ZCTA -\> 2021 Census Place crosswalk file

```{r}
#| label: import-crosswalk
zcta_place_xwalk <- read_csv(here("geographic-crosswalks/data/2010_ZCTA_2021_Census_Places_Crosswalk.csv")) %>%
  as_tibble()
```
...
```{r}
#| label: clean-crosswalk
zcta_place_xwalk <- zcta_place_xwalk %>%
  rename(
    zip_code = ZCTA5CE10,
    state_fips = STATEFP,
    place = PLACEFP,
...
**Check:** Do 2079 of these ZCTAs fall fully into a Census Place?
...
zcta_place_xwalk <-
  zcta_place_xwalk %>%
  select(zip_code, state_fips, place, place_name, IntersectArea, ZCTAinPlace, portion_in)
```
...
merged_sa_zip_city <-
  ...
  summarize(
    ...
    avg_orgs_per_place_weighted_by_area = weighted.mean(count_orgs, ZCTAinPlace),
    .by = c(state_fips, place)
  )
```

**Check:** Are there exactly 2,277 missing values in the `new_est_zip` variable?

```{r}
stopifnot(
  sum(is.na(merged_sa_zip_city$avg_orgs_per_place_weighted_by_area)) == 2277
)
```
...
Import the updated population-cutoff Places file
...
stopifnot(
  ...
  median(sa_city_data$zip_ratio) == 0,
  near(mean(sa_city_data$zip_ratio), 0.09, tol = 0.01),
  near(max(sa_city_data$zip_ratio), 0.85, tol = 0.01)
)
```

We can use this ratio to create a data quality tag for the final file. In particular, these values are - Data Quality 1 = 10% or more of the ZIPs fall mostly in the census place - Data Quality 2 = less than 10% of the ZIPs fall mostly in the census place
...
write_csv(
  sa_city_data,
  here("06_neighborhoods/social-capital/data/social_associations_city_2021.csv")
)
```
