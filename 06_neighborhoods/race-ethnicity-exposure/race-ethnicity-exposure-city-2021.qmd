...
tracts_raw <- map(years, ~get_vars(year = .x,
                               vars = vars,
                               geography = "tract",
...
**Note** : This is not true for 2016 and 2023, why?
```{r check-tract-sums}
stopifnot(
  tracts %>%
...
crosswalk_20 <-
  inner_join(
    crosswalk_20,
    distinct(places_of_interest, state_place, place_name),
    by = "state_place"
  )
...
test_joined %>%
  ...
  select(state, place, place_name, people_reported, people_interpolated) %>%
