...
# combine the point estimes and margins of error
other_moe <- left_join(values, moes, by = c("state", "county", "tract", "group"))
    
rm(moes, values)

# keep moes for non-zero estimates and keep the largest moe for zero estimates
other_moe <- other_moe %>%
...
tracts <- tracts %>%
  select(state, county, tract, people, hispanic, black_nh, other_nh, white_nh) 
...
Look at the margins of error. A large share of the Other Races and Ethnicities have coefficients of variaiton greater than 0.4.
...
tracts_moe %>%
  ...
  geom_abline(aes(slope = 0.4, intercept = 0)) +  
    labs(title = "Other Races and Ethnicities",
       subtitle = "Line represents a CV of 0.4") +
  coord_equal() +
  scatter_grid()
...
county_data <- county_data %>%
  ...
  mutate(other_nh_exposure_quality = set_quality(other_nh, white_nh_exposure))
