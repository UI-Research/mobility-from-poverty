...
tracts_cv <- tracts %>%
  mutate(
    hispanic_cv = hispanic_moe / hispanic,
    black_nh_cv = black_nh_moe / black_nh,
    white_nh_cv = white_nh_moe / white_nh,
    aian_nh_cv = aian_nh_moe / aian_nh,
    nhpi_nh_cv = nhpi_nh_moe / nhpi_nh,
    census_other_nh_cv = census_other_nh_moe / census_other_nh,
    two_or_more_nh_cv = two_or_more_nh_moe / two_or_more_nh
  )

```

Most tracts have very large coefficients of variation. Some of these tracts will be suppressed. Others will be included in calculations but have lower quality scores. Finally, averaging on the county level will reduce some of the imprecision. 
...
```{r}
tracts_moe %>%
  summarize(mean(other_nh_moe / other_nh > 0.4),
            mean(other_nh_moe_reduced / other_nh > 0.4))

```

```{r}
tracts_moe %>%
  ggplot(aes(black_nh, black_nh_moe)) +
  geom_point(alpha = 0.1, size = 1) +
  geom_abline(aes(slope = 0.4, intercept = 0)) +  
  labs(title = "Most Black, non-Hispanic Estimates Have Large MOE",
       subtitle = "Line represents a CV of 0.4") +  
  coord_equal() +
  scatter_grid()

tracts_moe %>%
  ggplot(aes(hispanic, hispanic_moe)) +
  geom_point(alpha = 0.1, size = 1) +
  geom_abline(aes(slope = 0.4, intercept = 0)) +
...
tracts_moe %>%
  ggplot(aes(other_nh, other_nh_moe)) +
...
tracts_moe %>%
  ggplot(aes(white_nh, white_nh_moe)) +
  geom_point(alpha = 0.1, size = 1) +
  geom_abline(aes(slope = 0.4, intercept = 0)) +  
...
county_data <- indices %>%
  ...
  summarize(
    ...
    # margins of errors
    black_nh_moe = sqrt(sum(black_nh_moe ^ 2)),
    hispanic_moe = sqrt(sum(hispanic_moe ^ 2)),
    other_nh_moe = sqrt(sum(other_nh_moe ^ 2)),
    white_nh_moe = sqrt(sum(white_nh_moe ^ 2)),
    # exposures
...
county_data <- county_data %>%
  mutate(
    black_nh_cv = black_nh_moe / black_nh,
    hispanic_cv = hispanic_moe / hispanic,
    other_nh_cv = other_nh_moe / other_nh,
    white_nh_cv = white_nh_moe / white_nh
  ) 
