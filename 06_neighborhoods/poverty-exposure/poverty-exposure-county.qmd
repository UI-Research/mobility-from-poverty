...
tracts_raw <-
  map(years, ~get_poverty_status_data(.x, "tract")) |> 
  list_rbind() |> 
...
write_csv(counties_test, here::here("06_neighborhoods/poverty-exposure/data/counties.csv"))
...
counties_test <- counties_test %>%
  ...
  select(year, state, county, poverty_rate_test) %>%
