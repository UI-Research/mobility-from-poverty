...
There was a data collection error in Rio Arriba County, NM that affected 2018 ACS estimates ([source](https://www.census.gov/programs-surveys/acs/technical-documentation/errata/125.html)). We dropped these observations in 2018, but the error does not apply to the 2021 5-year file, so we do not drop these observations. 
Check the number of people. It should be around [321,897,703](https://data.census.gov/table?q=B17001&tid=ACSDT5Y2021.B17001&hidePreview=false).
...
counties_summary %>%
  ...
  select(state, county, share_poverty_exposure)
...
counties_summary_subgroups_plots <- counties_summary %>%
  ...
  # pivot to very long
  pivot_longer(c(-state, -county), names_to = "subgroup", values_to = "share_poverty_exposure") %>%
  # clean up names
  mutate(subgroup = 
           recode(
             subgroup,
...
counties_summary_subgroups_plots <- left_join(counties_summary_subgroups_plots, select(counties_summary, -share_poverty_exposure), by = c("state", "county"))
...
counties_summary <- counties_summary %>%
  mutate(
    share_poverty_exposure_quality = set_quality(cv = poverty_cv, exposure = share_poverty_exposure),
    share_poverty_exposure_black_quality = set_quality(cv = poverty_black_cv, exposure = share_poverty_exposure_black),
    share_poverty_exposure_hispanic_quality = set_quality(cv = poverty_hispanic_cv, exposure = share_poverty_exposure_hispanic),
    share_poverty_exposure_other_races_quality = set_quality(cv = poverty_other_races_cv, exposure = share_poverty_exposure_other_races),
    share_poverty_exposure_white_nonhispanic_quality = set_quality(cv = poverty_white_nonhispanic_cv, exposure = share_poverty_exposure_white_nonhispanic)
  )

count(counties_summary, share_poverty_exposure_quality)
count(counties_summary, share_poverty_exposure_black_quality)           
count(counties_summary, share_poverty_exposure_hispanic_quality)
count(counties_summary, share_poverty_exposure_other_races_quality)
count(counties_summary, share_poverty_exposure_white_nonhispanic_quality)
...
write_csv(final_data,
          here::here("06_neighborhoods", "poverty-exposure", "poverty-exposure_county_2021.csv"))
...
stopifnot(min(counties_summary_subgroups$share_poverty_exposure, na.rm = TRUE) >= 0)
stopifnot(max(counties_summary_subgroups$share_poverty_exposure, na.rm = TRUE) <= 1)
...
write_csv(final_data_race_ethnicity,
          here::here("06_neighborhoods", "poverty-exposure", "poverty-exposure_race-ethnicity_county_2021.csv"))
