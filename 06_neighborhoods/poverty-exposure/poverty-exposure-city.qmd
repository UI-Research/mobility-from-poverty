...
get_poverty_status_data <- function(year){
...
values <- tracts %>%
  select(state, 
         ...
         poverty_other,
         poverty_twoplus) %>%
  pivot_longer(c(-state, -county, -tract, -year), names_to = "group", values_to = "value")
...
moes <- tracts %>%
  select(year,
         ...
         poverty_other_moe,
         poverty_twoplus_moe) %>%
  pivot_longer(c(-state, -county, -tract, -year), names_to = "group", values_to = "moe") %>%
...
places_summary <- tracts_joined %>%
  group_by(state, place, state_place, place_name, year) %>%
  summarize(
