...
tracts_raw <-
  map(years, ~get_poverty_status_data(.x, geography = "tract")) |> 
  list_rbind() |> 
  relocate(year, .before = everything())

if(!fs::dir_exists(here::here("06_neighborhoods/poverty-exposure/data"))){
  fs::dir_create(here::here("06_neighborhoods/poverty-exposure/data"))
}
...
crosswalk_10 <-
  read_csv(here::here("geographic-crosswalks", "data", "tract-place-crosswalk_2018.csv"), skip = 1) |> 
    ...
    place = `Place (2014)`, 
    tract = Tract, 
    afact = `tract to placefp14 allocation factor`,
    afact2 = `placefp14 to tract allocation factor`
  ) %>%
  mutate(county = substring(county, 3, 5),
         state_place = str_c(state, place),
         tract = str_remove(string = tract, pattern = "[.]")) %>%
...
places_test <- places_test %>%
  ...
  select(year, state, place, state_place, poverty_rate_test, people) %>%
...
```{r}
anti_join(places_of_interest |> filter(year %in% years), final_data,
