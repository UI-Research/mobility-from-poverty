...
vars <- c(
  ...
  "DP03_0128PM"  # MOE!!PERCENTAGE OF FAMILIES AND PEOPLE WHOSE INCOME IN 
)                # THE PAST 12 MONTHS IS BELOW THE POVERTY LEVEL!!All people
...
# B01003_001E is included in get_vars()
tracts <- tracts %>%
...
Check the number of people. It should be somewhere between 320 million and 330 million. 
...
Finally, we calculate the share of people who are excluded fromt he poverty calculation in each tract. 
...
Check to ensure that the anti_join() returns an empty data frame. 
...
counties_joined %>%
  ...
  coord_equal() +
  scatter_grid() +
  labs(title = "There is a Close Match Between the Reported County Population\nand the Tract Population Summed to Counties")
...
* `NA` - if the metric is missing. This is the case for the county in Illinois.
* `1` - nothing
