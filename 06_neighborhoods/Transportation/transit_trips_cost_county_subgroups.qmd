...
72739 observations for each (trips and cost) in 2015.
...
72742 observations for each (trips and cost) in 2019.
...
```
84095 observations for both files (cost and trips) in 2020.
...
Collapse to the county level to create ALL values (to be appended later). In order to collapse, we find the weighted
mean of all the counties, and weight by households (the number of households that fall in that community)
...
Now make sure to change the trips to national percentile ranks (this is how we normalize, instead of reporting the raw data).
...
```{r}
non_na_rows <- !is.na(trips_all_20$index_transit_trips)
trips_all_20$rank[non_na_rows] <- rank(trips_all_20$index_transit_trips[non_na_rows])
...
trips_all_20$percentile_rank <- (trips_all_20$rank - 1) / (nrow(trips_all_20) - 1) * 100
...
trips_all_20$index_transit_trips <- round(trips_all_20$percentile_rank, 2)
...
cat("Number of rows in trips_all_15:", nrow(trips_all_15), "\n")

trips_all_19 <- trips_all_19 %>%
  select(state, county, index_transit_trips, subgroup_type, subgroup, households)
cat("Number of rows in trips_all_19:", nrow(trips_all_19), "\n")

trips_all_20 <- trips_all_20 %>%
  select(state, county, index_transit_trips, subgroup_type, subgroup, households)
cat("Number of rows in trips_all_20:", nrow(trips_all_20), "\n")

```

2015 and 2019 have 3143 observations each. 2020 has 3144 observations (due to the added Alaska county) - this all makes sense and is as expected.
...
Create a second dummy df for different number of counties 2020 and beyond (Alaskan county split in two)
```{r}
county_expander2 <- expand_grid(
  count(cost_by_race_20, state, county) %>% select(-n),
  race_category = c("Majority White-NH Tracts", "Majority Non-White Tracts", "Mixed Race and Ethnicity Tracts")
)

```

Merge each of the six datasets into the appropriate dummy to account for all race categories
...
cat("Number of rows in cost_by_race_15:", nrow(cost_by_race_15), "\n")
```
...
cost_by_race_19 <- left_join(county_expander, cost_by_race_19, by = c("state", "county", "race_category")) %>%
  ...
  rename(subgroup = race_category,
         index_transportation_cost = transit_cost)
cat("Number of rows in cost_by_race_19:", nrow(cost_by_race_19), "\n")
...
```{r}
cost_by_race_20 <- left_join(county_expander2, cost_by_race_20, by = c("state", "county", "race_category")) %>%
  mutate(subgroup_type = "race-ethnicity") %>%
  rename(subgroup = race_category,
         index_transportation_cost = transit_cost)
cat("Number of rows in cost_by_race_20:", nrow(cost_by_race_20), "\n")

```

9432 obs = 3*3144 counties (for post-2019) -> good to go 

```{r}
trips_by_race_15 <- left_join(county_expander, trips_by_race_15, by = c("state", "county", "race_category")) %>%
  mutate(subgroup_type = "race-ethnicity") %>%
  rename(subgroup = race_category)
cat("Number of rows in trips_by_race_15:", nrow(trips_by_race_15), "\n")
...
trips_by_race_19 <- left_join(county_expander, trips_by_race_19, by = c("state", "county", "race_category")) %>%
  mutate(subgroup_type = "race-ethnicity") %>%
  rename(subgroup = race_category)
cat("Number of rows in trips_by_race_19:", nrow(trips_by_race_19), "\n")

```

```{r}
trips_by_race_20 <- left_join(county_expander2, trips_by_race_20, by = c("state", "county", "race_category")) %>%
  mutate(subgroup_type = "race-ethnicity") %>%
  rename(subgroup = race_category)
cat("Number of rows in trips_by_race_20:", nrow(trips_by_race_20), "\n")

```

Same number of observations across all three years for the trips' datasets - good to go.
...
Not understanding why it looks like there are negative values... Let's look at summary stats
...
```{r}
any(trips_by_race_15$index_transit_trips < 0, na.rm = TRUE)

#stopifnot(min(trips_by_race_15$index_transit_trips, na.rm = TRUE) >= 0)

```

Seems to be no negative values. Good to go
...
cost_by_race_19 <- cost_by_race_19 %>% 
  mutate(index_transportation_cost_quality = ifelse(households > 30, 1, ifelse(households < 30, 3, NA)))
cost_by_race_20 <- cost_by_race_20 %>% 
  mutate(index_transportation_cost_quality = ifelse(households > 30, 1, ifelse(households < 30, 3, NA)))
...
```{r}
transit_cost_subgroup_county <- rbind(cost_race_15, cost_race_19, cost_race_20)
cat("Number of rows in transit_cost_subgroup_county:", nrow(transit_cost_subgroup_county), "\n")

transit_trips_subgroup_county <- rbind(trips_race_15, trips_race_19, trips_race_20)
cat("Number of rows in transit_trips_subgroup_county:", nrow(transit_trips_subgroup_county), "\n")

```
37720 observations for each combined file.
