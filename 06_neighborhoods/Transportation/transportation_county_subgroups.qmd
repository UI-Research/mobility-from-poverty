...
```{r}
transportation_county <- combined_tracts %>%
...
target_statistics <- read_csv(here("geographic-crosswalks", "data", "county-populations.csv")) |>
  ...
  expand_grid(subgroup = c("Neighborhoods of color", "White neighborhoods", "Mixed neighborhoods")) |>
...
ggplot(transportation_county, aes(x = count_transit_trips)) +
  ...
  facet_wrap(subgroup ~ year, nrow = 2) +
  labs(
...
transportation_county <- transportation_county |>
  mutate(
    ...
    index_transportation_cost_quality = if_else(households > 30, true = 1, false = 3)
  )
