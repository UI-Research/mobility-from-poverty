...
abstract: This script calculates `share_transportation_cost` and `share_transit_trips` using Center for Neighborhood Technology (CNT) and Census Bureau data. 
date: today
format:
  html:
    toc: true
...
transportation_tracts <- transportation_tracts |>
  select(
    ...
    share_transit_trips = pct_transit_commuters_80ami, 
...
transportation_tracts <- transportation_tracts |>
  mutate(
    share_transit_trips = share_transit_trips / 100,
...
transportation_county_all <- combined_tracts |>
  ...
  summarize(
    n = n(),
    share_transit_trips = weighted.mean(x = share_transit_trips, w = households),
...
```{r}
transportation_county |>
  filter(is.na(share_transit_trips)) |>
  count(subgroup)
...
```{r}
stopifnot(min(transportation_county$share_transit_trips, na.rm = TRUE) >= 0)
```

Create a histogram.

```{r}
ggplot(transportation_county, aes(x = share_transit_trips)) +
  geom_histogram() + 
...
```{r}
summary(transportation_county$share_transit_trips)
```
...
transportation_county |>
  group_by(year) |>
  slice_max(n = 10, order_by = share_transit_trips) |>
  select(year, state, county, subgroup, share_transit_trips) |>
  print(n = Inf)
...
transportation_county |>
  ...
  select(state, county, year, subgroup, households, share_transit_trips) |>
  pivot_wider(
    names_from = year, 
    values_from = c(households, share_transit_trips)
  ) |>
  mutate(size = if_else(households_2019 > 200000, "Large counties", "Small counties")) |>
  ggplot(aes(share_transit_trips_2015, share_transit_trips_2019, size = households_2019)) +
...
transportation_county |>
  ...
  ggplot(aes(share_transit_trips_2019, share_transit_trips_2022, size = households_2022)) +
  geom_point(alpha = 0.1) +
  facet_grid(size ~ subgroup) +
  coord_equal() +
  labs(subtitle = "Large counties have at least 200,000 households")
...
transportation_county |>
  ...
  mutate(change_share_transit_trips = share_transit_trips - lag(share_transit_trips)) |>
  ungroup() |>
  filter(year %in% c(2019, 2022)) |>
  slice_max(order_by = abs(change_share_transit_trips), n = 10) |>
  select(year, state, county, subgroup, change_share_transit_trips)
...
transportation_county <- transportation_county |>
  mutate(
    share_transit_trips_quality = case_when(
  ...
  mutate(
    share_transit_trips_quality = if_else(is.na(share_transit_trips), NA, share_transit_trips_quality),
...
med_hh_income_cv <- med_hh_income_cv |>
  ...
  select(
    year, state, county, 
    share_transit_trips_quality_county = share_transit_trips_quality, 
...
transportation_county <- transportation_county |>
  mutate(
    share_transit_trips_quality = 
      pmax(share_transit_trips_quality, share_transit_trips_quality_county),
...
```{r}
count(transportation_county, year, share_transit_trips_quality)
...
stopifnot(
  transportation_county |>
    filter(
      (!is.na(share_transit_trips) & is.na(share_transit_trips_quality)) |
...
final_data <- transportation_county |>
  select(year, state, county, subgroup_type, subgroup, share_transit_trips, share_transit_trips_quality, share_transportation_cost, share_transportation_cost_quality) 
