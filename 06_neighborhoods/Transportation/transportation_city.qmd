...
transportation_tracts <- transportation_tracts |>
  rename(GEOID = tract) |>
  mutate(GEOID = str_remove_all(GEOID, pattern = "\"")) |>
  mutate(
...
# tract_place <- tract_place |>
  # filter(state_place %in% paste0(places$state, places$place))

```

## Join and Aggregate

The CNT data contains tracts not included in our places. 
...
Our places include tracts not included in the CNT data. 
