...
transportation_county |>
  ...
  select(state, county, year, households, share_transit_trips) |>
  pivot_wider(
    names_from = year, 
...
transportation_county |>
  ...
  mutate(share_transit_trips_change = share_transit_trips - lag(share_transit_trips)) |>
  ungroup() |>
  filter(year %in% c(2019, 2022)) |>
  slice_max(order_by = abs(share_transit_trips_change), n = 10) |>
  select(year, state, county, households, share_transit_trips, share_transit_trips_change) |>
  print(n = Inf)
...
transportation_county |>
  ...
  select(-population, -share_transit_trips) |>
  print(n = Inf)
...
transportation_county <- transportation_county |>
  mutate(
    share_transit_trips_quality = 
      if_else(is.na(share_transit_trips), NA, share_transit_trips_quality), 
...
final_data <- transportation_county |>
  select(year, state, county, share_transit_trips, share_transit_trips_quality, share_transportation_cost, share_transportation_cost_quality) |>
