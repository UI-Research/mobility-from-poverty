...
```{r}
tract_place <- read_csv(here("geographic-crosswalks", 
                             "data", 
...
The CNT data contains tons of tracts not included in our places. Our places include tracts not included in the CNT data. 
...
transportation_places_all <- transportation_tracts_places |>
  ...
  summarize(
    ...
    sample_size = sum(sample_size * afact)
  ) |>
  ungroup() |>
  dplyr::mutate(
