...
transportation_places_all <- transportation_tracts_places |>
  ...
  summarize(
    n = n(),
    share_transit_trips = weighted.mean(share_transit_trips, w = households * afact, na.rm = TRUE),
...
```{r}
stopifnot(min(transportation_city$share_transit_trips, na.rm = TRUE) >= 0)
```
...
```{r}
ggplot(transportation_city, aes(x = share_transit_trips)) +
  geom_histogram() + 
...
```{r}
summary(transportation_city$share_transit_trips)
```
...
transportation_city |>
  ...
  select(state, place, year, subgroup, population, share_transit_trips) |>
  pivot_wider(
    names_from = year, 
    values_from = c(population, share_transit_trips)
  ) |>
  mutate(size = if_else(population_2019 > 400000, "Large cities", "Small cities")) |>
  ggplot(aes(share_transit_trips_2015, share_transit_trips_2019, size = population_2019)) +
...
transportation_city |>
  ...
  ggplot(aes(share_transit_trips_2019, share_transit_trips_2022, size = population_2022)) +
  geom_point(alpha = 0.1) +
  facet_wrap(~ size) +
  coord_equal() +
  labs(subtitle = "Large counties have at least 400,000 people")
...
transportation_city |>
  ...
  mutate(change = share_transit_trips - lag(share_transit_trips)) |>
  ungroup() |>
  filter(year %in% c(2019, 2022)) |>
...
transportation_city <- transportation_city |>
  mutate(
    share_transit_trips_quality = pmax(cv_quality, income_quality, match_quality),
    share_transportation_cost_quality = pmax(cv_quality, income_quality, match_quality)
  ) |>
  mutate(
    share_transit_trips_quality = 
      ifelse(is.na(share_transit_trips), NA, share_transit_trips_quality),
...
```{r}
transportation_city |>
  count(year, share_transit_trips_quality)
...
final_data <- transportation_city |>
  select(
    year, state, place, share_transit_trips, share_transit_trips_quality,
