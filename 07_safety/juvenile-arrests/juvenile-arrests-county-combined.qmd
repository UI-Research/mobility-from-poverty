...
5. Move the files for 2021-2024 to the `mobility-from-poverty/07_safety/data/arrestee-segment` directory.
...
5. Move the files for 2021-2024 to the `mobility-from-poverty/07_safety/data/group-b-arrest-report-segment` directory.
...
For the 2026 metric update, we used the ICPSR version (`batch_header_rds_1991_2024.zip` from V11) which includes 2024. Download and unzip, then move `nibrs_batch_header_1991_2024.rds` to the `mobility-from-poverty/07_safety/data/batch-header-segment-and-agencies` directory.

## 4.2 Load and clean NIBRS Batch Header Files
Load Batch Header File
```{r load-bhf, message=FALSE}
batch_header_file_full <- readRDS(here::here("07_safety", "data", "batch-header-segment-and-agencies", "nibrs_batch_header_1991_2024.rds")) |>
...
Filter the file to the appropriate year, select relevant necessary, and rename as necessary.
```{r clean-bhf}
...
### 4.4e Join arrest counts onto the weight file and Multiply arrest variables by population weight
