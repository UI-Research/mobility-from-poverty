...
This file creates the county-level metric for juvenile arrests per 100,000 juveniles (age 10-17) for years 2021-2023.
...
1. Background
2. Manually download NIBRS arrest data from Harvard Dataverse
...
# 0. Changes from previous round of updates.
...
The data for this metric come from the FBI's [National Incident-Based Reporting System (NIBRS)](https://bjs.ojp.gov/national-incident-based-reporting-system-nibrs). This is the national standard for law enforcement crime data reporting in the United States. Processing the raw NIBRS data directly from the FBI website would take a lot of work.

Instead of downloading the raw data directly, we use criminologist Jacob Kaplan's pre-processed data that he has made publicly available on [Harvard Dataverse](https://dataverse.harvard.edu/dataverse/ucrdata).

The NIBRS data are separated into segments. For this metric, we use the following two segments:

* [Arrestee Segment](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/ZULQVE)
* [Group B Arrest Report Segment](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/CIQIVF)
...
To learn more about NIBRS data, and the segments related to the creation of this metric specifically, consult Jacob Kaplan's book titled [Uniform Crime Reporting (UCR) Program Data: An Opinionated Guide to FBI Data](https://ucrbook.com/arrestee.html). For these metrics, the most important thing to know about these data is that most police agencies do not report NIBRS data prior to 2021. In 2019, only 8,500 out of approximately 18,000 police agencies in the United States (covering about 45% of the US population) reported NIBRS data. Instead, agencies primarily reported to the [Summary Reporting System (SRS)](https://ucrbook.com/index.html#goal-of-the-book). However, in 2020, the FBI stopped collecting data from the SRS and only allowed submissions from NIBRS. 
...
## 2.2 Group B Arrest Report Segment
...
## 3.1 Read and clean files for 2021-2023 from the Arrestee Segment
```{r}
...
## 3.2 Read and clean files for 2021-2023 from the Group B Arrest Report Segment
```{r}
...
## 3.3 Append Group A and Group B files for all arrests
```{r}
# Append files
...
## 3.4 Create juvenile arrests file
...
arrests_agency <- arrests %>%
  ...
  summarize(
    ...
    ethn_hispanic = sum(ethnicity_of_arrestee == "hispanic or latino"),
    ethn_nonhisp = sum(ethnicity_of_arrestee == "not hispanic or latino"),
    # Sex subgroups
    sex_male = sum(sex_of_arrestee == "male"),
    sex_female = sum(sex_of_arrestee == "female"),
    # Age subgroups
    age_10_14 = sum(age_of_arrestee >= 10 & age_of_arrestee <= 14),
    age_15_17 = sum(age_of_arrestee >= 15 & age_of_arrestee <= 17),
...
```{r}
arrests_agency_clean <- arrests_agency |>
  select(
    year,
    ori, 
    arr_total,
...
arrests_all_county <- arrests_all_agency_county %>%
  group_by(year, state_fips, fips_county_code) %>%
  summarize(
    n = n(), 
...
arrests_county_demo <- arrests_county_demo %>%
...
## Old way of reading in new data before it was updated on 2025-02-08 to be single-year files
```{r}
nibrs_arrestee <- readRDS(here::here("07_safety", "data", "nibrs_arrestee_segment_1991_2023.rds"))
```


```{r}
nibrs_arrestee_2023 <- nibrs_arrestee |>
  filter(year==2023)

nibrs_arrestee_2022 <- nibrs_arrestee |>
  filter(year==2022)

nibrs_arrestee_2021 <- nibrs_arrestee |>
  filter(year==2021)

arrow::write_parquet(
  x = nibrs_arrestee_2023,
  sink = here::here("07_safety", "data", "nibrs_arrestee_2023.parquet")
)

arrow::write_parquet(
  x = nibrs_arrestee_2022,
  sink = here::here("07_safety", "data", "nibrs_arrestee_2022.parquet")
)

arrow::write_parquet(
  x = nibrs_arrestee_2021,
  sink = here::here("07_safety", "data", "nibrs_arrestee_2021.parquet")
)
```
...
```{r}
nibrs_groupb <- readRDS(here::here("07_safety", "data", "nibrs_group_b_arrest_report_segment_1991_2023.rds"))

nibrs_groupb_2023 <- nibrs_groupb |>
  filter(year==2023)

nibrs_groupb_2022 <- nibrs_groupb |>
  filter(year==2022)

nibrs_groupb_2021 <- nibrs_groupb |>
  filter(year==2021)

arrow::write_parquet(
  x = nibrs_groupb_2023,
  sink = here::here("07_safety", "data", "nibrs_groupb_2023.parquet")
)

arrow::write_parquet(
  x = nibrs_groupb_2022,
  sink = here::here("07_safety", "data", "nibrs_groupb_2022.parquet")
)

arrow::write_parquet(
  x = nibrs_groupb_2021,
  sink = here::here("07_safety", "data", "nibrs_groupb_2021.parquet")
)
```

## Classifying crime types (violent and property) - used in crime metric but not arrest metric
...
## Recoding race and ethnicity in the Group B file
```{r}
