...
There are a lot of unmatched observations. 23,948 observations are only in our universe of agencies with county-agency weights. My first thought is that since this is the universe of *all* agencies, some agencies may not have reported arrests and are therefore not in the arrests data. 1,127 observations are only in the 2021-2022 agency-level arrests data. My first thought is that since we only used the Batch Header File from 2022 to construct the universe of agencies and agency-county weights, it may be missing agencies from other years. 
...
To test this second hypothesis, I repeat the join for 2022 only. There is a proportional number of unmatched observations from each data set for 2022 as compared to 2021-2022. So, I think this is okay.
    todo(): Reviewer, does this logic make sense to you? I don't know what else I can do here to explain the unmatched observations. Many agencies did not report NIBRS data prior to 2021, so we expect a lot of missings.
```{r}
test_2022 <- left_join(
...
There are a lot of unmatched observations. 17,397 observations are only in our universe of agencies with county-agency weights. My first thought is that since this is the universe of *all* agencies, some agencies may not have reported arrests and are therefore not in the arrests data. 1,985 observations are only in the 2021-2022 agency-level arrests data. My first thought is that since we only used the Batch Header File from 2022 to construct the universe of agencies and agency-county weights, it may be missing agencies from other years. 
...
# Note that future updates could consider renaming these variables when pulled form the ACS in `01_agency_geo_county.Rmd` but I didn't have the foresight so I'm just renaming them now
