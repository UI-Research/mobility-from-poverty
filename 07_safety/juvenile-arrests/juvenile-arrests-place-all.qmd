...
Join total number of arrests for each agency onto the place/agency file. Note that these files are not joined by year, as there is not year in `bhf_place_urb`. 
```{r}
...
All places (in our universe) had 100 percent of agencies reporting.
```{r}
arrest_rates_place %>%
  group_by(year) %>%
...
Checking arrest rates for Black juveniles. The maximum for all years is 101,562.5. Rates are of arrests, not persons arrested, so this value is plausible and indicates some people were arrested more than once.
```{r}
quantile(
