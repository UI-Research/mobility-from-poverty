...
## 3. Read in and clean NIBRS arrest data
This section extracts the necessary files from each zip file downloaded in the previous step and reads the files into the R environment. Each zip file contains data files for 1991-2022. However, we only want 2021-2022. Note that future updates of this metric may want to include historical NIBRS data back to 2016 and should update this section to include all years.
...
#### 3.1c Append the 2021-2022 Group A file and the 2016-2022 Group B file.
...
arrests_all <- read.csv(here::here("07_safety", "juvenile-arrests", "data", "arrests_all_21_22.csv"))
...
Roughly half of observations are not in either category of arrests. These are all crimes against society or Group B offenses.
```{r}
arrests %>%
...
arrests_place <- arrests_agency_place %>%
  ...
  summarize(across(arr_total:race_asian_other, ~sum(.x, na.rm=TRUE))) %>%
...
stopifnot(
  near(rowSums(select(arrests_place, sex_female, sex_male)), arrests_place$arr_total, tol = 0.01)
)
...
stopifnot(
  near(rowSums(select(arrests_place, age_10_14, age_15_17)), arrests_place$arr_total, tol = 0.01)
)
...
stopifnot(
  near(rowSums(select(arrests_place, matches("race_asian_other|race_black|race_white|race_unkno"))), arrests_place$arr_total, tol = 0.01)
)
...
# Note that future updates could consider renaming these variables when pulled form the ACS in `01_agency_geo_place.Rmd` but I didn't have the foresight so I'm just renaming them now
...
arrests_place_demo <- arrests_place_demo %>%
...
arrest_rates_place <- arrests_place_demo %>%
  mutate(
    # Total ages 10-17
    rate_juv_arrest = if_else(
      n_reporting != 0, arr_total / age_1017 * 100000, NA
    ),
    # Race/ethnicity subgroups
    rate_juv_arrest_white = if_else(
      n_reporting != 0, race_white / age_1017_white * 100000, NA
    ),
    rate_juv_arrest_black = if_else(
      n_reporting != 0, race_black / age_1017_black * 100000, NA
    ),
    rate_juv_arrest_hispanic  = if_else(
      n_reporting != 0, ethn_hispanic / age_1017_hispanic * 100000, NA
    ),
    rate_juv_arrest_asian_other = if_else(
      n_reporting != 0, race_asian_other / age_1017_asian_other * 100000, NA
    ),
    # Sex subgroups
    rate_juv_arrest_male = if_else(
      n_reporting != 0, sex_male / age_1017_male * 100000, NA
    ),
    rate_juv_arrest_female = if_else(
      n_reporting != 0, sex_female / age_1017_female * 100000, NA
    ),
    # Age subgroups
    rate_juv_arrest_1014 = if_else(
      n_reporting != 0, age_10_14 / age_1017_1014 * 100000, NA
    ),
    rate_juv_arrest_1517 = if_else(
      n_reporting != 0, age_15_17 / age_1017_1517 * 100000, NA
    )
  )
...
```{r}
quantile(
  arrest_rates_place$rate_juv_arrest,
...
```{r}
quantile(
  arrest_rates_place$rate_juv_arrest_white,
  probs = c(0, 0.2, 0.4, 0.6, 0.8, 0.9, 0.99, 1),
  na.rm = TRUE
)
```

Checking arrest rates for Black juveniles. The maximum for all years is 60,095.6. Rates are of arrests, not persons arrested, so this value is plausible and indicates some people were arrested more than once.
```{r}
quantile(
  arrest_rates_place$rate_juv_arrest_black,
  probs = c(0, 0.2, 0.4, 0.6, 0.8, 0.9, 0.99, 1),
  na.rm = TRUE
)
```
...
```{r}
quantile(
  arrest_rates_place$rate_juv_arrest_hispanic,
  probs = c(0, 0.2, 0.4, 0.6, 0.8, 0.9, 0.99, 1),
  na.rm = TRUE
)
```
...
```{r}
quantile(
  arrest_rates_place$rate_juv_arrest_asian_other,
  probs = c(0, 0.2, 0.4, 0.6, 0.8, 0.9, 0.99, 1),
  na.rm = TRUE
)
```
...
```{r}
quantile(
  arrest_rates_place$rate_juv_arrest_male,
  probs = c(0, 0.2, 0.4, 0.6, 0.8, 0.9, 0.99, 1),
  na.rm = TRUE
)
```
...
```{r}
quantile(
  arrest_rates_place$rate_juv_arrest_female,
  probs = c(0, 0.2, 0.4, 0.6, 0.8, 0.9, 0.99, 1),
  na.rm = TRUE
)
```
...
```{r}
quantile(
  arrest_rates_place$rate_juv_arrest_1014,
  probs = c(0, 0.2, 0.4, 0.6, 0.8, 0.9, 0.99, 1),
  na.rm = TRUE
)
```
...
```{r}
quantile(
  arrest_rates_place$rate_juv_arrest_1517,
  probs = c(0, 0.2, 0.4, 0.6, 0.8, 0.9, 0.99, 1),
  na.rm = TRUE
)
```
