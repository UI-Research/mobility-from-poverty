...
The Law Enforcement Agency Identifiers Crosswalk (LEAIC) facilitates linking reported crime data with socio-economic data. Note that 2012 is the most recent year for which this crosswalk is available. The LEAIC file is available for download from the National Archive of Criminal Justice Data (NACJD) available through ICPSR at the following link:
...
# Note that there are only 485 places in 2016 and 2017
places %>%
  group_by(year) %>%
  count()
...
```{r}
# Check ACS variables for 2016 and 2022 to see if they change over time
variables2016 <- tidycensus::load_variables(2016, "acs5")
...
### 4.3 Pull 2016-2022 5-year ACS data at the place level 
...
rm(variables2016, variables2022, years, my_vars)
...
Limit ACS data to relevant variables and to places in the Urban universe of 486 places
```{r}
...
```{r}
# This file has 485 places by 2 years and 486 places by 5 years = 3,400 observations
...
470 places have any agency, and the maximum number of agencies in a place is 37.
```{r}
...
```{r}
# This file has 485 places by 2 years and 486 places by 5 years = 3,400 observations. Only 470/486 places had any agency, so 110 observations (accounting for all years) are only in the demographic file
...
```
  ...
  todo(): state is missing for some observations but the full state/place GEOID is not
```{r}
place_demo_agency %>%
