...
# 3 Merge geography----
# start with full universe of agencies in Urban places, merge on crime
# indicate agencies that are reporting
of_agency_geo <- ba_pl_ur %>%
  left_join(of_agency, by = "ori") %>%
  mutate(reporting = ifelse(is.na(all), 0, 1))
...
skim(of_agency_place)

# 5 Calculate rates----
# Merge demographic file to get population denominator
pl_county_demo <- place_demos %>%
  select(c(GEOID, total_people)) %>%
  left_join(of_agency_place, by = "GEOID")

skim(pl_county_demo)

# Calculate crime rates
of_by_place <- pl_county_demo %>%
...
# note no places in 2021 have population less than 30
of_by_place <- of_by_place %>%
...
of_by_place %>%
  ...
  skim()
# missing for 16 places with no law enforcement agencies
...
of_by_place <- of_by_place %>%
  mutate(
    ...
    crime_rate_quality = case_when(
      ...
      agencies_reporting > 0 ~ 3,
      agencies_reporting == 0 ~ NA_real_),
    all_crime_rate_quality = ifelse(total_people < 30, NA, all_crime_rate_quality),
    crime_rate_quality = ifelse(total_people < 30, NA, crime_rate_quality)
  )

# check distribution based on definition
of_by_place %>%
  count(all_crime_rate_quality, crime_rate_quality)
# very similar - core reporting moves 67 places from 3 to 2 

# 8 Save data----
of_by_place_2021 <- of_by_place %>%
  mutate(year=2021,
  ...
  select(c(year, state, place, starts_with("crime")))

skim(of_by_place_2021)

write_csv(of_by_place_2021, file = "07_safety/modified data/2021_crime_rate_place.csv")