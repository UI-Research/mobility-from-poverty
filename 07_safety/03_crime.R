...
# universe of agencies linked to counties with weight for each county
...
# 3 Merge geography----
# start with full universe of agencies, merge on juvenile arrests
# multiply stats by agency-county weight
ar_juv_agency_geo <- ba_long_wt %>%
...
ar_juv_county <- ar_juv_agency_geo %>%
...
# merge geography
# indicate reporting by county-agency, weight total arrest by county weight
ar_any_agency_geo <- ba_long_wt %>%
...
ar_any_county <- ar_any_agency_geo %>%
  group_by(state, county) %>%
  summarize(n = n(), 
...
ar_juv_county <- ar_juv_county %>%
  left_join(ar_any_county, by = c("GEOID", "state", "county")) 
...
ar_juv_county_demo <- county_demos %>%
  left_join(ar_juv_county, by = "GEOID")
...
# with core reporting, 215 counties move from 3 to 2

# 8 Save data----
juv_arrest_by_county_2021 <- juv_arrest_by_county %>%
  mutate(year=2021,
  ...
  select(c(year, state, county, starts_with("juv_arrest"), age_1017, age_1017_white,
           age_1017_black, age_1017_asian_other, age_1017_hispanic,
           agencies_reporting, core_reporting))

skim(juv_arrest_by_county_2021)

# file with only total juvenile arrest rates
juv_arrest_county_2021 <- juv_arrest_by_county_2021 %>%
  select(year, state, county, age_1017, juv_arrest_rate, juv_arrest_rate_violent,
         juv_arrest_rate_property, juv_arrest_rate_quality,
         agencies_reporting, core_reporting)

skim(juv_arrest_county_2021)
# note 7 counties have 0 agencies, so NA for all metrics
write_csv(juv_arrest_county_2021, file = "07_safety/modified data/2021_juv_arrest_rate_county.csv")
...
juv_pop_2021_subgroup <- juv_arrest_by_county_2021 %>%
  mutate(age_1017_all = age_1017) %>%
  select(year, state, county, age_1017_all, age_1017_white, age_1017_black, age_1017_asian_other, 
         age_1017_hispanic, 
         juv_arrest_rate_quality, agencies_reporting, core_reporting) %>%
...
juv_arrest_2021_subgroup <- juv_arrest_by_county_2021 %>%
  mutate(juv_arrest_rate_all = juv_arrest_rate) %>%
  select(year, state, county, juv_arrest_rate_all, juv_arrest_rate_white, juv_arrest_rate_black,
...
juv_arrest_county_2021_subgroup <- juv_pop_2021_subgroup %>%
  left_join(juv_arrest_2021_subgroup, by = c("year", "state", "county", "subgroup")) %>%
  mutate(subgroup_type = "race-ethnicity",
         juv_arrest_rate_quality = ifelse(age_1017 < 30, NA, juv_arrest_rate_quality))

skim(juv_arrest_county_2021_subgroup)
juv_arrest_county_2021_subgroup %>% count(juv_arrest_rate_quality, is.na(juv_arrest_rate))


write_csv(juv_arrest_county_2021_subgroup, file = "07_safety/modified data/2021_juv_arrest_rate_county_subgroup.csv")