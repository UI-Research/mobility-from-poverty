...
This metric represents the place-level rate of reported violent crimes per 100,000 people and reported property crimes per 100,000 people for years 2016-2022.
...
## 3. Read in and clean NIBRS offense data
This section extracts the necessary files from the zip file downloaded in the previous step and reads the files into the R environment. The zip file contains data files for 1991-2022. However, we only want 2016-2022.

### 3.1 Extract files for 2016-2022 from the zip file
Extract each file for the years 2016-2022 from the `offense` segment.
```{r}
...
# Extract years 2016-2022 for the offense segment
...
Note that the offense segment files are very large. They were too large to read in and append in one step (at least with the code that I was using). Instead, I create a function that reads in the offense segment for each year and cleans it. This makes each file smaller and more manageable, and we are then able to append those smaller files for each year.
...
```{r}
offenses <- bind_rows(
  offenses_2016, offenses_2017, offenses_2018, offenses_2019, 
  offenses_2020, offenses_2021, offenses_2022
)
...
write_csv(offenses, file = here::here("07_safety", "reported-crime", "data", "offenses_16_22.csv"))

# Read agency-level offense data back in for ease of future use
offenses_agency <- read.csv(here::here("07_safety", "reported-crime", "data", "offenses_16_22.csv"))
...
### 4.3 Summarize by place
    todo(): In the 2022 update, we want there to be a quality variable assigned to each of the two metric values, not one overall (previously the variable was `rate_crime_quality` for both). I think this is the first code chunk in the program where that change would start to be implemented.
...
# The following code is my first attempt at creating a quality metric for both rates instead of one overall quality metric 

# offenses_county <- offenses_agency_geo %>%
...
#     n_core_city = sum(core_city),
#     n_core_city_rpt = sum(core_city==1 & reporting_all==1),
#     n_core_city_rpt_violent = sum(core_city==1 & reporting_violent==1),
#     n_core_city_rpt_property = sum(core_city==1 & reporting_property==1),
#     across(all:property, ~sum(.x, na.rm=TRUE))
#   )
#   mutate(
...
rates_place <- offense_place_demo %>%
  mutate(
    rate_crime_violent = violent / total_people * 100000,
    rate_crime_property = property / total_people * 100000
  )
...
## 7. Construct quality indicators
    todo(): In the 2022 update, we want there to be a quality variable assigned to each of the two metric values, not one overall (previously the variable was `rate_crime_quality` for both).
...
rates_place <- rates_place %>%
  mutate(
    rate_crime_quality = case_when(
      agencies_reporting_all == 1 ~ 1,
      agencies_reporting_all >= 0.8 ~ 2,
      agencies_reporting_all > 0 ~ 3,
      agencies_reporting_all == 0 ~ NA_real_),
    rate_crime_core_quality = case_when(
      agencies_reporting_all == 1 ~ 1,
      agencies_reporting_all >= 0.8 | core_reporting==1 ~ 2,
      agencies_reporting_all > 0 ~ 3,
      agencies_reporting_all == 0 ~ NA_real_),
    rate_crime_quality = ifelse(total_people < 30, NA, rate_crime_quality),
    rate_crime_core_quality = ifelse(total_people < 30, NA, rate_crime_core_quality)
  )

```

With core reporting, XX places move from a quality indicator of 3 to a quality indicator of 2.
```{r}
# Check distribution of quality for all agencies and for core reporting agencies
rates_place %>%
  count(rate_crime_quality, rate_crime_core_quality)

# Remove the all quality metric and rename core reporting quality metric.
rates_place <- rates_place %>%
  select(-rate_crime_quality) %>%
  rename(rate_crime_quality = rate_crime_core_quality)

```


## 8. Save and write out data
This section saves and write out the final data file. This file is long by place and year (2016-2022). It contains our place-level estimates of reported violent crimes per 100,000 people and reported property crimes per 100,000 people for each year.

Construct the final file.
```{r}
rates_crime_place_all <- rates_place %>%
  mutate(
    across(c(rate_crime_violent, rate_crime_property),
           ~ifelse(is.na(rate_crime_quality), NA, .x)),
...
write_csv(
  ...
  file = "07_safety/modified data/rates_crime_place_all.csv"
)
