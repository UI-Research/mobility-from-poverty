...
This metric represents the county-level rate of reported violent crimes per 100,000 people and reported property crimes per 100,000 people for years 2014-2022.
...
## 3. Read in and clean NIBRS offense data
This section extracts the necessary files from the zip file downloaded in the previous step and reads the files into the R environment. The zip file contains data files for 1991-2022. However, we only want 2014-2022.

### 3.1 Extract files for 2014-2022 from the zip file
Extract each file for the years 2014-2022 from the `offense` segment.
```{r}
...
# Extract years 2014-2022 for the offense segment
...
write_csv(offenses, file = here::here("07_safety", "reported-crime", "data", "offenses_14_22.csv"))

# Read agency-level offense data back in for ease of future use
offenses_agency <- read.csv(here::here("07_safety", "reported-crime", "data", "offenses_14_22.csv"))
...
## 8. Save and write out data
This section saves and write out the final data file. This file is long by county and year (2014-2022). It contains our county-level estimates of reported violent crimes per 100,000 people and reported property crimes per 100,000 people for each year.
...
write_csv(
  ...
  file = "07_safety/modified data/rates_crime_county_all.csv"
)
