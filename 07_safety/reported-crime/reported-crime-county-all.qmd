...
All data used in this file are available on Box [here](https://urbanorg.box.com/s/poqnegsa6i74phcdr1eap7tacpe4x1gf).
...
Suppress rates for counties that have population less than 30 people.
    Note that no counties have a population of less than 30, but some counties have small populations with high crime rates (e.g., 21,000 crimes per 100,000 people with only 83 people in the county). Future updates should consider changing the population threshold for suppression.
```{r}
...
Loving County, TX has a population between 30 and 100 in both years with really high property crime rates and relatively high violent crime rates.
```{r}
rates_county |>
  filter(total_people < 100) |>
  count(GEOID, total_people, rate_crime_property)
```
```{r}
rates_county |>
  filter(total_people < 100) |>
  count(GEOID, total_people, rate_crime_violent)
...
There is one large outlier in Loving County, TX, which has a very small population.
```{r}
rates_county |>
  filter(rate_crime_property > 20000)
```
...
There are two large outliers in Buffalo County, SD, and Todd County, SD.
```{r}
rates_county |>
...
write_csv(
  ...
  file = here::here("07_safety", "modified data", "rates_crime_county_all.csv")
)
