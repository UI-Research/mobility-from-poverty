...
+ Previous versions of the metric relied on two files: One that created population weights for each agency in order to more accurately aggregate agency-level crimes to the county-level, and one that actually calculated the metric. I've integrated the two programs in this round of updates to be more straightforward and intuitive.
+ The storage of the NIBRS data we use has changed, so the instructions for downloading and reading in the data have changed.
...
This section uses the Batch Header Segment of the NIBRS. The Batch Header Segment includes metadata about each participating police agency. Each individual police agency (identied by originating agency identifier, or ORI) that has submitted any information to NIBRS for that year will appear in this segment only once. Police agencies that have not submitted their data to NIBRS do not appear in this segment, so it is ingerently incomplete. It also links each agency to all counties associated with that agency. We use this file to create population weights for each county and to link/aggregate offenses from the agency-level to the county-level.
...
### 4.4e Join arrests data onto the weight file and Multiply arrest variables by population weight
