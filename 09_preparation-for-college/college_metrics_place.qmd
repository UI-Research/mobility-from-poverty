...
source(here::here("09_preparation-for-college", "R", "finalize_metric_college.R"))
source(here::here("09_preparation-for-college", "R", "calc_survey_mean_college.R"))
source(here::here("09_preparation-for-college", "R", "calc_survey_mean_subgroup_college.R"))
```
...
```{r}
repwts_person <- ipums_repwt_college_aws(
...
if(isTRUE(bucket_exists(bucket = my_bucket))){
 if (!aws.s3::object_exists(paste0(s3_dir, "/metrics_college_all", "_", params$year[1], ".csv"), bucket = my_bucket)){
    ...
    put_object(tmp, object = paste0(s3_dir, "/metrics_college_all", "_", params$year, ".csv"), bucket = my_bucket, 
...
stopifnot(nrow(results_race_ethnicity_expanded) == 486 * n_distinct(results_all_expand$year) * 4)
```
...
stopifnot(nrow(results_gender_expanded) == 486 * n_distinct(results_all_expand$year) * 2)
```
...
stopifnot(nrow(results_disability_expanded) == 486 * n_distinct(results_all_expand$year) * 2)
```
...
```{r}
 if(isTRUE(bucket_exists(bucket = my_bucket))){
   if (!aws.s3::object_exists(paste0(s3_dir, "/metrics_college_sub", "_", params$year, ".csv"), bucket = my_bucket)){
    ...
    # put object with an upload progress bar
    put_object(tmp, object = paste0(s3_dir, "/metrics_college_sub", "_", params$year, ".csv"), bucket = my_bucket, 
               show_progress = TRUE, multipart = FALSE)
...
read_intermediate <- function(year, file_type){
  ...
  read_csv(paste0(here::here("09_preparation-for-college", "data", "intermediate", "college"), 
                  "/", "metrics_college_", file_type, "_", year, ".csv"))
  }
...
check <- get_bucket_df(bucket = my_bucket)
...
metrics_education_all %>% 
  ...
  theme_minimal() +
  ggtitle(paste0("Distribution of share college ready, by place,", params$year, "(5-year data)")) +
  ylab("Density")
...
metrics_education_sub %>% 
  ...
  geom_density(alpha = 0.15) +
  theme_minimal() +
  ggtitle(paste0("Distribution of share college ready, subgroups, by place", params$year, "(5-year data)")) +
  ylab("Density")
...
metrics_education_sub %>% 
  ...
  facet_wrap(~subgroup) +
  labs(
    title = paste0("Distribution of share college ready, race/ethnicity, by place", params$year, "(5-year data)"),
    y = "Density"
  ) +
  theme_minimal()
```
...
metrics_education_sub %>% 
  ...
  facet_wrap(~subgroup) +
  labs(
    title = paste0("Distribution of share college ready, gender, by place", params$year, "(5-year data)"),
    y = "Density"
  ) +
  theme_minimal()
```
...
metrics_education_sub %>% 
  ...
  facet_wrap(~subgroup) +
  labs(
    title = paste0("Distribution of share college ready, disability, by place", params$year, "(5-year data)"),
    y = "Density"
  ) +
  theme_minimal()
```
...
evaluate_final_data(data = metrics_education_all_clean, exp_form_path = here::here("99a_final-evaluation", "evaluation_form_college_overall_place.csv"), geography = "place", subgroups = FALSE, confidence_intervals = TRUE)
...
evaluate_final_data(data = metrics_education_race, exp_form_path = here::here("99a_final-evaluation", "evaluation_form_college_race_eth_county.csv"), geography = "place", subgroups = TRUE, confidence_intervals = TRUE)
```
...
evaluate_final_data(data = metrics_education_gender, exp_form_path = here::here("99a_final-evaluation", "evaluation_form_college_gender_county.csv"), geography = "place", subgroups = TRUE, confidence_intervals = TRUE)
```
...
evaluate_final_data(data = metrics_education_disability, exp_form_path = here::here("99a_final-evaluation", "evaluation_form_college_disability_county.csv"), geography = "place", subgroups = TRUE, confidence_intervals = TRUE)
```
...
```{r}
write_csv(metrics_education_all_clean, here::here("09_preparation-for-college", "data", "final", "metrics_college_place_all_longitudinal.csv"))

write_csv(metrics_education_race, here::here("09_preparation-for-college", "data", "final", "metrics_college_place_race-ethnicity_longitudinal.csv"))

write_csv(metrics_education_gender, here::here("09_preparation-for-college", "data", "final", "metrics_college_place_gender_longitudinal.csv"))


write_csv(metrics_education_disability, here::here("09_preparation-for-college", "data", "final", "metrics_college_place_disability_longitudinal.csv"))
```
