...
*This program reads in the datasets created by access_to_preschool_5_year_multiple.
*That program needs to be run before this one can be run. It then removes people in group quarters and begins to calculate the college metrics
*=================================================================;
...
%compute_metrics_college(x.main_2019,edu.metrics_college_5_2019);
%compute_metrics_college(x.main_2018,edu.metrics_college_5_2018);
%compute_metrics_college(x.main_2017,edu.metrics_college_5_2017);
%compute_metrics_college(x.main_2016,edu.metrics_college_5_2016);
%compute_metrics_college(x.main_2015_all,edu.metrics_college_5_2015);
proc means data=edu.metrics_college_subgroup;
run;

proc freq data=x.raw_5_2016;
 where statefip = 27 and PUMA in (1301,1302,1303,1304) and age in (19,20);
 table hispan*educd / nocol norow nopercent;
 weight perwt;
run;
