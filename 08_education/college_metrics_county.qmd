...
Read in the ACS extracts using the extract_ipums function. Make sure to change the survey list to reflect what years you want to include in the data. Currently we are including 4 years of 5-year data (2014, 2016, 2018 and 2021c). 5-year 2023 data will be added pending its release.
...
```{r}
acs <- extract_ipums(
  extract_name = "umf_data_18_21_5yr",
  extract_description = "Microdata pull for College Readiness Metric Predictors. American Community Survey, years 2014 through 2021 (5-year).",
  survey = list("us2014c", "us2016c", "us2018c", "us2021c"))
```
...
```{r}
repwts <- ipums_repwt_college(
  extract_name = "college_replicate_weights",
  extract_description = "Person replicate weights for College Readiness Metric Predictors. American Community Survey, years 2014 through 2021 (5-year).",
  survey = list("us2014c", "us2016c", "us2018c", "us2021c")
)  %>% 
...
stopifnot(nrow(results_all_expand) == 3143 * 2)
```
...
stopifnot(nrow(results_race_ethnicity_expanded) == 3143 * 2 * 4)
```
...
stopifnot(nrow(results_gender_expanded) == 3143 * 2 * 2)
```
...
stopifnot(nrow(results_disability_expanded) == 3143 * 2 * 2)
```
...
metrics_education_all %>% 
  ...
  theme_minimal() +
  ggtitle("Distribution of share college ready, by county 2021 (5-year data)") +
  ylab("Density")
...
metrics_education_sub %>% 
  ...
  geom_density(alpha = 0.15) +
  theme_minimal() +
  ggtitle("Distribution of share college ready, subgroups, by county 2021 (5-year data)") +
  ylab("Density")
...
metrics_education_sub %>% 
  ...
  facet_wrap(~subgroup) +
  labs(
    title = "Distribution of share college ready, race/ethnicity, by county 2021 (5-year data)",
    y = "Density"
  ) +
  theme_minimal()
```
...
metrics_education_sub %>% 
  ...
  facet_wrap(~subgroup) +
  labs(
    title = "Distribution of share college ready, gender, by county 2021 (5-year data)",
    y = "Density"
  ) +
  theme_minimal()
```
...
metrics_education_sub %>% 
  ...
  facet_wrap(~subgroup) +
  labs(
    title = "Distribution of share college ready, disability, by county 2021 (5-year data)",
    y = "Density"
  ) +
  theme_minimal()
```
...
metrics_education_all %>%
  ...
  ggplot(aes(`2014`, `2016`, `2018`, `2021`)) +
  geom_abline() +
