...
metrics_education_all <- left_join(results_all_expand, place_puma, by=c("crosswalk_period", "statefip","place"))
```
...
metrics_education_sub <- left_join(metrics_income_sub, place_puma, by=c("crosswalk_period", "statefip","place"))
```
...
Look at distribution of prek_attendance share by place.
```{r}
...
metrics_education_all %>% 
  ...
  theme_minimal() +
  ggtitle("Distribution of share prek_attendance, by place 2022 (5-year data)") +
  ylab("Density")
...
metrics_education_sub %>% 
  ...
  geom_density(alpha = 0.15) +
  theme_minimal() +
  ggtitle("Distribution of share prek_attendance, subgroups, by place 2022 (5-year data)") +
  ylab("Density")
  
```

Look at distribution of prek_attendance share by place by race.
```{r}

metrics_education_sub %>% 
  filter(year == 2021, subgroup == "Black, Non-Hispanic") %>% 
  select(place, share_in_preschool) %>% 
  ggplot(aes(x = share_in_preschool)) +
  geom_density(alpha = 0.15) +
  theme_minimal() +
...
```{r}
write_csv(metrics_education_all, here::here("08_education", "data", "final", "metrics_preschool_place_all_longitudinal.csv"))
