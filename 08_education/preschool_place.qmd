...
GRADEATT: N/As reported as "0" in all modern surveys. 
...
Confirm the missing values from the value labels of the GRADEATT variable to ensure we are eliminating the correct values. "0" is N/A or missing and should be turned into NA values that R recognizes. 
```{r}
head(ipums_val_labels(acs$gradeatt), 50) %>% 
  reactable()
```

Turn the missing/"not in universe" value from the gradeatt variable into NAs and remove the labels for the rest of the analysis.
```{r}
acs_clean <- acs %>%
  mutate(gradeatt = zap_labels(gradeatt), 
         gradeatt = ifelse(gradeatt %in% c(1, 0, 999), NA_integer_,
                          gradeatt)
  )
```

Look at the distribution of gradeatt after adjustment for NAs. We can see that there are no longer records for "1" but we now see "NA".
```{r}
acs_clean %>% 
  count(gradeatt) %>% 
  ggplot(mapping = aes(x = factor(gradeatt), y = n)) +
...
```{r}
acs_clean <- acs_clean %>% 
  mutate(college_ready = ifelse(gradeatt <= 116 & gradeatt >= 62, 1, 0)) 
```
