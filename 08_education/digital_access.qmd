...
We are using the 1-yr ACS for the communities overall (not the subgroups), from
2016 to 2022. This will be based on table B28003.
...
acs1_years <- c(2016, 2017, 2018, 2019, 2021, 2022)
...
Black non-Hispanic, Hispanic, White non-Hispanic, and All (total population in
households)
...
aggregate_income_subgroup <- function(data) {
  data |>
    ...
    select(year, geoid, name, income_less_than_50k, moe_income_less_than_50k, income_more_than_50k, moe_income_more_than_50k, income_less_than_50k_digital, moe_income_less_than_50k_digital, income_more_than_50k_digital, moe_income_more_than_50k_digital)
}
...
## Calculate Coefficients of Variation and Confidence Intervals
...
1.  Convert the MOE into a standard error by dividing by 1.96 because we pulled
    census using a 95% confidence interval
...
calculate_cov <- function(data, zscore = 1.96) {
  data |>
    # cur_column() dynamically references current column, simplifying pairwise iteration in across().
    mutate(across(
      .cols = ends_with("digital") & !starts_with("moe"),
      .fns = ~ (get(paste0("moe_", cur_column())) / zscore) / .x,
      .names = "cv_{.col}"
    ))
}
...
We can use the width of the MOE to determine quality. One suggestion is if the
value minus the lower bound is \<0.1, then the quality is strong (\_quality=1);
if it is =\>.1 and \<.2, the quality is marginal (\_quality=2), and if it is
\>=.2, then the quality is weak (\_quality=3).

Otherwise, if any of the census table values are suppressed, those should have
the metric value show as "NA" and the data quality should also be "NA."
...
get_summary_stats <- function(data) {
  data |>
    summarise(across(c(share_digital_access, share_digital_access_lb, share_digital_access_ub),
      .fns = list(
...
county_overall_digital_access |> get_summary_stats()

place_overall_digital_access |> get_summary_stats()

county_raceeth_digital_access |> get_summary_stats()

place_raceeth_digital_access |> get_summary_stats()

county_income_digital_access |> get_summary_stats()

place_income_digital_access |> get_summary_stats()
```
...
export_metric <- function(data, type = c("recent", "subgroup", "multi-year")) {
  ...
  if (type == "recent") {
    data |>
      ...
      select(-year) |>
      write_csv(
        here::here("08_education", "data", "final", glue("digital_access_{geo}_{max(data$year)}.csv"))
      )
  } else if (type == "multi-year") {
    write_csv(
      data,
      here::here("08_education", "data", "final", glue("digital_access_{geo}_all.csv"))
    )
  } else if (type == "subgroup") {
    write_csv(
      data,
      here::here("08_education", "data", "final", glue("digital_access_{geo}_subgroup_all.csv"))
    )
  } else {
    stop("type must be 'recent', 'multi-year', or 'subgroup'")
  }
}

export_metric(county_overall_digital_access, "recent")
export_metric(
  county_overall_digital_access,
  "multi-year"
)
...
export_metric(place_overall_digital_access, "recent")
export_metric(
  place_overall_digital_access,
  "multi-year"
)
