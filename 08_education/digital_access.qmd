...
raceeth_vars <-
  c( # Population variables
    ...
    "white_nh_digital" = "S2802_C02_013"
  )
...
aggregate_raceeth <- function(data) {
  data |>
    ...
    mutate(
      ...
      moe_white_digital = sqrt(
        moe_white_digital^2 +
          moe_white_nh_digital^2 +
          moe_white_digital^2
      )
    ) |>
    select(
...
calculate_digital_access_metric <- function(pop_data, digital_data) {
  pop_data |>
    mutate(across(
      ...
      .fns = ~ get(paste0(cur_column(), "_digital")) / .x,
      .names = "share_digital_access_{.col}"
    )) |>
...
reshape_for_export <- function(data, subgroup = "race-ethnicity") {
  ...
  if (subgroup == "overall") {
    data <-
      data |>
      rename(
        share_digital_access = share_digital_access_total_households,
        share_digital_access_quality = share_digital_access_total_households_quality
      )
