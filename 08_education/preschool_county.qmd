...
stopifnot(nrow(acs_combined) == nrow(acs))
...
results_race_ethnicity_expanded <- acs_crosswalked %>% 
  ...
  mutate(
    statefip = str_sub(geoid, 1, 2),
    county = str_sub(geoid, 3, 5)
  ) %>% 
  left_join(
    results_race_ethnicity, 
    by = c("year", "statefip", "county", "subgroup_race")
  ) %>% 
  mutate(subgroup_type = "race-ethnicity") %>% 
  rename(subgroup = subgroup_race) %>%
  arrange(year, statefip, county, subgroup)
...
write_csv(
  metrics_education_sub, 
  file = here::here("08_education", "data", "final", "metrics_preschool_county_race-ethnicity_longitudinal.csv")
)
```
