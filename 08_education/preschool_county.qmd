...
The `share_in_preschool_quality` variable combines information on the quality of the crosswalk and the sample size (effective sample) to create a final quality flag. Note that any county with NA for the `share_in_preschool` metric is being suppressed due to sample size and is given a quality flag of 3.  
...
metrics_education_sub %>% 
  ...
  geom_density(alpha = 0.15) +
  theme_minimal() +
  ggtitle("Distribution of share prek_attendance, subgroups, by county 2021 (5-year data)") +
  ylab("Density")
...
metrics_education_sub %>% 
  ...
  facet_wrap(~subgroup) +
  labs(
    title = "Distribution of share prek_attendance, subgroups, by county 2021 (5-year data)",
    y = "Density"
  ) +
  theme_minimal()
...
metrics_education_all %>%
  select(year, geoid, share_in_preschool) %>%
  pivot_wider(
    names_from = year, 
...
write_csv(metrics_education_all, 
          here::here("08_education", "data", "final", "metrics_preschool_county_all_longitudinal.csv")
)

metrics_education_sub %>% 
  head(10)

write_csv(metrics_education_sub, 
          here::here("08_education", "data", "final", "metrics_preschool_county_race-ethnicity_longitudinal.csv")
)
```
