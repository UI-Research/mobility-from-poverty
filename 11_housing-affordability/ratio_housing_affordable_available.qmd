...
extract_housing_microdata <- function(years) {
  # Create sample IDs for all requested years (e.g., "us2014a", "us2015a", ...)
  sample_ids <- paste0("us", years, "a")

  microdata <- define_extract_micro(
    collection = "usa",
    description = paste("Housing microdata extract:", paste(years, collapse = ", ")),
    samples = sample_ids,
    variables = list( ## a list is necessary to accommodate the var_spec() object
      "YEAR", ## year of the sample (needed to split multi-year extracts)
      ...
      var_spec("GQ", case_selections = c("1", "2")), ## only households (GQ < 3), exclude group quarters
      "OWNERSHP", ## tenure
  ...
  # Split by year and save to year-specific files (maintains backward compatibility with code that reads year-by-year files)
  microdata %>%
    group_by(YEAR) %>%
    group_walk(~ write_csv(.x, here(temporary_data_path, paste0("housing_microdata_", .y$YEAR, ".csv"))))
}
...
extract_vacancy_microdata <- function(years) {
  # Create sample IDs for all requested years
  sample_ids <- paste0("us", years, "a")

  microdata <- define_extract_micro(
    collection = "usa",
    description = paste("Vacancy microdata extract:", paste(years, collapse = ", ")),
    samples = sample_ids,
    variables = list( ## a list is necessary to accommodate the var_spec() object
      "YEAR", ## year of the sample (needed to split multi-year extracts)
      "HHWT",
  ...
  microdata %>%
    group_by(YEAR) %>%
    group_walk(~ write_csv(.x, here(temporary_data_path, paste0("vacancy_microdata_", .y$YEAR, ".csv"))))
}
