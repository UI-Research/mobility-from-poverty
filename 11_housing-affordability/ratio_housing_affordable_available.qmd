...
params: 
  ...
  geography: "place"
  county_years: !expr c(2014:2019, 2021:2024)
  place_years: !expr c(2014:2019, 2021:2024)
format: 
  html:
    toc: true
...
```{r}
#| label: example-output-structure
#| echo: false
...
-   Some parallel approaches used to calculate housing affordability and availability make adjustments to what is considered affordable based on the number of bedrooms in the unit and based on the number of people in the household. At present, we do not make these adjustments
...
-   Some of the key features of this analysis include:
    ...
    -   These income thresholds are defined by HUD relative to the area median income (AMI). Generally, HUD calculates AMI thresholds at the county level, but in some states in New England, AMI thresholds vary at the sub-county level. We need to interpolate these AMI thresholds to our geography of interest (either places or counties).
...
# AWS S3 configuration for caching IPUMS extracts and intermediate results
# Internal users: enter the passkey into your environment using Sys.setenv().
# Authentication steps are explained here: https://www.gormanalysis.com/blog/connecting-to-aws-s3-with-r/
# You will have to request access to the access key.
s3_bucket <- "mobility-from-poverty-test"
s3_dir <- "housing-affordability/data"
ipums_extract_date <- "02_11_26" # Update this when changing IPUMS extract variables

# Check if AWS is available (will be FALSE for external users without credentials)
aws_available <- tryCatch(
 bucket_exists(bucket = s3_bucket),
 error = function(e) FALSE
)

# Helper function to check if S3 object exists (suppresses 404 noise when object doesn't exist)
s3_object_exists <- function(object, bucket) {
  tryCatch(
    object_exists(object = object, bucket = bucket),
    error = function(e) FALSE
  )
}
...
place_years_check <- all(place_years %>% as.numeric() %in% c(2014:2019, 2021:2024))
county_years_check <- all(county_years %>% as.numeric() %in% c(2014:2019, 2021:2024))
...
#### ----Setup-----####
temporary_data_path <- here("11_housing-affordability", "data", "temp")
...
```{r}
#| label: download-housing-microdata

# Function to extract housing microdata for multiple years in a single API call
# This is much faster than making separate API calls for each year
# Uses AWS S3 caching when available to avoid redundant IPUMS API calls
# Caches individual year files to S3 for flexibility
extract_housing_microdata <- function(years) {
  # Check which years are already cached on S3 or available locally
  years_to_download <- years
  if (aws_available) {
    years_on_s3 <- years[map_lgl(years, ~ s3_object_exists(
      paste0(s3_dir, "/ipums/housing_microdata_", .x, ".csv"),
      s3_bucket
    ))]

    # Download cached years from S3
    for (year in years_on_s3) {
      s3_object <- paste0(s3_dir, "/ipums/housing_microdata_", year, ".csv")
      local_path <- here(temporary_data_path, paste0("housing_microdata_", year, ".csv"))
      message(paste0("Reading housing microdata for ", year, " from S3 cache..."))
      save_object(object = s3_object, bucket = s3_bucket, file = local_path)
    }

    years_to_download <- setdiff(years, years_on_s3)
  }

  # Download remaining years from IPUMS if needed
  if (length(years_to_download) > 0) {
    message(paste0("Downloading housing microdata from IPUMS API for years: ", paste(years_to_download, collapse = ", ")))
    sample_ids <- paste0("us", years_to_download, "a")

    microdata <- define_extract_micro(
      collection = "usa",
      description = paste("Housing microdata extract:", paste(years_to_download, collapse = ", ")),
      samples = sample_ids,
      variables = list(
        "YEAR",
        "HHWT",
        ...
        "STATEFIP",
        "PUMA",
        var_spec("GQ", case_selections = c("1", "2")),
        "OWNERSHP",
        "OWNCOST",
        "RENT",
        "RENTGRS",
    ...
    # Split by year, save locally, and cache to S3
    microdata %>%
      group_by(YEAR) %>%
      group_walk(function(data, key) {
        ...
        if (aws_available) {
          message(paste0("Caching housing microdata for ", year, " to S3..."))
          s3_object <- paste0(s3_dir, "/ipums/housing_microdata_", year, ".csv")
          put_object(local_path, object = s3_object, bucket = s3_bucket, show_progress = TRUE, multipart = TRUE)
        }
      })
  }
}
...
get_years_to_query <- function(
    directory = here("11_housing-affordability", "data", "temp"),
    years,
    geography) {
...
# Submit a single API request for all missing years (much faster than year-by-year)
years_to_query <- get_years_to_query(years = county_years, geography = geography)
if (length(years_to_query) > 0) {
  extract_housing_microdata(years_to_query)
}
```
...
```{r}
#| label: download-vacancy-microdata

# Function to extract vacancy microdata for multiple years in a single API call
# This is much faster than making separate API calls for each year
# Uses AWS S3 caching when available to avoid redundant IPUMS API calls
# Caches individual year files to S3 for flexibility
extract_vacancy_microdata <- function(years) {
  # Check which years are already cached on S3 or available locally
  years_to_download <- years
  if (aws_available) {
    years_on_s3 <- years[map_lgl(years, ~ s3_object_exists(
      paste0(s3_dir, "/ipums/vacancy_microdata_", .x, ".csv"),
      s3_bucket
    ))]

    # Download cached years from S3
    for (year in years_on_s3) {
      s3_object <- paste0(s3_dir, "/ipums/vacancy_microdata_", year, ".csv")
      local_path <- here(temporary_data_path, paste0("vacancy_microdata_", year, ".csv"))
      message(paste0("Reading vacancy microdata for ", year, " from S3 cache..."))
      save_object(object = s3_object, bucket = s3_bucket, file = local_path)
    }
  ...
  # Download remaining years from IPUMS if needed
  if (length(years_to_download) > 0) {
    message(paste0("Downloading vacancy microdata from IPUMS API for years: ", paste(years_to_download, collapse = ", ")))
    sample_ids <- paste0("us", years_to_download, "a")

    microdata <- define_extract_micro(
      collection = "usa",
      description = paste("Vacancy microdata extract:", paste(years_to_download, collapse = ", ")),
      samples = sample_ids,
      variables = list(
        "YEAR",
        "HHWT",
        "REPWT",
        var_spec("GQ", case_selections = c("0")),
        "ADJUST",
        "STATEFIP",
        "PUMA",
        "VALUEH",
    ...
    microdata %>%
      ...
      group_walk(function(data, key) {
        ...
        # Cache to S3 if available
        if (aws_available) {
          message(paste0("Caching vacancy microdata for ", year, " to S3..."))
          s3_object <- paste0(s3_dir, "/ipums/vacancy_microdata_", year, ".csv")
          put_object(local_path, object = s3_object, bucket = s3_bucket, show_progress = TRUE, multipart = TRUE)
        }
      })
...
# Submit a single API request for all missing years (much faster than year-by-year)
if (length(years_to_query) > 0) {
  extract_vacancy_microdata(years_to_query)
}
```
...
```{r}
#| label: crosswalk-puma-to-geography
...
puma_geography_crosswalk <- function(year, geography) {
  ...
  crosswalk <- here("geographic-crosswalks", "data", paste0("crosswalk_puma_to_", geography, ".csv")) %>%
    ...
    {
      if (crosswalk_period %in% c(2022:2024)) mutate(., year = year) else .
    }
...
crosswalk_microdata <- function(year, geography) {
  output_path <- here(temporary_data_path, paste0(geography, "_prepared_microdata_", year, ".csv"))

  # Skip if output already exists
  if (file.exists(output_path)) {
    message(paste0("Skipping ", geography, " ", year, " - prepared microdata already exists"))
  ...
  read_csv(here(temporary_data_path, paste0("housing_microdata_", year, ".csv"))) %>%
    ...
    mutate(
      ...
      ## due to coding NAs as zero, every other value is one too high
      BEDROOMS = case_when(
        BEDROOMS == 0 ~ NA,
        TRUE ~ BEDROOMS - 1
      ),
      # the weight of each person/household is adjusted by the area of the PUMA
      # that falls into a given place
...
```{r}
#| label: prepare-occupied-housing-microdata
...
housing_microdata <- list.files(here(temporary_data_path), full.names = TRUE) %>%
  ...
  keep(
    ~ str_detect(
      .x,
      str_c(
        geography, "_prepared_microdata_",
  ...
  map_dfr(
    ~ read_csv(.x) %>%
      ...
      select(
        -c(
          PERWT, PERNUM, SAMPLE, SERIAL, GQ, products, afact2, sum_products,
          sum_products, microdata_id
        )
      )
  )
...
```{r}
#| label: prepare-vacancy-microdata
...
vacancy_microdata <- list.files(here(temporary_data_path), full.names = TRUE) %>%
  ...
  map_dfr(
    ~ read_csv(.x) %>%
      ...
      mutate(
        year = as.numeric(str_extract(.x, "[0-9]{4}")), # extract year from filename
        # "0" represents NA: https://usa.ipums.org/usa-action/variables/RENT#codes_section
        # And, though not noted in IPUMS documentation, this also applies for the
        # RENTGRS variable
        across(
      ...
      select(year, crosswalk_period, puma, statefip, HHWT, ADJUST, RENT, RENTGRS, VALUEH, monthly_cost_total, BEDROOMS)
  )
...
income_limits_urls <- tibble(
  ...
  url_years = c(2014:2019, 2021:2025)
) %>%
  filter(url_years %in% years)

# Downloads and processes HUD income limits for a given year
# Uses AWS S3 caching when available to avoid redundant downloads
# Caches the original xlsx/xls file with a standardized name
download_hud_income_limits <- function(year, url) {
  # Extract original file extension from URL
  original_filename <- str_split(url, "/") %>%
    .[[1]] %>%
    .[[length(.)]]
  file_ext <- tools::file_ext(original_filename)

  # Standardized filename with original extension preserved
  filename <- paste0("hud_income_limits_", year, ".", file_ext)
  s3_object <- paste0(s3_dir, "/hud/", filename)
  local_path <- here(temporary_data_path, filename)

  # Check if cached file exists on S3
  if (aws_available && s3_object_exists(s3_object, s3_bucket)) {
    message(paste0("Reading HUD income limits for ", year, " from S3 cache..."))
    save_object(object = s3_object, bucket = s3_bucket, file = local_path)
  } else {
    message(paste0("Downloading HUD income limits for ", year, " from HUD..."))
    download.file(url = url, destfile = local_path, mode = "wb")

    # Cache to S3 if available
    if (aws_available) {
      message(paste0("Caching HUD income limits for ", year, " to S3..."))
      put_object(local_path, object = s3_object, bucket = s3_bucket, show_progress = TRUE, multipart = TRUE)
    }
  }

  # Import the data file as a dataframe
  fmr_income_limits <- readxl::read_excel(path = local_path) %>%
    janitor::clean_names() %>%
    ## the name for the fips variable changes after 2023
    {
    ...
    transmute(
      ...
      # ELI_4 is 30% of median rent for a four-person household: Extremely low-income
      # l50_4 is 50% of median rent for a four-person household: Very low-income
      # l80_4 is 80% of median rent for a four-person household: Low-income
      level_30ami = eli_4,
      level_50ami = l50_4,
      level_80ami = l80_4
    )
...
-   We aggregate our income limits to the county level. For most counties, this doesn't require any work, because there's only a single income limit for the county. For counties with sub-county-level income limits, we take the mean of these limits. For revised CT counties (beginning in 2022), we draw in a special crosswalk to go from sub-counties to counties.
...
```{r}
#| label: crosswalk-ami-to-geography

## addressing revised (starting in 2022) CT counties
if (as.numeric(max(c(county_years, place_years)) >= 2022)) {
  ## a crosswalk that links CT subcounties to revised CT counties
  ct_county_crosswalk <- read_csv(
    "https://raw.githubusercontent.com/CT-Data-Collaborative/ct-town-to-planning-region/refs/heads/main/ct-town-to-planning-region.csv"
  ) %>%
    transmute(
      fips = str_pad(as.character(town_fips_2020), side = "left", width = 10, pad = "0"),
      county_adjusted = str_sub(as.character(ce_fips_2022), 2, 4)
    )

  ## subset our HUD-published crosswalk to CT records from 2022 onward
  ## then join these to the CT-published crosswalk
  ct_county_income_limits <- fmr_income_limits_1 %>%
    filter(year >= 2022, state == "09") %>%
    tidylog::left_join(
      ct_county_crosswalk,
      by = "fips",
      relationship = "many-to-many"
    ) %>%
    mutate(county = county_adjusted) %>%
    select(-county_adjusted)

  fmr_income_limits_2 <- fmr_income_limits_1 %>%
    ## add our modified CT income limits to our original HUD income limits
    ## dropping the original records for CT from 2022 onward
    filter(!(fips %in% ct_county_income_limits$fips & as.numeric(year) >= 2022)) %>%
    bind_rows(ct_county_income_limits)
} else {
  ## if we're not dealing with 2022-onwards CT, no need to make adjustments
  fmr_income_limits_2 <- fmr_income_limits_1
}

county_income_limits1 <- fmr_income_limits_2 %>%
...
## in 2019, Valdez-Cordova Census Area -- a county-equivalent geography -- is abolished
...
county_income_limits3 <- tibble(
  ...
  left_join(
    county_income_limits2 %>%
      filter(
        statefip == "02",
        ## change happens in 2019, so we only want to apply new census areas
        ## for 2020 onwards
        year > 2019
      ),
...
county_income_limits <- county_income_limits2 %>%
  filter(!(statefip == "02" & county == "261" & year > 2019)) %>%
...
if (geography == "place") {
  ## Read period-specific crosswalks and standardize column names
  ## Pre-2020 crosswalk (2014 vintage geographies, used for years < 2020)
  ## Note: Row 2 contains descriptions, not data, so we skip it after reading
  crosswalk_pre_2020 <- read_csv(
    here("geographic-crosswalks", "data", "geocorr2018_county_place_2014_thru_2019.csv")
  ) %>%
    filter(county14 != "County code (2014)") %>% ## remove description row
    transmute(
      countyfip = county14,
      state = str_pad(as.numeric(state), side = "left", width = 2, pad = "0"),
      place = str_pad(as.numeric(placefp14), side = "left", width = 5, pad = "0"),
      population = as.numeric(pop10),
      afact = as.numeric(afact)
    ) %>%
    filter(state != "72") ## exclude Puerto Rico

  ## 2020-2021 crosswalk
  crosswalk_2020_2021 <- read_csv(
    here("geographic-crosswalks", "data", "geocorr2022_county_place_2020_2021.csv")
  ) %>%
    transmute(
      countyfip = str_pad(as.character(countyfip), side = "left", width = 5, pad = "0"),
      state = str_pad(as.numeric(state), side = "left", width = 2, pad = "0"),
  ...
  ## 2022+ crosswalk (includes CT planning regions replacing CT counties)
  crosswalk_2022_plus <- read_csv(
    here("geographic-crosswalks", "data", "geocorr2022_county_place_2022_onwards.csv")
  ) %>%
    transmute(
      countyfip,
      state = str_pad(as.numeric(state), side = "left", width = 2, pad = "0"),
  ...
  ## Expand each crosswalk by the relevant years and combine
  years_numeric <- as.numeric(years)

  county_place_crosswalk <- bind_rows(
    crosswalk_pre_2020 %>%
      crossing(year = as.character(years_numeric[years_numeric < 2020])),
    crosswalk_2020_2021 %>%
      crossing(year = as.character(years_numeric[years_numeric %in% 2020:2021])),
    crosswalk_2022_plus %>%
      crossing(year = as.character(years_numeric[years_numeric >= 2022]))
  ) %>%
    transmute(
      county = str_sub(countyfip, 3, 5),
      year,
      state,
      place,
      afact,
  ...
  place_income_limits <- county_income_limits %>%
    left_join(county_place_crosswalk, by = c("state", "county", "year"), relationship = "many-to-many") %>%
    group_by(state, place, year) %>%
    summarize(
      across(
        .cols = matches("ami"),
        .fns = ~ weighted.mean(x = .x, na.rm = TRUE, w = population)
      )
    ) %>%
    ungroup() %>%
    mutate(
  ...
  crosswalked_income_limits <- county_income_limits
}
```

## Step 4: Generate AMI indicators

Here we calculate indicator variables that designate which units are affordable at 80%, 50%, and 30% of AMI and which households--including breakdowns for renter and owner subgroups--fall below those same AMI levels.

The base AMI level is for a four-person household living in a three-bedroom unit (technically, 2.67 bedrooms, because it is assumed that each bedroom houses 1.5 people). As noted above, we do not make adjustments to our estimates based on the number of people in each household, nor on the number of bedrooms in each unit. Instead, we apply the base AMI level uniformly across all households and units.

```{r}
#| label: calculate-household-affordability
...
household_affordability <- tidylog::left_join(
  ...
  mutate(
    ...
    ## renter-occupied households with affordable housing costs
    across(
    ...
    ## owner-occupied households with affordable housing costs
    across(
      .cols = c(level_30ami, level_50ami, level_80ami),
      .fns = ~ case_when(
        is.na(applicable_housing_cost) ~ NA,
...
```{r}
#| label: calculate-vacant-affordability
...
```{r}
#| label: aggregate-calculate-metrics
...
housing_subgroups <- housing_summed_quality %>%
  ...
  rename_with(
    ...
    .fn = ~ .x %>%
      str_remove("_quality") %>%
      str_replace("below", "affordable_quality_level") %>%
      str_c("share_", .)
  ) %>%
  ...
  ## renaming to maintain consistency with prior productions / external references to
  ## this metric historically (though this is a ratio, not a percentage)
  rename_with(
    .cols = everything(),
    .fn = ~ str_replace_all(.x, c("ratio" = "share", "ami" = "_ami"))
  ) %>%
  rename_with(
    .cols = matches("available"),
    .fn = ~ str_replace(.x, "available", "affordable_available")
  ) %>%
  select(
    year,
    state,
    ...
    subgroup_type,
    subgroup,
    matches("share_affordable_[0-9]{2}_ami$"),
    matches("share_affordable_[0-9]{2}_ami_quality$"),
    matches("share_affordable_available.*ami$"),
    matches("share_affordable_available.*quality$")
  ) %>%
  arrange(state, .data[[geography]], subgroup, desc(year)) %>%
  ## due to crosswalking non-alignment, we drop south fulton georgia for 2018-2021
  {
    if (geography == "place") filter(., !(place == "72122" & state == "13" & (year %in% c(2018:2021)))) else .
  }
...
# export the final data locally
write_csv(
  housing_overall,
  here("11_housing-affordability", "data", "final", str_c("housing_", geography, "_", years, ".csv"))
)

write_csv(
  housing_subgroups,
  here("11_housing-affordability", "data", "final", str_c("housing_", geography, "_", years, "_subgroups.csv"))
)

# also cache final results to S3 if available
if (aws_available) {
  message("Caching final results to S3...")

  # Cache overall metrics
  tmp_overall <- tempfile(fileext = ".csv")
  write_csv(housing_overall, tmp_overall)
  put_object(
    tmp_overall,
    object = paste0(s3_dir, "/final/housing_", geography, "_", years, ".csv"),
    bucket = s3_bucket,
    show_progress = TRUE
  )
  unlink(tmp_overall)

  # Cache subgroup metrics
  tmp_subgroups <- tempfile(fileext = ".csv")
  write_csv(housing_subgroups, tmp_subgroups)
  put_object(
    tmp_subgroups,
    object = paste0(s3_dir, "/final/housing_", geography, "_", years, "_subgroups.csv"),
    bucket = s3_bucket,
    show_progress = TRUE
  )
  unlink(tmp_subgroups)
}
```
...
```{r}
#| label: run-standardized-qc-checks

## we modify the expectations form to contain the same years for which we're running
## this script:
data_form_paths <- list.files(
  here("10a_final-evaluation"),
  full.names = TRUE
) %>%
  keep(~ str_detect(.x, "housing.*(affordability|availability).*tenure"))
...
walk(
  ...
  ~ read_csv(.x) %>%
    mutate(
      ...
      `...3` = if_else(row_number > 3, years %>% str_replace_all("_", ";"), `...3`)
    ) %>%
    write_csv(.x)
)
      ...
      walk(
        ...
        ~ evaluate_final_data(
          ...
          data = housing_subgroups %>%
            {
              if (str_detect(.x, "availability")) {
                select(., -matches("affordable_[0-9]{2}"))
              } else {
                select(., -matches("available"))
              }
            } %>%
            rename_with(
              .cols = everything(),
              .fn = ~ str_replace(.x, "ratio", "share")
            ),
          geography = geography,
          subgroups = TRUE,
...
```{r}
#| label: test-geography-counts

population_listing1 <- read_csv(here("geographic-crosswalks", "data", paste0(geography, "-populations.csv"))) %>%
  mutate(
...
if (geography == "county") {
  ## though Chugach and Copper River Census Areas don't exist until 2020,
  ## we create metrics for these counties in the years 2014:2019 for continuity
  ## as reflected in the PUMA-county crosswalk. accordingly, we adjust the population
  ## listing here to capture this
  adjusted_ak_counties <- data.frame(
    year = rep(c(2014:2019) %>% as.character(), 2),
    geoid = c(rep("02063", 6), rep("02066", 6))
  ) %>%
    left_join(
      population_listing1 %>%
  ...
  population_listing <- population_listing1 %>%
    filter(geoid != "02261") %>%
    bind_rows(adjusted_ak_counties)
} else {
  population_listing <- population_listing1
}
...
geography_universe <- population_listing %>%
  ...
  summarize(geography_count = n_distinct(geoid)) %>%
  {
    if (geography == "place") {
      mutate(.,
        ## South Fulton, GA was incorporated in 2017 and appears in place-populations
        ## starting in 2018. However, the pre-2022 PUMA-place crosswalk (based on 2010
        ## boundaries) predates its existence, so we cannot produce estimates for
        ## 2018-2021. We adjust expected counts accordingly.
        geography_count = case_when(
          year %in% c(2018:2021) ~ geography_count - 1,
    ...
    expect_true(
      all(
        housing_subgroups %>%
          ...
          left_join(geography_universe, by = "year") %>%
...
#### ----to identify geography non-alignment----####
# housing_year_geographies = housing_subgroups %>%
#   transmute(year, geoid = str_c(state, .data[[geography]]), flag = 1) %>%
#   distinct()
#
# ## geographies in the official universe of geographies
# ## where we don't have a matching record in our metric data
# population_listing %>%
#   filter(!year == "2020") %>%
#   anti_join(housing_year_geographies, by = c("year", "geoid"))
#
# ## geographies in our metric data where we don't have a matching record in the
# ## official universe of geographies
# housing_year_geographies %>%
#   anti_join(population_listing %>% filter(year != "2020"))
```
...
test_that(
  "No ratios are negative or zero",
  {
    expect_true(
      housing_overall %>%
        filter(if_any(.cols = c(matches("share"), -matches("quality")), .fns = ~ .x <= 0)) %>%
        nrow() == 0
    )
  }
)
...
***No State Comprises Only NA Values***

In particular, this is a check for Connecticut due to changing counties (pre-post 2022),

```{r}
#| label: test-no-all-na-states
...
test_that(
  "No state comprises only NA values",
  {
    expect_true(
      housing_subgroups %>%
        filter(subgroup == "All") %>%
        select(c(year, state, !!geography, matches("share"), -matches("quality"))) %>%
        group_by(state, year) %>%
        summarize(
          across(
...
```{r}
#| label: test-available-less-than-affordable
  ...
  "Affordable-and-available ratios are less than or equal to affordable ratios",
  {
    expect_true(
      housing_overall %>%
        ...
        mutate(
          across(
            .cols = c(matches("share_affordable_[0-9]")),
            .fns = ~ .x - get(str_replace(cur_column(), "affordable", "affordable_available")),
            .names = "{.col}_difference"
          )
        ) %>%
        arrange(share_affordable_80_ami_difference) %>%
...
```{r}
#| label: compare-prior-year-estimates
...
if (geography == "place") {
  metrics_prior1 <- read_csv(
    ...
    "https://urban-data-catalog.s3.amazonaws.com/drupal-root-live/2025/05/13/67_mobility-metrics_place_tenure_longitudinal.csv"
  )
} else {
  metrics_prior1 <- read_csv(
    ## this corresponds to longitudinal, county-level metrics that are broken down by tenure
    "https://urban-data-catalog.s3.amazonaws.com/drupal-root-live/2025/05/13/17_mobility-metrics_county_tenure_longitudinal.csv"
  )
}
...
metrics_differences_wide_plot %>%
  ...
  labs(
    title = "Share of non-missing geographies with differences in calculated values for 2023 data, 2025 vs. 2026" %>% str_wrap(90),
    y = "", x = ""
  ) +
  theme_urbn_print()
...
metrics_differences_wide_plot %>%
  ...
  theme_urbn_print() +
  labs(
    title = "Differences in calculated values for 2023 data, 2025 vs. 2026",
    x = "Difference in value, 2025 vs 2026",
    y = "Count"
  )
```

```{r}
#| label: test-no-breaking-changes

# Test that current calculations match prior published values (within floating point tolerance)
tolerance <- 1e-10

test_that(
  "Values produced by current codebase match prior year's published values (within tolerance)", {

    differences <- metrics_prior %>%
      tidylog::left_join(
        housing_subgroups %>%
          mutate(subgroup = subgroup %>% str_to_sentence()) %>%
          select(-subgroup_type, -matches("quality")),
      ...
      transmute(
        ...
        across(
          .cols = matches("_prior$"),
          .fns = ~ abs(.x - get(cur_column() %>% str_remove("_prior"))),
          .names = "{.col}_diff"
        )
      ) %>%
      summarize(
        across(matches("_diff$"), ~ max(.x, na.rm = TRUE))
      ) %>%
      pivot_longer(everything(), names_to = "metric", values_to = "max_diff")

    # All max differences should be below tolerance
    expect_true(
      all(differences$max_diff < tolerance),
      info = paste(
        "Max differences by metric:",
        paste(differences$metric, "=", format(differences$max_diff, scientific = TRUE), collapse = ", ")
      )
    )
  }
)
...
We anticipate that missingness is greater for the more restrictive income subgroups (e.g., missingness for 30 AMI is greater than missingness for 80 AMI).

```{r}
#| label: describe-missingness

housing_subgroups %>%
  select(year, c(matches("share"), -matches("quality"))) %>%
  group_by(year) %>%
  summarize(
    across(
      .cols = everything(),
...
housing_subgroups %>%
  select(c(subgroup, year, matches("share"), -matches("quality"))) %>%
  pivot_longer(-c(subgroup, year)) %>%
  filter(str_detect(name, "affordable_[0-9]")) %>%
  mutate(name = name %>% str_replace_all(c("_" = " ", "share" = "", "affordable" = "aff.", "available" = "avail.")) %>% str_to_sentence()) %>%
  ggplot() +
...
housing_subgroups %>%
  ...
  mutate(name = name %>% str_replace_all(c("_" = " ", "share" = "", "affordable " = "", "available" = "avail.")) %>% str_to_sentence()) %>%
  ggplot() +
  geom_histogram(aes(x = value, fill = subgroup)) +
  facet_wrap(~ name + year, ncol = 9) +
...
```{r}
#| label: plot-yoy-changes

if (length(c(county_years, place_years) %>% unique()) > 1) {
  year_over_year_changes <- housing_subgroups %>%
    select(-matches("quality")) %>%
    pivot_longer(cols = c(matches("share"))) %>%
    filter(value > .01) %>%
  ...
  year_over_year_changes %>%
    ggplot() +
    geom_histogram(aes(x = year_over_year_change, fill = subgroup), bins = 100) +
    facet_wrap(~name) +
...
if (geography == "county") {
  nlihc_estimates_metro <- tribble(
    ...
    select(
      metro_area,
      share_affordable_available_30_ami_nlihc = at_or_below_eli_2,
      share_affordable_available_50_ami_nlihc = at_or_below_50_percent_ami_2,
      share_affordable_available_80_ami_nlihc = at_or_below_80_percent_ami
    ) %>%
    mutate(
  ...
  nlihc_estimates_metro %>%
    tidylog::left_join(
      housing_subgroups %>%
        ...
        select(c(matches("share_affordable_available"), -matches("quality")), geoid),
      by = c("county_match_geoid" = "geoid")
    ) %>%
    filter(!is.na(share_affordable_available_30_ami)) %>%
    mutate(
