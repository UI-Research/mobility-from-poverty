...
create_comparison_scatterplot <- function(geo, bedroom_adjustment) {
  ...
  if (bedroom_adjustment) {
    ...
    years <- 2023 # Only outputted 2023 values without bedroom adjustment for now
  ...
  # Read updated data
  updated <- read_csv(
    here("11_housing-affordability", "data", path, str_c("housing_", geo, "_", years, "_subgroups.csv"))) |>
    pivot_longer(cols = starts_with("share_")) |>
  ...
  previous |>
    ...
    labs(
      title = str_c("Comparison of ratios (", label, ", ", geo, ", 2023)"),
      y = "Updated",
      x = "Previous") +
    theme_urbn_print() 
}
