...
source(here::here("03_opportunities-for-income", "R", "calc_income_quantiles_subgroup.R"))
```
...
Median income for select places over time by race and ethnicity
...
metrics_income_sub %>%
  ...
  mutate(place = case_when(place == "14000" ~ "Chicago, IL",
         ...
         place = factor(place, levels = c("Chicago, IL",
                                            ...
                                            "Houston, TX",
                                            "Richmond, VA"))) %>% 
  ggplot(aes(year, pctl_income_50, color = subgroup)) +
  geom_line() +
...
80th percentile income for select places over time by race and ethnicity
...
metrics_income_sub %>%
  ...
  mutate(place = case_when(place == "14000" ~ "Chicago, IL",
         ...
         place = factor(place, levels = c("Chicago, IL",
                                            ...
                                            "Houston, TX",
                                            "Richmond, VA"))) %>% 
  ggplot(aes(year, pctl_income_80, color = subgroup)) +
  geom_line() +
  facet_wrap(~place, ncol = 4) +
  scale_x_continuous(expand = expansion(mult = c(0.002, 0)), 
                     limits = c(2014, 2024)) +
...
Median income for select places over time by gender
...
write_csv(metrics_income_gender, 
          here::here("03_opportunities-for-income", "final", "metrics_income_place_gender_longitudinal.csv"))

metrics_income_race %>% 
  head(10)

write_csv(metrics_income_race, 
          here::here("03_opportunities-for-income", "final", "metrics_income_place_race-ethnicity_longitudinal.csv"))
