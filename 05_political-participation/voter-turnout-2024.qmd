...
output:
  html:
    ...
    toc: TRUE
    toc_float: TRUE
    css: !expr here::here("05_political-participation", "www", "web_report.css")
editor_options:
...
intermediate_files <- here::here("05_political-participation",
                                 "data")
...
mit_directory <- here::here("05_political-participation", 
                            "data",
                            "mit")

mit_data <- here::here("05_political-participation", 
                       "data",
                       "mit",
...
if (!file.exists(mit_data)) {
  ...
  download.file(
    "https://dataverse.harvard.edu/api/access/datafile/13390025?format=original&gbrecs=true",
    destfile = mit_data
  )
...
```{r}
mit <- read_csv(here::here("05_political-participation", 
                           "data",
                           "mit", 
                           "countypres_2000-2024.csv")) %>%
...
The voting `mode` category and the categories under `candidate` differ by state in the 2024 data. This requires further cleaning to get the totals as including the entire vote category now results in duplicates.
...
The table below shows the differences in state candidate categories. Some states have "total votes" as a candidate category. 
...
The total are inconsistent in these data and you can get different answers based on which values are used. Showing Vermont as an example, we can see we get slightly different totals using three approaches. 

1) Using the 'total votes' column which is unique for each county. Note that this value results in very different numbers overall for states when compared to Wikipedia and census published totals,

```{r}
mit %>% 
  filter(state == "VERMONT") %>% 
  select(state, county_name, FIPS, totalvotes) %>% 
  unique() %>% 
  summarise(total_votes = sum(totalvotes, na.rm = TRUE))
```

2) Using the 'candidatevotes' column and filtering the candidate column for the value "TOTAL VOTES CAST" category

```{r}
mit %>% 
  filter(state == "VERMONT", candidate == "TOTAL VOTES CAST") %>% 
  select(state, county_name, FIPS, candidatevotes) %>% 
  unique() %>% 
  summarise(total_votes = sum(candidatevotes, na.rm = TRUE))
```

3) Using the 'candidatevotes' column and filtering the candidate column to exclude the "TOTAL VOTES CAST" category
...
For consistency we need to pick an approach.

We decide to only use "TOTAL VOTES" or NA from the mode and we need to remove "TOTAL VOTES CAST" from the candidate category.

Show the vote distribution by mode for all states 
...
New Mexico requires additional attention because it does not include a total row. 

To fix we change all of the mode category to "TOTAL VOTES" for New Mexico and then for the entire data filter to either "TOTAL VOTES" or NA.
```{r}
mit %>% 
...
mit %>% 
  filter(state == "SOUTH DAKOTA", county_name == "BROOKINGS") %>% 
  group_by(mode) %>% 
  summarise(votes = sum(candidatevotes))

mit <- mit %>% 
  mutate(mode = case_when(state == "NEW MEXICO" ~ "TOTAL VOTES", 
                          state == "SOUTH DAKOTA" & mode == "VOTE CENTER" ~ "TOTAL VOTES",
                          TRUE ~ mode)) %>% 
  filter(candidate != "TOTAL VOTES CAST", mode %in% c("TOTAL VOTES", NA_character_))
...
For St. Francis county Arkansas there seems to be a mistake in the data - all [census documentation](https://www.census.gov/quickfacts/fact/table/stfranciscountyarkansas/PST045224) indicates this county FIP is "05123" but it is being reported by MIT as "05135". We see this result in a mismatch later in this code when joining to the CVAP data. 

Correct it here.

```{r}
mit <- mit %>%
  mutate(FIPS = if_else(county_name == "ST FRANCIS" & state == "ARKANSAS", "05123", FIPS))
```
...
The aggregated nationwide total from MIT of 155,115,321 is also very close to the 155,240,955 votes reported on Wikipedia. We have 125,634 less votes than Wikipedia.
...
Read in Census data on 2024 election trunout 
...
```{r}
cvap_zip <- here::here("05_political-participation", 
                       "data",
                       "CVAP_2020-2024_ACS_csv_files.zip")

cvap_data <- here::here("05_political-participation", 
                        "data",
...
```{r}
cvap <- read_csv(here::here("05_political-participation", 
                            "data",
                            "CVAP_2020-2024_ACS_csv_files", 
                            "County.csv"))
...
28 observations have voter turnout above `1`. For states outside of Kansas, this is likely because of sampling error in the denominator. We set these values to one and all of these cases will be flagged. 
...
However Kansas has a problem.

This code scrapes the vote count from Wikipedia: 

```{r}
# scrape data from Wikipedia with rvest
wiki_kansas <- read_html("https://en.wikipedia.org/wiki/2024_United_States_presidential_election_in_Kansas")

wiki_votes_kansas <- wiki_kansas %>%
...
# the name column in the data doesn't scrape well so we manually state it
kansas_counties <- kansas_data %>% 
  select(county_name, totalvotes) %>% 
  unique()

kansas_comp <- bind_cols(kansas_counties, as_tibble(wiki_votes_kansas)) %>% 
  mutate(diff = totalvotes - value,
         diff_check_lead = totalvotes - lead(value, 1))

rm(wiki_kansas, wiki_votes_kansas, kansas_counties)

```

The MIT data started to incorrectly assign the vote data to counties halfway through this list due to skipping Johnson county. 
...
all_counties %>%
  select(-population) %>%
  write_csv(here::here("05_political-participation",
                       "data",
                       "final",
...
all_counties %>%
  select(-population) %>%
  bind_rows(
    read_csv(here::here("05_political-participation", "data", "final", "voter-turnout-2016.csv")),
    read_csv(here::here("05_political-participation", "data", "final", "voter-turnout-2020.csv"))
  ) %>% 
  arrange(year) %>% 
  write_csv(here::here("05_political-participation",
                       "data",
...
```{r}
turnout2020 <- read_csv(here::here("05_political-participation",
                                    "data",
                                    "final",
