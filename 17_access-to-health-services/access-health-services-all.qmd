...
## 2. Download and read in data from AHRF
This section uses a helper function, `ahrf_fetch()`, to download and unzip AHRF zip files from the AHRF website. After it runs, you should inspect the unzipped contents and pass the explicit `.sas7bdat` path to `haven::read_sas()` to read the files.  
This section downloads two AHRF zip files from the AHRF website. Also, check for
any irrelevant files such as those containing documentation for each file (e.g., 
`"2021-2022_AHRFDUA.doc"` in the 2021-2022 release.

The primary data for this metric come from the [Area Health Resource File (AHRF)](https://data.hrsa.gov/data/download). 

To get all years 2014-2022, we must download and read in three files:

* 2023-2024 County Level Data - This is the most recent release and includes primary care physicians data for 2022
...
**Note:** each code chunk below downloads the `.zip` file for each year, extracts the data we need, and deletes the `.zip` file. The `.zip` files contain the technical documentation for each file, and future analysts may want to explore this documentation before deleting the files.
```{r load-ahrf-sas-files}
#' Fetch (download + unzip) an AHRF SAS release
#'
#' @description
#' Given a direct URL to an AHRF SAS .zip, this function downloads the archive
#' to a temporary file, unzips it into `data_dir`, and deletes the temporary zip.
#' After it runs, inspect `data_dir` to identify the exact `.sas7bdat` path
#' produced by that release, then call `haven::read_sas()` yourself using that
#' explicit path. This keeps all year-specific quirks outside the function.
#'
#' @param url Character. Direct link to the AHRF SAS zip.
#' @param data_dir Character. Directory to unzip into.
#'   Default: `here::here("17_access-to-health-services","data")`.
#'
#' @return Invisibly returns `data_dir` (character). Files are written to disk.
#'
#' @examples
#' \dontrun{
#' ahrf_fetch("https://data.hrsa.gov/DataDownload/AHRF/AHRF%202023-2024%20SAS.zip")
#' # Then manually point read_sas() to the file path you expect, e.g.:
#' # haven::read_sas(here::here("17_access-to-health-services","data",
#' #                             "AHRF 2023-2024 SAS","ahrf2024_feb2025.sas7bdat"),
#' #                 catalog_encoding = "Latin1")
#' }
ahrf_fetch <- function(
  url,
  data_dir = here::here("17_access-to-health-services", "data")
) {
  ...
  utils::download.file(url, destfile = zip_tmp, mode = "wb")
  utils::unzip(zipfile = zip_tmp, exdir = data_dir, overwrite = TRUE)
  unlink(zip_tmp)

  invisible(normalizePath(data_dir))
}

# 2021–2022
ahrf_fetch("https://data.hrsa.gov//DataDownload/AHRF/AHRF_2021-2022_SAS.zip")
ahrf22 <- haven::read_sas(
  here::here("17_access-to-health-services","data","ahrf2022.sas7bdat"),
  catalog_encoding = "Latin1"
)

# Remove zip file and irrelevant contents 
unlink(here::here("17_access-to-health-services", "data", "2021-2022_AHRFDUA.doc"))

# 2022–2023
ahrf_fetch("https://data.hrsa.gov//DataDownload/AHRF/AHRF_SAS_2022-2023.zip")
ahrf23 <- haven::read_sas(
  here::here(
    "17_access-to-health-services","data",
...
ahrf24 <- ahrf24 |>
  select(
    state = fips_st,
    county = fips_cnty,
    pc_physicians_22 = phys_nf_prim_care_pc_exc_rsdt_22,
    pop_22 = popn_est_22
  )
...
ahrf_joined <- full_join(
  ...
  full_join(ahrf22, by = c("state", "county")) |> 
...
The 2023-2024 file (which has 2022 information for primary care physicians) has nine extra observations corresponding to the Connecticut planning regions.
...
all_counties <- read_csv(here::here("geographic-crosswalks", "data", "county-populations.csv"), locale = locale(encoding = "Latin1")) |>
  filter(!year %in% c(2023)) |>
  mutate(state = str_pad(state, 2, pad = "0"),
         county = str_pad(county, 3, pad = "0"))

# Make sure that all years are included in the file
testthat::test_that("Check that all years are included in both files", {
  expect_equal(
    unique(all_counties$year),
    unique(ahrf_long$year)
  )
})
...
* Shannon County, SD (46113), [is renamed to Oglala
Lakota County, SD (FIPS 46102) in 2015](https://www.cdc.gov/nchs/data/data_acces_files/County-Geography.pdf). In the AHRF data the FIPS code used for Shannon County, SD is  46102 in 2014 but in the population file the original code (46113) is used. We will replace  FIPS code 46102 with the old code 46113.
* Wade-Hampton Census Area, AK
(02270) was renamed Kusilvak, AK and assigned a new FIPS code (02158) effective in 2014. We will replace this FIPS code in 2014 with the old code since this is what is present in the county file.
...
We drop these observations from the AHRF data and replace the appropriate FIPS codes.
```{r fix-missing-urban}
ahrf_clean <- ahrf_long |>
  mutate(GEOID = str_c(state, county)) |>
  mutate(county = case_when(year == "2014" & state == "46" & county == "102" ~ "113",
                            year == "2014" & state == "02" & county == "158" ~ "270",
                            TRUE ~ county)) |>
  
  filter(!(state == "09" & (year %in% c("2021", "2022"))),
         !GEOID %in% c("02001", "02201", "02232", "02280", "51515", "51560"),
         !(GEOID %in% c("02063", "02066") & 
             year %in% c("2014", "2015", "2016", "2017", "2018", "2019")),
         !(GEOID %in% c("02261") & year %in% c("2020", "2021", "2022"))) |>
  select(-GEOID)
...
Urban Institute and AHRF population sources match for every year.
```{r check-pop}
...
In the previous section, we showed that the only `NA` values of primary care physicians in the AHRF data were for nonexistent counties. Therefore, we are unable to distinguish between counties that reported 0 primary care physicians and counties that are coded as `0` but should really be `NA` because they did not report at all or because information was not available. Note that these are administrative data, so it is unclear to me how reporting errors could appear, but it isn't impossible.

County Health Rankings suppresses data for counties with population greater than 2,000 and 0 primary care physicians. They report a missing value for these observations. The rationale for this suppression rule is not explained in their documentation, but our assumption is that they assume "0 PCPs" could be a reporting error for counties with a decent-sized population; it is likely that these counties have at least 1 PCP, so they don't trust the reported `0` value and code it as `NA` instead. Future updates may want to consider contacting County Health Rankings to learn more about this suppression decision.

We apply the same rule for suppression. If population is less than 2000 and 0 PCPs are reported, we assign a value of 0. If the population is greater than 2000 and 0 PCPs are reported, we assign a value of `NA` and assume that these are "false zeros." If there is at least 1 PCP reported, we assign a value that equals the population per 1 primary care physician. The antecedent of the ratio (numerator) is population and the consequent of the ratio (denominator) is number of primary care physicians.
...
joined_data <- joined_data |>
  mutate(
    ratio_population_pc_physician = case_when(
      population <= 2000 & pc_physicians == 0 ~ 0,
      population > 2000 & pc_physicians == 0 ~ NA,
      pc_physicians > 0 ~ round(population/pc_physicians),
      is.na(pc_physicians) ~ NA
      )
  )
...
The county-level ratio of population to primary care physicians ranges from 0 to 46,784. 6.8 percent of observations do not have a primary care physician.
...
There are a similar number of missing ratios in each year. These are likely counties that we assigned to missing because their population was greater than 2000 but they had 0 reported primary care physicians. There are also 2 counties in 2020 (from Alaska) and 9 in 2021 (from Connecticut) that are missing due to geography changes.
```{r check-missings}
joined_data |>
  group_by(year) |>
  summarize(
...
```{r load-chr}
# Download file directly from internet if haven't already
if (
  !file.exists(here::here("17_access-to-health-services", "data", "chr2023.xlsx"))
){
  download.file(
  ...
  destfile = here::here("17_access-to-health-services", "data", "chr2023.xlsx"),
  mode = "wb"
)
}
...
chr2023 <- readxl::read_excel(here::here("17_access-to-health-services", "data", "chr2023.xlsx"),
...
County Health Rankings reports a missing value for counties with population greater than 2,000 and 0 primary care physicians. There are 147 of these observations in 2020. We remove these observations for this exercise.
...
Two counties from Missouri (Jasper and Newton Counties) are missing PCP counts from CHR but not from our data. These counties do not seem to be disbanded or otherwise unreliable. CHR does not have a count of primary care physicians for these counties, but does have a ratio. We remove these counties for this exercise.
```{r check-missouri}
...
# Download the Vintage 2020 population estimates directly from internet if haven't already
if (
  !file.exists(here::here("17_access-to-health-services", "data", "chr_pop.csv"))
){
  
  download.file(
    url = "https://www2.census.gov/programs-surveys/popest/datasets/2010-2020/counties/totals/co-est2020.csv",
    destfile = here::here("17_access-to-health-services", "data", "chr_pop.csv"),
    mode = "wb"
  )
...
chr_pop <- read_csv(here::here("17_access-to-health-services", "data", "chr_pop.csv"),
...
## 7. Add data quality flags
This section assigns a data quality flag based on the ratio's sensitivity to an extra primary care physician in the county.

We calculate the sensitivity of the ratio as measured by percent change in the ratio when adding 1 PCP to each county. We then assign poor quality to counties with high sensitivity values. E.g., if there are 800 people in a county with 1 physician, the ratio value is 800. The addition of 1 more physician changes the ratio drastically to a value of 400.
...
joined_data <- joined_data |>
  mutate(
    ...
    sensitivity = ifelse(
      !is.na(ratio_population_pc_physician),
...
In constructing the ratios, we applied the following rules:

* If population <= 2000 and 0 PCPs are reported, we assigned a value of `0` and assumed that these are "true zeros"
* If population >  2000 and 0 PCPs are reported, we assigned a value of `NA` and assumed that these are "false zeros"

The counties that we are considering false zeros are assigned a `sensitivity` value of `NA` and will therefore receive a `quality` value of `NA`. However, counties that we consider true zeros get assigned a `sensitivity` value of `Inf` because the sensitivity calculation attempts to divide by zero.

```{r check-sensitivity-values}
joined_data |>
  filter(pc_physicians == 0) |>
  count(sensitivity, ratio_population_pc_physician)
  
```

We replace the `Inf` values of `sensitivity` with the maximum defined value of `sensitivity`. This will also ensure that these observations are assigned a low quality value in the next step, which is an accurate assignment because we cannot say with certainty that these observations are "true" zeros. A county with 900 people and 0 primary care physicians has a ratio value of zero, however it may be the case that this county has 1 PCP and the true ratio is 900:1. We want the quality value to reflect this possibility.
```{r}
max_sensitivity <- joined_data |>
  filter(sensitivity != Inf) |>
  pull(sensitivity) |>
  max()

joined_data <- joined_data |>
  mutate(sensitivity = if_else(sensitivity==Inf, max_sensitivity, sensitivity))
...
To measure the data quality, we've added 1 to the number of primary care physicians in each county and re-created the ratio of population per primary care physician:

* If the ratio changes less than 20 percent we assign a quality value of 1.
* If the ratio changes between 20 percent and 39 percent we assign a quality value of 2.
* If the ratio changes between 40 percent and 59 percent we assign a quality value of 3.
...
If the `ratio` value is `NA`, the `quality` value is also `NA`.
```{r}
stopifnot(!all(is.na(joined_data$ratio_population_pc_physician) &
               is.na(joined_data$ratio_population_pc_physician_quality)))
```
...
```{r}
evaluate_final_data(exp_form_path = here::here("10a_final-evaluation", "evaluation_form_ratio_population_pc_physician_overall_county.csv"), data = final_data, geography = "county", subgroups = FALSE, confidence_intervals = FALSE)
```
