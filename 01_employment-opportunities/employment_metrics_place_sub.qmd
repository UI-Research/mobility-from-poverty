...
ACS Code: Initially written by Tina Chelidze, updated by JP Walsh & Aaron Williams 2022-2023 and again by JP Walsh in 2024-2025.
...
source(here::here("01_employment-opportunities", "R", "finalize_metric.R"))
source(here::here("01_employment-opportunities", "R", "calc_survey_mean.R"))
source(here::here("01_employment-opportunities", "R", "calc_survey_mean_subgroup.R"))
...
repwts <- ipums_repwt_employment_aws(
  ...
  select(-cbserial, -cbpernum, -multyear)
...
```{r}
acs_clean <- acs_clean %>% 
  mutate(crosswalk_period = case_when(multyear < 2012 ~ "2k",
                                      multyear >= 2012 & multyear < 2022 ~ "2012",
                                      multyear >= 2022 ~ "2022"))
```
...
```{r}
count(acs_clean, crosswalk_period, year, multyear)
...
Update the crosswalk period so that it is uniform for 2014 subgroup data.

```{r}
acs_crosswalked <- acs_crosswalked %>% 
  mutate(crosswalk_period = ifelse(crosswalk_period %in% c("2k"), "2012", crosswalk_period))
```
...
Calc_survey_mean_subgroup takes the acs data and transitions it into a srvyr object. This allows the program to apply survey_mean which is a powerful function that calculates precise confidence intervals from datasets with replicate weights.  
```{r}
...
if(isTRUE(bucket_exists(bucket = my_bucket))){
    ...
    put_object(tmp, object = paste0(s3_dir, "/metrics_employment_sub", "_", params$year, ".csv"), bucket = my_bucket, 
               show_progress = FALSE, multipart = FALSE)
    
}else{write_csv(paste0(here::here("01_employment-opportunities", "data", "intermediate"), 
                  "/", "metrics_employment_sub_", params$year, ".csv"))
  }
...
metrics_employment_sub %>% 
  ...
  theme_minimal() +
  ggtitle(paste0("Distribution of share employed, by place", params$year, "(5-year data)")) +
  ylab("Density")
...
metrics_employment_sub %>% 
  ...
  facet_wrap(~subgroup) +
  labs(
    title = paste0("Distribution of share employed, race/ethnicity, by place", params$year, "(5-year data)"),
    y = "Density"
  ) +
  theme_minimal()
...
metrics_employment_sub %>% 
  ...
  facet_wrap(~subgroup) +
  labs(
    title = paste0("Distribution of share employed, gender, by place", params$year, "(5-year data)"),
    y = "Density"
  ) +
  theme_minimal()
```
...
metrics_employment_sub %>% 
  ...
  facet_wrap(~subgroup) +
  labs(
    title = paste0("Distribution of share employed, disability, by place", params$year, "(5-year data)"),
    y = "Density"
  ) +
  theme_minimal()
...
evaluate_final_data(data = metrics_employment_race, exp_form_path = here::here("99a_final-evaluation", "evaluation_form_employment_race_eth_place.csv"), geography = "place", subgroups = TRUE, confidence_intervals = TRUE)
...
evaluate_final_data(data = metrics_employment_gender, exp_form_path = here::here("99a_final-evaluation", "evaluation_form_employment_gender_place.csv"), geography = "place", subgroups = TRUE, confidence_intervals = TRUE)
```
...
evaluate_final_data(data = metrics_employment_disability, exp_form_path = here::here("99a_final-evaluation", "evaluation_form_employment_disability_place.csv"), geography = "place", subgroups = TRUE, confidence_intervals = TRUE)
...
write_csv(metrics_employment_race, here::here("01_employment-opportunities", "data", "final", "metrics_employment_place_race-ethnicity_longitudinal.csv"))

write_csv(metrics_employment_gender, here::here("01_employment-opportunities", "data", "final", "metrics_employment_place_gender_longitudinal.csv"))

write_csv(metrics_employment_disability, here::here("01_employment-opportunities", "data", "final", "metrics_employment_place_disability_longitudinal.csv"))
```
