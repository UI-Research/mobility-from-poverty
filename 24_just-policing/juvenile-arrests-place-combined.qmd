...
This file creates the place-level metric for juvenile arrests per 100,000 juveniles (age 10-17) for years 2021-2024.
...
We use criminologist Jacob Kaplan's pre-processed NIBRS data available on [Harvard Dataverse](https://dataverse.harvard.edu/dataverse/ucrdata) and [ICPSR](https://www.openicpsr.org/openicpsr/project/118281) instead of raw data downloaded directly from the FBI. As of April 2026, the 2024 data was not yet on Harvard Dataverse, so all years (2021-2024) were downloaded from [Jacob Kaplan's ICPSR repository](https://www.openicpsr.org/openicpsr/project/118281) (V11, released August 2025). Instructions for downloading the data are included in the next section.
...
county_place_xw <- county_place_xw |>
  mutate(
    ...
    county_fips_3 = substr(countyfip, 3, 5),
  ...
  distinct(state_fips, county_fips, place, GEOID)
...
### 5.1c Clean ACS data and create subgroups
...
```{r}
evaluate_final_data(
  exp_form_path = "99a_final-evaluation/evaluation_form_rate_arrests_overall_place.csv",
...
acs_pops_subgroup <- acs_pops |>
  ...
  mutate(state = str_sub(GEOID, start = 1, end = 2), place = str_sub(GEOID, start = 3, end = 7)) |>
  select(year, state, place, subgroup, pop)
...
evaluate_final_data(
  exp_form_path = "99a_final-evaluation/evaluation_form_rate_arrests_race-ethnicity_place.csv",
...
evaluate_final_data(
  exp_form_path = "99a_final-evaluation/evaluation_form_rate_arrests_gender_place.csv",
...
evaluate_final_data(
  exp_form_path = "99a_final-evaluation/evaluation_form_rate_arrests_age_place.csv",
