...
This metric represents the county-level rate of juvenile arrests per 100,000 juveniles (age 10-17) for years 2021-2024.

The rest of this program is organized as follows:

0. Background and changes
1. Manually download NIBRS arrest data
2. Clean and append NIBRS arrest data
3. Aggregate arrests from individual to agency-level
4. Aggregate arrests from agency to county-level
5. Calculate metric
...
All data used for the creation of metrics in this program are available on Box [here](https://urbanorg.box.com/s/kaltrcovowfvzh1tnrrw073ijsuubpdi). Data stored on Box are only available to Urban Institute researchers.
...
## 0.2 Changes from previous round of updates (2026)
+ Added the newest year of data, 2024.
+ The 2024 Arrestee Segment and Group B Arrest Report Segment data were not yet available on Harvard Dataverse as of April 2026. All years (2021-2024) were downloaded from [Jacob Kaplan's ICPSR repository](https://www.openicpsr.org/openicpsr/project/118281) (V11, released August 2025). The per-year `.rds` files have the same structure and column names as the Harvard Dataverse files. All years were re-run with the latest data release, as agencies backfill prior years.
+ Updated the Batch Header file to `nibrs_batch_header_1991_2024.rds` from ICPSR.
+ Fixed `core_city` indicator matching to handle lowercase `"y"`/`"n"` values in ICPSR V11 (previously only matched uppercase `"Y"`/`"N"`).
+ Normalized batch header state name and abbreviation casing (ICPSR V11 uses lowercase; previous version used title case).

## 0.3 Changes from round prior to that (2025)
...
Accessing these data requires completing a license/data use agreement. Therefore, we do not directly download the data in this program.

For data years 2021-2023, both segments are available on Harvard Dataverse as individual per-year `.rds` files: [Arrestee Segment](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/ZULQVE) and [Group B Arrest Report Segment](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/CIQIVF). As of April 2026, 2024 data was not yet on Harvard Dataverse.

For the 2026 metric update, all years (2021-2024) were downloaded from [Jacob Kaplan's ICPSR repository](https://www.openicpsr.org/openicpsr/project/118281) (V11, released August 2025). The download process:

## 1.1 Arrestee Segment
1. Navigate to [Kaplan's NIBRS data on ICPSR](https://www.openicpsr.org/openicpsr/project/118281).
2. Download `arrestee_segment_rds_1991_2024.zip`.
3. Complete the license/data use agreement.
4. Unzip the file. It contains individual per-year `.rds` files with the same naming convention as Harvard Dataverse (e.g., `nibrs_arrestee_segment_YYYY.rds`).
5. Move the files for 2021-2024 to the `mobility-from-poverty/24_just-policing/data/arrestee-segment` directory.

## 1.2 Group B Arrest Report Segment
1. Navigate to [Kaplan's NIBRS data on ICPSR](https://www.openicpsr.org/openicpsr/project/118281).
2. Download `group_b_arrest_report_segment_rds_1991_2024.zip`.
3. Complete the license/data use agreement.
4. Unzip the file. It contains individual per-year `.rds` files (e.g., `nibrs_group_b_arrest_report_segment_YYYY.rds`).
5. Move the files for 2021-2024 to the `mobility-from-poverty/24_just-policing/data/group-b-arrest-report-segment` directory.

# 2. Read in, clean, and append both NIBRS arrest segments
This section reads in, cleans, and appends the NIBRS Arrest Segment and Group B Arrest Report Segment data for 2021-2024.

## 2.1 Read and clean files for 2021-2024 from the Arrestee Segment
```{r read-arrestee}
# List all files in the Arrestee Segment folder (Group A)
file_list <- list.files(here::here(
  "24_just-policing", "data", "arrestee-segment"),
...
## 2.2 Read and clean files for 2021-2024 from the Group B Arrest Report Segment
```{r read-groupb}
# List all files in the Arrest Report Segment folder (Group B)
file_list <- list.files(here::here(
  "24_just-policing", "data", "group-b-arrest-report-segment"),
  pattern = "[.]rds$", full.names = TRUE)
...
# 4. Aggregate arrests to county-level
...
The Batch Header Segment is available on Harvard Dataverse [here](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/HUI9WF) and on [ICPSR](https://www.openicpsr.org/openicpsr/project/118281).

For the 2026 metric update, we used the ICPSR version (`batch_header_rds_1991_2024.zip` from V11) which includes 2024. Download and unzip, then move `nibrs_batch_header_1991_2024.rds` to the `mobility-from-poverty/24_just-policing/data/batch-header-segment-and-agencies` directory.

## 4.2 Load and clean NIBRS Batch Header Files
Load Batch Header File
```{r load-bhf, message=FALSE}
batch_header_file_full <- readRDS(here::here("24_just-policing", "data", "batch-header-segment-and-agencies", "nibrs_batch_header_1991_2024.rds")) |>
  # ICPSR V11 uses lowercase state names and abbreviations; normalize to title case
  mutate(
    state = str_to_title(state),
    state_abbreviation = str_to_upper(state_abbreviation)
  )
...
batch_header_file <- batch_header_file |>
  mutate(
    core_city = case_when(
      core_city %in% c("Y", "y") ~ as.numeric(1),
      core_city %in% c("N", "n") ~ as.numeric(0),
      TRUE ~ NA_real_
    )
  )
...
Accessing these data requires an ICPSR account. Manually downloading these data is a five-step process, but you will only need to do each of these steps once for the creation of this metric:
  ...
  5. This will download a zip file titled `ICPSR_35158-V2`. Unzip the file and navigate to `ICPSR_35158\DS0001\35158-0001-Data.tsv`. Move this file to the `mobility-from-poverty\24_just-policing\data\batch-header-segment-and-agencies` directory after you have cloned the repository for this project from GitHub.
...
```{r load-leaic}
# Read in crosswalk
leaic <- read_tsv(here::here("24_just-policing", "data", "batch-header-segment-and-agencies", "35158-0001-Data.tsv"))
...
```{r replace-counties}
batch_header_file <- batch_header_file |>
  # Drop the NIBRS agencies with a missing county value
  filter(!is.na(county_fips_1)) |>
  # Append those agencies back onto main data with the county info gleaned from LEAIC
...
variables_ly <- tidycensus::load_variables(2024, "acs5")
...
First create vectors for years and variables of interest, which are county-level population totals for the entire county and for our subgroups.
```{r specify-acs-years-vars}
# Create a list of all our years
years <- lst(2021, 2022, 2023, 2024)
...
### 4.4c Clean ACS data and create subgroups
...
batch_header_file_long <- batch_header_file |>
  ...
  # Some agencies only have jurisdiction in a single county, and their values for `county_fips_2`, `county_fips_3`, etc. are missing. These observations are still reshaped above (i.e., each agency observation becomes five agency-county observations), but we only want to keep actual agency-county pairs, so we drop the observations that are missing county.
  drop_na(county_fips) |>
  select(-c(name))
...
### 4.4f Join arrest counts onto the weight file and multiply arrest variables by population weight
Join arrests onto the full universe of county-agencies and multiply counts by the agency-county weight.

We expect a lot of unmatched observations from the `agency_weights` file, because this is derived from the Batch Header Segment. Agencies may be included in the Batch Header Segment if they submitted any information to NIBRS across all segments, but may not have provided information on arrests.

I'm not sure why there are unmatched observations in the `arrests_juv_agency` file. Theoretically, because an agency included in either arrest segment is by definition included in the Batch Header Segment, all of these observations should match onto the Batch Header Segment.
```{r}
# Join agency-level juvenile arrests onto universe of agencies
arrests_juv_agency_county <- left_join(
  x = agency_weights,
  y = arrests_juv_agency, 
  by = c("ori", "year")
) |>
# Multiply counts by agency-county weight
  mutate(across(starts_with("arr"), ~.x * weight))

```



### 4.4g Finish aggregating from agency-level to county-level
...
arrests_juv_county <- arrests_juv_agency_county |>
  group_by(year, GEOID) |>
...
arrest_rates_juv_county <- arrests_pop_county |>
  mutate(
    # Total ages 10-17
...
arrest_rates_juv_county <- arrest_rates_juv_county %>%
  mutate(
...
I'm not sure why there are unmatched observations in the `arrests_all_agency` file. Theoretically, because an agency included in either arrest segment is by definition included in the Batch Header Segment, all of these observations should match onto the Batch Header Segment.
```{r}
...
Weight total arrests and indicate reporting by county-agency pairs.
```{r}
arrests_all_agency_county <- arrests_all_agency_county |>
  mutate(
    # Weight total arrest by county weight
    arr_total_any_wt = arr_total_allage * weight,
    # Indicate reporting by county-agency
    reporting = ifelse(is.na(arr_total_allage), 0, 1)
  )
...
arrests_all_county <- arrests_all_agency_county %>%
  group_by(year, state_fips, county_fips) %>%
  summarize(
    n = n(), 
    ...
    across(arr_total_allage, ~sum(.x, na.rm=TRUE))
  ) %>%
  mutate(
  ...
  ungroup() |>
  select(year, state_fips, county_fips, agencies_reporting, core_reporting)
...
```{r}
arrests_all_county <- arrests_all_county |>
    mutate(GEOID = str_c(state_fips, county_fips),
           year = as.character(year))

# Join all arrests and juvenile arrests
arrest_rates_county <- left_join(
  x = arrest_rates_juv_county, # juvenile arrests
...
Check the distribution of rates of juvenile arrests for 2021-2022, both overall and by subgroup. All rates are plausible. Rates are of arrests, not persons arrested, so values greater than 100,000 are plausible and indicates some people in that population group were arrested more than once.
...
arrest_rates_county_all <- arrest_rates_county |>
  select(c(year, GEOID, starts_with("rate"))) |>
  mutate(
...
rate_juv_arrest_county_all <- arrest_rates_county_all |>
  select(
    year,
    state,
    county,
...
```{r}
evaluate_final_data(
  exp_form_path = "99a_final-evaluation/evaluation_form_rate_arrests_overall_county.csv",
  data = rate_juv_arrest_county_all,  
  geography = "county",
  subgroups = FALSE, 
...
rate_juv_arrest_county_all |>
  write_csv(here::here("24_just-policing",
                      "data",
                       "final",
                       "rate_arrests_county_all.csv"))
...
juv_arrest_all_subgroup <- arrest_rates_county_all |>
  ...
  select(
    ...
    quality = rate_juv_arrest_quality
  ) |>
  ...
  rename(rate_juv_arrest_quality = quality)

acs_pops_subgroup <- acs_pops |>
  mutate(pop_1017_all = pop_1017) |>
  pivot_longer(cols = contains("pop_1017_"), 
               names_to = "subgroup",
               values_to = "pop",
               values_drop_na = TRUE,
               names_pattern = "pop_1017_(.*)") |>
  select(year, state = state_fips, county = county_fips, subgroup, pop)
...
juv_arrest_county_all_subgroup <- left_join(
  ...
  mutate(
    ...
    rate_juv_arrest_quality = ifelse(pop < 30, NA, rate_juv_arrest_quality))

juv_arrest_county_all_subgroup <- juv_arrest_county_all_subgroup |>
  select(
    year,
    state,
    county,
...
```
```{r}
juv_arrest_county_all_raceth <- juv_arrest_county_all_subgroup |>
  filter(subgroup_type %in% c("all", "race-ethnicity"))
  
evaluate_final_data(
  exp_form_path = "99a_final-evaluation/evaluation_form_rate_arrests_race-ethnicity_county.csv",
  data = juv_arrest_county_all_raceth,  
  geography = "county",
  subgroups = TRUE, 
  confidence_intervals = FALSE
  )

juv_arrest_county_all_gender <- juv_arrest_county_all_subgroup |>
  filter(subgroup_type %in% c("all", "gender"))

evaluate_final_data(
  exp_form_path = "99a_final-evaluation/evaluation_form_rate_arrests_gender_county.csv",
  data = juv_arrest_county_all_gender,  
  geography = "county",
  subgroups = TRUE, 
  confidence_intervals = FALSE
  )

juv_arrest_county_all_age <- juv_arrest_county_all_subgroup |>
  filter(subgroup_type %in% c("all", "age"))

evaluate_final_data(
  exp_form_path = "99a_final-evaluation/evaluation_form_rate_arrests_age_county.csv",
  data = juv_arrest_county_all_age,  
  geography = "county",
  subgroups = TRUE, 
  confidence_intervals = FALSE
  )
...
juv_arrest_county_all_subgroup |>
  write_csv(here::here("24_just-policing",
                      "data",
                       "final",
                       "rate_arrests_county_all_subgroup.csv"))
