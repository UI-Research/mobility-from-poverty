...
```{r}
read_csv(here("01_financial-well-being", "share_debt_2018_long.csv"), guess_max = Inf) %>%
  select(-county_name) %>%
  ...
  write_csv(here("data", "temp", "share_debt_race-ethnicity.csv"))

read_csv(here("06_neighborhoods", "transportation", "output", "county_transport_stats_by_race_final.csv"))  %>%
  mutate(subgroup_type = if_else(subgroup == "All", "all", subgroup_type)) %>%
  write_csv(here("data", "temp", "county_transport_stats_by_race_final.csv"))
...
files <- c(
  here("data", "temp", "share_debt_race-ethnicity.csv"),
  here("data", "temp", "county_transport_stats_by_race_final.csv"),
...
db <- db %>%
  rename(
    transportation_cost = transit_cost,
    transportation_cost_quality = transit_cost_quality,
    index_transit_trips = transit_trips,
    index_transit_trips_quality = transit_trips_quality,
    share_debt_col = share_debt_coll,
    share_debt_col_ub = share_debt_coll_ub,
    share_debt_col_lb = share_debt_coll_lb,
    share_debt_col_quality = share_debt_coll_quality,
    air_quality_index = environmental,
    air_quality_index_quality = environmental_quality
  )
...
read_csv_recent <- function(file) {
  ...
  file <- read_csv(file, guess_max = Inf)  %>%
    mutate(state = stringr::str_pad(state, width = 2, pad = "0"),
           county = stringr::str_pad(county, width = 3, pad = "0")) %>%
    mutate(
      subgroup = case_when(
...
db_recent <- tidylog::full_join(frame_recent, db_recent, by = c("state", "county", "subgroup_type", "subgroup"))

stopifnot(nrow(frame_recent) == nrow(db_recent))
...
db_values %>%
  ...
  assert(within_bounds(0, 100), transportation_cost, index_transit_trips, air_quality_index)
...
```{r}
 db %>%
  verify(is.na(share_debt_col) | share_debt_col_lb < share_debt_col) %>%
  verify(is.na(share_debt_col) | share_debt_col_lb < share_debt_col_ub) %>%
  verify(is.na(share_debt_col) | share_debt_col < share_debt_col_ub)
...
```{r}
write_csv(db, here("data", "03_mobility-metrics_race-share_longitudinal.csv"))
