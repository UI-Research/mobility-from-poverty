...
metric_qual_crosswalk <- as_tibble(
  metrics |> 
  select((where(is.numeric) & !matches('quality')) | index_air_hazard_quality) |> # why is air quality singled out here
  colnames()
) |> 
  rename(name = value) |> 
...
metrics <- left_join(metrics, population, by = c("state", "county", "year")) # there is already population in the metrics-- does this need to happen?

sum(is.na(metrics$population.y)) # 16 missing when use data for Y but 3142 missing for X
metrics <- metrics |> filter(!is.na(population.y)) # throws an error-- assess if this can be eliminated?; error because there are two population variables, fixed with code update 
```
...
weighted_median <- metrics |>
  select(where(is.numeric) & (!ends_with('quality') | index_air_hazard_quality)) |> 
  summarize(
    across(
      .cols = -population.y, 
      .fns = ~safe_wtd_quantile(., weights = population.y, probs = 0.5, na.rm = TRUE)
    ), .by = 'year'
  ) |>
...
state_count <- metrics |> 
    mutate(across(ends_with("quality"), ~case_when(. == "Strong" ~ 1,
                                                 . == "Marginal" ~ 2,
                                                 . == "Weak" ~ 3))) |>
  pivot_longer(cols = ends_with('_quality') & -index_air_hazard_quality, names_to = 'qual_varname', values_to = 'quality') |> 
...
missing_population <- metrics |>
  ...
  summarize(
    across(
      .cols = -population.y, 
      .fns = ~missing_share(.x, weight = population.y)
    ), .by='year') |> 
...
low_pctle <- metrics |>
  ...
  summarize(
    across(
      .cols = -population.y, 
      .fns = ~safe_wtd_quantile(., weights = population.y, probs = 0.05, na.rm = TRUE)
    ), .by = 'year'
  ) |>
...
high_pctle <- metrics |>
  ...
  summarize(
    across(
      .cols = -population.y, 
      .fns = ~safe_wtd_quantile(., weights = population.y, probs = 0.95, na.rm = TRUE)
    ), .by = 'year'
  ) |>
