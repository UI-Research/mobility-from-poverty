...
`share_affordable_available_30_ami` and `share_affordable_available_50_ami` are incorrectly calculated, so we drop them from the medians file.

```{r}
weighted_med_table |>
  mutate(
    share_affordable_available_30_ami = NA,
    share_affordable_available_50_ami = NA
  ) |>
