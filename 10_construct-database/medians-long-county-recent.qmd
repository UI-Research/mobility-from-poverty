...
** update this with suppression vars for each region
...
metrics_recent_suppressed <- metrics_long_recent |> 
  mutate(weighted_median = ifelse(share_lq3 > 0.15, NA, weighted_median),
         ...
         northeast_median = ifelse(share_lq3 > 0.15, NA, weighted_median_northeast),
         midwest_median = ifelse(share_lq3 > 0.15, NA, weighted_median_midwest),
         south_median = ifelse(share_lq3 > 0.15, NA, weighted_median_south),
         west_median = ifelse(share_lq3 > 0.15, NA, weighted_median_west)) |> 
  filter(!name == c("black", "black_nh", "white", "white_nh", "other", "hispanic", "all")) |> # arg why wont this filter work??
