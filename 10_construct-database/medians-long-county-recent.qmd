...
Load RUCS codes (needed to create urban and rural breakdowns)
...
# merge datasets together -- only 61 rows unaligned in 2023 RUCS codes, mostly in CT (because of updated FIPS codes, so we need the 2013 codes for these counties)
metrics_rucs <- left_join(x = metrics,
                          y = rucs_codes_23,
                          by = "fips")

# expect 61 matches-- and this is what we get!
metrics_rucs <- left_join(x = metrics_rucs,
                          y = rucs_codes_13,
                          by = "fips")

# now, make one harmonized RUCS variable
metrics <- metrics_rucs |>
...
weighted_rural <- rural |>
  ...
  pivot_longer(cols = -year, values_to = "weighted_median_rural")

weighted_urban <- urban |>
  select(where(is.numeric) & (!ends_with('quality'))) |> 
  summarize(
    across(
      .cols = -population, 
  ...
  pivot_longer(cols = -year, values_to = "weighted_median_urban")
...
weighted_med_table <- weighted_median |> 
  ...
  left_join(weighted_rural, by = c("year", "name")) |>
  left_join(weighted_urban, by = c("year", "name")) |> 
...
qual_rural <- stratification_suppression(data = rural,
                                         qual_var = share_lq3_rural)

qual_urban <- stratification_suppression(data = urban,
...
weighted_med_table_supp <- weighted_med_table |> 
  ...
  full_join(qual_rural, by = c("year", "name")) |>
  full_join(qual_urban, by = c("year", "name")) 
...
metrics_recent_suppressed <- metrics_long_recent |> 
  mutate(weighted_median = ifelse(share_lq3 > 0.15, NA, weighted_median),
         ...
         rural_median = ifelse(share_lq3_rural > 0.15, NA, weighted_median_rural),
         urban_median = ifelse(share_lq3_urban > 0.15, NA, weighted_median_urban)) |> 
  filter(!is.na(pillar)) |> # arg why wont this filter work??
  select(pillar, name, year, weighted_median, low_pctle, high_pctle, black_median, hispanic_median, white_median, northeast_median, midwest_median, south_median, west_median, rural_median, urban_median, share_lq3) 
...
Now, we count the number of metrics each county has in the top percentile, for purposes of the national medians brief.

```{r}

# Select the variables needed to count
# 13 metrics total
metrics_county_ct <- metrics |>
  select(index_air_hazard, rate_learning, share_in_preschool, share_hs_degree, share_digital_access, share_affordable_available_30_ami, count_membership_associations_per_10k, ratio_high_low_ses_fb_friends, share_election_turnout, share_employed, ratio_living_wage, index_transit_trips, index_transportation_cost, state, county, population, year)

# need to do the "flipped" vars separately -- 4 total
metrics_county_ct_flip <- metrics |>
  select(rate_injury_death, ratio_population_pc_physician, share_poverty_exposure, share_debt_coll, state, county, population, year)
...
metrics_county_ct_long <- metrics_county_ct %>%
  pivot_longer(cols = -c("state", "county", "population", "year"), names_to = "variable", values_to = "value")

# Identify the most recent year for each variable
recent_year_data_2 <- recent_year_data |>
  rename(variable = name) 

# Filter the dataset to only include the most recent year of data for each variable
metrics_county_ct_recent <- metrics_county_ct_long %>%
  semi_join(recent_year_data_2, by = c("variable", "year" = "recent_year"))
...
# Reshape the data to a long format
metrics_county_ct_long_flip <- metrics_county_ct_flip %>%
  pivot_longer(cols = -c("state", "county", "population", "year"), names_to = "variable", values_to = "value")

# Filter the dataset to only include the most recent year of data for each variable
metrics_county_ct_recent_flip <- metrics_county_ct_long_flip %>%
  semi_join(recent_year_data_2, by = c("variable", "year" = "recent_year"))

```

Create a function to implement

```{r}

# Function to calculate population-weighted percentile
weighted_percentile <- function(x, w, p) {
  df <- tibble(x = x, w = w) %>%
    arrange(x) %>%
    mutate(cum_weight = cumsum(w) / sum(w))
...
weighted_percentile_less <- function(x, w, p) {
  df <- tibble(x = x, w = w) %>%
    arrange(x) %>%
    mutate(cum_weight = cumsum(w) / sum(w))
...
First, the 5th percentile (WORST COUNTIES, HERE ALSO DO THE FLIP)

```{r}

# Calculate the weighted 5th percentile for each variable
percentiles_05 <- metrics_county_ct_recent %>%
  group_by(variable) %>%
  summarize(weighted_5th_percentile = weighted_percentile_less(value, population, p = 0.05))

percentiles_05_flip <- metrics_county_ct_recent_flip %>%
  group_by(variable) %>%
  summarize(weighted_5th_percentile = weighted_percentile(value, population, p = 0.95)) # need this to be 95 to capture the flipped
...
metrics_county_ct_recent_ptcle_join_05 <- metrics_county_ct_recent %>%
  left_join(percentiles_05, by = "variable")

# Count the number of variables above the weighted 5th percentile for each county
pctle_ct_05 <- metrics_county_ct_recent_ptcle_join_05 %>%
  group_by(state, county) %>%
  summarize(
    count_above_percentile = sum(value > weighted_5th_percentile, na.rm = TRUE)
  )

# summary
summary(pctle_ct_05$count_above_percentile)
...
# Join the percentiles back to the original data
metrics_county_ct_recent_ptcle_join_05_flip <- metrics_county_ct_recent_flip %>%
  left_join(percentiles_05_flip, by = "variable")

# Count the number of variables above the weighted 5th percentile for each county
pctle_ct_05_flip <- metrics_county_ct_recent_ptcle_join_05_flip %>%
  group_by(state, county) %>%
  summarize(
    count_above_percentile = sum(value > weighted_5th_percentile, na.rm = TRUE)
  )
...
# summary
summary(pctle_ct_05_flip$count_above_percentile)

```

Now, do the 95th percentile (the BEST COUNTIES)

```{r}

# Calculate the weighted 95th percentile for each variable
percentiles_95 <- metrics_county_ct_recent %>%
  group_by(variable) %>%
  summarize(weighted_95th_percentile = weighted_percentile(value, population, p = 0.95))

percentiles_95_flip <- metrics_county_ct_recent_flip %>%
  group_by(variable) %>%
  summarize(weighted_95th_percentile = weighted_percentile_less(value, population, p = 0.05))
...
# Join the percentiles back to the original data
metrics_county_ct_recent_ptcle_join_95 <- metrics_county_ct_recent %>%
  left_join(percentiles_95, by = "variable")

# Count the number of variables above the weighted 5th percentile for each county
pctle_ct_95 <- metrics_county_ct_recent_ptcle_join_95 %>%
  group_by(state, county) %>%
  summarize(
    count_above_percentile = sum(value > weighted_95th_percentile, na.rm = TRUE)
  )

# When filtering we see 211 counties have values in top 5th percentile for all 17 metrics
summary(pctle_ct_95$count_above_percentile)
...
# Join the percentiles back to the original data
metrics_county_ct_recent_ptcle_join_95_flip <- metrics_county_ct_recent_flip %>%
  left_join(percentiles_95_flip, by = "variable")

# Count the number of variables above the weighted 5th percentile for each county
pctle_ct_95_flip <- metrics_county_ct_recent_ptcle_join_95_flip %>%
  group_by(state, county) %>%
  summarize(
    count_above_percentile = sum(value > weighted_95th_percentile, na.rm = TRUE)
  )
...
# When filtering we see 211 counties have values in top 5th percentile for all 17 metrics
summary(pctle_ct_95_flip$count_above_percentile)
