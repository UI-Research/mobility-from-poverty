...
metrics_county_ct <- metrics |>
  select(index_air_hazard, rate_injury_death, ratio_population_pc_physician, rate_learning, share_in_preschool, share_hs_degree, share_digital_access, share_poverty_exposure, share_affordable_available_30_ami, count_membership_associations_per_10k, ratio_high_low_ses_fb_friends, share_election_turnout, share_employed, ratio_living_wage, index_transit_trips, index_transportation_cost, share_debt_coll, state, county, population, year)
...
most_recent_years <- metrics_county_ct_long %>%
  group_by(variable) %>%
...
metrics_county_ct_recent <- metrics_county_ct_long %>%
  inner_join(most_recent_years, by = c("variable", "year" = "most_recent_year"))
...
weighted_percentile <- function(x, w, p = 0.05) {
  df <- tibble(x = x, w = w) %>%
    arrange(x) %>%
...
percentiles <- metrics_county_ct_recent %>%
  group_by(variable) %>%
  summarize(weighted_5th_percentile = weighted_percentile(value, population))
...
metrics_county_ct_recent_ptcle_join <- metrics_county_ct_recent %>%
  left_join(percentiles, by = "variable")

# Count the number of variables above the weighted 5th percentile for each county
pctle_ct <- metrics_county_ct_recent_ptcle_join %>%
  group_by(county) %>%
  summarize(
...
# Calculate the weighted 5th percentile for each variable
percentiles <- metrics_county_ct %>%
  pivot_longer(cols = -c("state", "county", "population"), names_to = "variable", values_to = "value") %>%
  group_by(variable) %>%
  summarize(weighted_5th_percentile = weighted_percentile(value, population))

# Join the percentiles back to the original data and select most recent year data
metrics_county_ct_long <- metrics_county_ct %>%
  pivot_longer(cols = -c("state", "county", "population"), names_to = "variable", values_to = "value") # %>%
 # left_join(percentiles, by = "variable")

metrics_county_ct_long_recent <- metrics_county_ct_long %>%
  group_by(variable) %>%
  filter(year == max(year)) %>%
  ungroup()

# Count the number of variables above the weighted 5th percentile for each county
percentile_count <- metrics_county_ct_long %>%
