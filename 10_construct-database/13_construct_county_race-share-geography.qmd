...
bind_rows(
  ...
  mutate(share_debt_coll_quality = ifelse(is.na(share_debt_coll), NA_character_, share_debt_coll_quality))|>
...
read_csv(here("data", "temp", "county_debt_longitudinal_race-ethnicity.csv")) |>
  mutate(subgroup = case_when(subgroup == "Majority White" ~ "White communities",
                              subgroup == "No Majority Race/Ethnicity" ~ "Mixed communities", 
                              subgroup == "Majority Non-White" ~ "Communities of color",
                              TRUE ~ subgroup),
...
read_csv(here("06_neighborhoods/environment/data/final/environment_county_race-ethnicity_longitudinal.csv")) |>
  mutate(subgroup = case_when(subgroup == "Majority White, Non-Hispanic" ~ "White communities",
                              subgroup == "No Majority Race/Ethnicity" ~ "Mixed communities", 
                              subgroup == "Majority Non-White" ~ "Communities of color",
                              TRUE ~ subgroup)) |>
  write_csv(
...
read_csv(here("06_neighborhoods/Transportation/final/transit_trips_all_subgroups_county.csv")) |>
  mutate(subgroup = case_when(subgroup == "Majority Non-White Tracts" ~ "Communities of color", 
                              subgroup == "Majority White-NH Tracts" ~ "White communities",
                              subgroup == "Mixed Race and Ethnicity Tracts" ~ "Mixed communities",
                              TRUE ~ subgroup),
         subgroup_type = ifelse(subgroup == "All", "all", subgroup_type)) |>
  write_csv(
    here("data", "temp", "transit_trips_all_county_subgroups_race_share.csv")
  )


read_csv(here("06_neighborhoods/Transportation/final/transit_cost_all_subgroups_county.csv")) |>
  mutate(subgroup = case_when(subgroup == "Majority Non-White Tracts" ~ "Communities of color", 
                              subgroup == "Majority White-NH Tracts" ~ "White communities",
                              subgroup == "Mixed Race and Ethnicity Tracts" ~ "Mixed communities",
                              TRUE ~ subgroup),
         subgroup_type = ifelse(subgroup == "All", "all", subgroup_type)) |>
  write_csv(
    here("data", "temp", "transit_cost_all_county_subgroups_race_share.csv")
  )
...
expand_grid(
  read_csv(here("geographic-crosswalks", "data", "county-populations.csv")),
           subgroup = c("All", "Communities of color", "White communities", 
                        "Mixed communities")
) %>% 
