...
weighted_northeast <- northeast_01 |>
  select(!ends_with('quality'), -state_name, -county_name, -subgroup_type, -state, -county) |> 
  summarize(
    across(
      .cols = -population, 
...
northeast_01 |>
  write_csv(here("10_construct-database", "northeast_01.csv"))

```

```{r}

  weighted_median <- metrics_01_df |>
    mutate(county_state = paste0(county_name, ', ', state_name)) |> 
    select(county_state, subgroup, where(is.numeric) & (!ends_with('quality'))) |> 
    summarize(
      across(
