...
# old file extracts that are not needed
...
metrics_01_long <- metrics_01 |>
  mutate(across(ends_with("quality"), ~case_when(. == "Strong" ~ 1,
                                                 . == "Marginal" ~ 2,
                                                 . == "Weak" ~ 3))) |>
  pivot_longer(cols = pctl_income_20:share_employed_quality,
               names_to = "variable",
               values_to = "value")
...
# reshape back to wide for later functions and mutate quality vars

# mutate quality vars first-- also does not work
quality_vars <- metrics_01_long_recent |>
  filter(str_detect(variable, "quality$")) |>
  distinct(variable) |>
  pull(variable)

metrics_01_long_recent <- metrics_01_long_recent |>
  mutate(across(all_of(quality_vars), ~case_when(. == 1 ~ "Strong",
                                                  . == 2 ~ "Marginal",
                                                  . == 3 ~ "Weak")))

# pivot wider DOES work
metrics_01_recent <- metrics_01_long_recent |>
...
population <- map_dfr(c(2015:2022), ~get_estimates(
  geography = "county", 
...
-- NEEDS TO BE DE-BUGGED W/R/T POP ESTIMATES -- 
