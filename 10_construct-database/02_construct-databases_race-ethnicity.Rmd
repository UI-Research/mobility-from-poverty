...
wrong_files <- c(
  here("temp_data", "metrics_income_subgroup.csv"),
  here("temp_data", "metrics_famstruc_subgroup.csv"),
  here("temp_data", "metrics_college_subgroup.csv"),  
  here("temp_data", "metrics_preschool_subgroup.csv"),
  here("temp_data", "metrics_employment_subgroup.csv")
)

add_all <- function(all, subgroup) {
  
  # read all
  all_csv <- read_csv(all) %>%
    mutate(subgroup_type = "all",
           subgroup = "All")
  
  # read subgroup
  subgroup_csv <- read_csv(subgroup) %>%
...
files <- c(
  here("temp_data", "metrics_income_subgroup.csv"),
  here("temp_data", "metrics_famstruc_subgroup.csv"),
  here("04_health", "final_data", "neonatal_health_subgroup.csv"),
  here("06_neighborhoods", "poverty-exposure", "poverty-exposure_race-ethnicity.csv"), 
  here("08_education", "SEDA_race-ethnicity.csv"),
  here("temp_data", "metrics_preschool_subgroup.csv"),
  here("temp_data", "metrics_college_subgroup.csv"),
  here("temp_data", "metrics_employment_subgroup.csv")
)
