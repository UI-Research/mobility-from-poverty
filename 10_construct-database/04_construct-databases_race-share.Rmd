...
read_csv(here("01_financial-well-being", "share_debt_2018_long.csv")) %>%
  ...
  mutate(subgroup = 
           case_when(
             subgroup == "Majority white" ~ "Predominantly White",
             subgroup == "Majority non-white" ~ "Predominantly People of Color",
             TRUE ~ subgroup
           )
  ) %>%
...
frame <- 
  bind_rows(
    ...
    expand_grid(
      ...
      subgroup = c("No Predominant Racial Group", "Predominantly People of Color", "Predominantly White")
    )
  ) %>%
  arrange(year, state, county, subgroup_type, subgroup)
