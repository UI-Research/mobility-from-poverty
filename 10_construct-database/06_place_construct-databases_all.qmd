...
read_csv(here("01_financial-well-being", "data", "final", "metrics_income_place_all_longitudinal.csv")) %>%
  filter(year < 2022) %>%
  mutate(
    pctl_income_quality = pctl_20_quality
  ) %>%
  write_csv(here("data", "temp", "metrics_income_place_all_longitudinal.csv"))

read_csv(here("09_employment", "data", "final", "metrics_employment_place_all_longitudinal.csv")) %>%
  filter(year < 2022) %>%
  write_csv(here("data", "temp", "metrics_employment_place_all_longitudinal.csv"))  
...
files <- c(
  ...
  here("data", "temp", "metrics_income_place_all_longitudinal.csv"),
  here("data", "temp", 
       "city-debt-col-shares_CORRECT.csv"), 
  ...
  here("02_housing", "data", "housing_city_2021.csv"), 
  ...
  ### SAFETY ###
  here("07_safety", "modified data", "2021_juv_arrest_rate_place.csv"),
  here("07_safety", "modified data", "2021_crime_rate_place.csv"),

  ### WORK ###
  here("data", "temp", "metrics_employment_place_all_longitudinal.csv")
)
...
clean_data <- function(db_unclean){
  ...
  rename(
    ...
    share_affordable_30_ami = share_affordable_30AMI,
    share_affordable_50_ami = share_affordable_50AMI,
    share_affordable_80_ami = share_affordable_80AMI,
  ...
  #   ratio_average_to_living_wage_quality = wage_ratio_quality
    ...
    select(-c(rate_juv_arrest_violent, rate_juv_arrest_property))
  # 
...
share_cols <- share_cols[!(grepl("affordable", share_cols))]
...
db_values %>%
  ...
  ### HOUSING ###
  assert(within_bounds(0, 10), starts_with("share_affordable")) %>%
