...
desc_rep_CLEAN <- desc_rep %>%
  rename_with(.cols = asian_other:white_nonhispanic_quality, 
              ~str_c("descriptive_representation_", .x))

write_csv(desc_rep_CLEAN,
          here::here(
            "05_local-governance",
             "descriptive-representation",
             "CLEAN_descriptive-representation.csv"
          ))
...
db_values %>%
  ...
  #Add assert statements for the following variables as they come in:
  #Social capital- association membership
  #Social capital - SES FB friends
