...
neonatal <- rbind(neonatal_2018, neonatal_2020) %>%
  rename_with(~ gsub("lbw", "low_birth_weight", .x, fixed = TRUE))
...
```{r fix-income-names}
income <- read_csv(
  here::here("01_financial-well-being",
             "metrics_income.csv")
)

income_CORRECT <- income %>%
...
write_csv(
  income_CORRECT,
  here::here("01_financial-well-being", 
             "CORRECT_metrics_income.csv")
)

```

```{r fix-descriptive-representation-names}
desc_rep <- read_csv(
  here::here("05_local-governance",
             "descriptive-representation",
             "data", 
             "descriptive_rep_denominator_county_2022.csv"
             )
)

desc_rep_CLEAN <- desc_rep %>%
...
write_csv(desc_rep_CLEAN,
          here::here(
            "05_local-governance",
            "descriptive-representation",
            "data", 
            "descriptive_rep_denominator_county_2022_CLEAN.csv"
          )
          )

```

```{r rename-housing-wealth}
county_hh_hw <- read_csv(
  here::here("01_financial-well-being",
             "home-values", 
             "county_hh_hw_raceeth_2014_2021.csv")
)

county_hh_hw_clean <- county_hh_hw %>%
  rename_with(~gsub("hv_hh", "house_value_households", .x, fixed=TRUE))
write_csv(county_hh_hw_clean,
          here::here(
            "01_financial-well-being",
            "home-values",
            "county_hh_hw_reaceeth_2014_2021_CLEAN.csv"
          ))

```


```{r remove-se-social-capital-econ-connectedness}
econ_connect <- read_csv(
  here("06_neighborhoods", 
       "social-capital", 
       "data", 
       "economic_connectedness_county_2022.csv")
  )

econ_connect_clean <- econ_connect %>%
  select(-contains("_se"))

write_csv(econ_connect_clean, 
          here("06_neighborhoods", 
               "social-capital", 
               "data", 
               "economic_connectedness_county_2022_CLEAN.csv"
               )
          )
```
...
files <- c(
  ...
  here("01_financial-well-being", "CORRECT_metrics_income.csv"),
  here("01_financial-well-being", "share_debt_2018.csv"), 
  here("01_financial-well-being", "home-values", "county_hh_hw_reaceeth_2014_2021_CLEAN.csv"),
  
  ### HEALTH ### 
  #Missing: Ratio of population per PCP. Think it is associated with: 
  #here("04_health", "final_data", "hpsa_2020.csv"), #This file not on github
  ...
  here::here("05_local-governance", "descriptive-representation",
            "data", "descriptive_rep_denominator_county_2022_CLEAN.csv"), 
  ...
  #Missing: Environment
  here("06_neighborhoods", "social-capital", "data", "economic_connectedness_county_2022_CLEAN.csv"),
  ...
  here("09_employment", "metrics_wage_ratio_years.csv") #GM: I think this corresponds to the access to jobs paying a living wage but I am not sure
  
  
  
  
  #Removed following files because these are not in Metrics 2.0 tracker
...
db_values %>%
  ...
  assert(within_bounds(-1.5, 10), learning_rate) %>% #There are some values below -1, but these could be legit, so changed to -1.5
  ...
  #r_black_nh_hv_hh", "r_hispanic_hv_hh", "r_other_nh_hv_hh", "r_white_nh_hv_hh"
  #variables. These are ranges of form X%:Y% and thus hard to test 
  
  ### HEALTH ###
  #assert(in_set(0, 1), hpsa_yn) %>%
  assert(within_bounds(0, 1), low_birth_weight) %>%
  ...
  ### LOCAL GOVERNANCE ###
  assert(within_bounds(0, Inf), 
         descriptive_representation_asian_other,
         descriptive_representation_black_nonhispanic,
         descriptive_representation_hispanic,
         descriptive_representation_white_nonhispanic
         ) %>%
  assert(within_bounds(0, 1), election_turnout) %>%
    
  ### NEIGHBORHOODS ### 
  assert(within_bounds(0, 1), poverty_exposure) %>%
  assert(within_bounds(0, 1), ends_with("exposure")) %>%
  assert(within_bounds(0, 100), transit_cost, transit_trips) %>%
  assert(within_bounds(0, 100), social_connection) %>%
  assert(within_bounds(0, 1), economic_connectedness) %>%
