  ...
  db <- db_unclean %>%
    ...
    mutate(
      ...
      place_name = case_when(
        ...
        state == "32" & place == "71400" ~ "Temple Terrace",
        TRUE ~ place_name
      )
      )
...
xwalk <- read_csv(here("geographic-crosswalks", 
                     ...
                     guess_max = 5000) %>%
  filter(year == 2018) %>%
  select(-c(year, state_name, place_name))
...
#Frame shape = 3 race shares + all * 3 years * 486 places = 4 * 3 * 486 = 

db <- tidylog::full_join(db, frame, by = c("state", "place", "year", "subgroup_type", "subgroup"))
#We have three places not in the lookup table- they are the three identified above
