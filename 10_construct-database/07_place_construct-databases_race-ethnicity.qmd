...
```{r}
read_csv(here("09_employment", "data", "final", "metrics_employment_place_race-ethnicity_longitudinal.csv")) %>%
  filter(year < 2022) %>%
  write_csv(here("data", "temp", "metrics_employment_place_race-ethnicity_longitudinal.csv"))

read_csv(here("01_financial-well-being", "data", "final", "metrics_income_place_race-ethnicity_longitudinal.csv")) %>%
  filter(year < 2022) %>%
  mutate(pctl_income_quality = pctl_20_quality) %>%
  write_csv(here("data", "temp", "metrics_income_place_race-ethnicity_longitudinal.csv"))
...
files <- c(
  ...
  ### FINANCIAL WELLBEING ###
  here("data", "temp", "metrics_income_place_race-ethnicity_longitudinal.csv"),
  ...
  ### WORK ### 
  here("data", "temp", "metrics_employment_place_race-ethnicity_longitudinal.csv")
...
read_csv_and_clean <- function(file, is_recent = FALSE) {
  ...
  df <- df %>%
    ...
    select(-any_of(c("state_name", "place_name", "population"))) %>%
...
db_values %>%
  ...
  assert(within_bounds(0, 100000), starts_with("count_")) %>%
