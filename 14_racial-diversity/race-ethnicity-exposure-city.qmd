...
source(here::here("14_racial-diversity", "R", "get_vars.R"))
source(here::here("functions", "testing", "evaluate_final_data.R"))
# source(here::here("14_racial-diversity", "R", "census_api_key.R"))
```
...
#| echo: false
knitr::include_graphics(here::here("14_racial-diversity", "www", "images", "race-ethnicity.png"))
```
...
if(!file.exists(here::here("14_racial-diversity/data/tracts.csv"))){
  tracts_raw <- map(years, ~ pull_and_rename(.x)) |>
  list_rbind() |>
  relocate(year, .before = everything())
  
  
if(!dir.exists(here::here("14_racial-diversity/data"))){
  dir.create(here::here("14_racial-diversity/data"))
}
write_csv(tracts_raw, here::here("14_racial-diversity/data/tracts.csv"))
...
tracts_raw <- read_csv(here::here("14_racial-diversity/data/tracts.csv"))
```
...
We need to know which census places have any overlap with each census tract. We read in two census-tract-to-place crosswalks that were originally downloaded from the [Missouri Census Data Center's Geocorr tool](https://mcdc.missouri.edu/applications/geocorr2022.html). One crosswalk translates 2010-vintage tracts to 2014-vintage places (note that we use Geocorr 2018 for this crosswalk, originally), and the other translates 2020-vintage tracts to 2020-vintage places. We read these files in directly from the repository, but the process for creating them from scratch is as follows:
...
```{r}
tracts_joined <- tidylog::left_join(tracts, crosswalk, by = c("year", "state", "county", "tract")) %>%
  arrange(state_place)
```
...
We don't have data for South Fulton in 2018, but otherwise we have data for every
year-place combination:
```{r}
place_data |> 
  count(year) %>%
  rename(analysis_places = n) %>%
  left_join(
    places_of_interest %>%
      filter(year %in% years) |>
      count(year) %>%
      rename(target_places = n))
...
place_data %>%
  ...
  labs(
    title = "There is a negative relationship between a group's share and exposure",
...
There is very limited missingness.
...
place_data %>%
  ...
  write_csv(here::here("14_racial-diversity", "final", "race-ethnicity-exposure-city.csv"))
```
...
```{r}
final_data <- read_csv(here::here("14_racial-diversity", "final",
                                  "final", "race-ethnicity-exposure-city.csv"),
                       show_col_types = FALSE)

evaluate_final_data(
  exp_form_path = "99a_final-evaluation/evaluation_form_share_race_exposure_overall_place.csv",
