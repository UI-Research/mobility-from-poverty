...
This program creates the **Ratio of the share of a community’s housing
wealth held by a racial or ethnic group to the share of households of
the same group** metric
...
-   [Add Age Subgroup](#add-age-subgroup)
-   [Merge Crosswalk](#merge-crosswalk)
-   [Create Housing Wealth Metric](#create-employment-metric)
...
We extract the 1 year ACS files from the IPUMS API using `ipumsr`. You
will need to have an API key in order to do so. If you don't have one,
you can register to the [IPUMS
USA](https://uma.pop.umn.edu/usa/user/new?return_url=https%3A%2F%2Fusa.ipums.org%2Fusa-action%2Fmenu)
data collection and create an API key.
...
`set_ipums_api_key(api_key = "paste-your-key-here", save = TRUE)`. This
...
We load the 2014-2022 1 Year ACS microdata we downloaded from IPUMS
above. IPUMS loads the data as a person-level data set with some
household variables. For example `hhincome` shows total household income
with repeated values for each person in the same household. Therefore,
we can analyze the data at the household level by keeping the first
person sampled in the household (i.e., `pernum == 1` where `pernum` is
the "Person number in sample unit").
...
Next, we exclude individuals living in group quarters because we only
want households for this metric. In particular, we exclude people living
in places other than housing units (e.g., correctional facilities,
nursing homes, mental hospitals, dorms, barracks, group homes,
missions). For more information, see the [variable
description](https://usa.ipums.org/usa-action/variables/GQ#codes_section)
on IPUMS USA.
...
FIPS codes should always contain leading zeros so that state codes are
two digits, county codes are three digits, and place codes are five
digits. We can ensure this is the case with `stringr::str_pad()`.
...
For 2022 ACS and onward we use a crosswalk going from 2020 PUMA to 2020
Place. For pre-2022 ACS data, we use a crosswalk from 2012 PUMA to 2014
Place.

See #Crosswalks_README.md

```{r}
read_csv(here::here("geographic-crosswalks", "data", "crosswalk_puma_to_county.csv")) %>% View()
```

We merge our 1-year ACS data with PUMA-County and PUMA-Place geocodes to
get County and City boundaries. Counties with missing county codes in
the ACS (i.e., those for which `county code == 0`) were updated using
the PUMA-County crosswalk.

::: panel-tabset
#### County

Only the counties where ACS did not provide county codes (i.e.,
`county code == 0`) were updated using the PUMA-County crosswalk.

```{r}
#| label: puma-to-county-crosswalk

puma_county <- read_csv(here("geographic-crosswalks/data/geocorr2014_puma_to_county.csv")) %>%
  mutate(
...
For cities, only places with more than 50,000 population in 2020 were
selected.
...
Next, we adjust the household weights to account for the allocation from
PUMAs to counties using the puma to county allocation factor variable
(`afact`). In other words, `afact` indicates the portion of the PUMA
geocodes contained within the county geocode. Conversely, `afact2` is
the county to puma allocation factor. We use `afact` and `afact2` to
create a `sum_afact` variable, which is defined as the sum of
`afact * afact2`.
...
### Add Age Subgroup {#sec-add-age-subgroup}

Add Age subgroup (max 2 groups) - Jung Choi suggested age 45 as the
split, so something like \<age 45 and 45+

## Create Housing Wealth Metric

### Housing Share

We will now compute the share of households by race and ethnicity (i.e.,
total number of households for each race and ethnic group divided by the
...
We visualize the distribution of the share of households by race and
ethnicity
...
We keep observations for homeowners only in order to calculate total
aggregated housing wealth for homeowners for each race and ethnic group
divided by the total aggregated housing wealth
...
The metrics for the ACS indices are based on: - (1) sample size of each
race and ethnic household at the county/place and\
- (2) the number of observations that comes from the specific
county/place calculated from the PUMA-county/place crosswalk.

Therefore, the data quality is poor for counties/places with a extremely
small number of households (N). To assess the data quality of
counties/places with larger N, we need to consider what percent of data
for the county/place actually came from the county/place itself.

For this metric we say that: - Data quality is good (`1`) if N is
greater than 30 and more than 75% of observations are from the
county/place. - Data quality is marginal (`2`) N is greater than 30 but
only 35% to 75% of observations are from the county/place. - Data
quality is poor (`3`) if N is less than 30 **or** if less than 35% of
