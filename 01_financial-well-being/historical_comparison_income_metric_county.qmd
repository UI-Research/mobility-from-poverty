...
This program compares the results for the program updated by JP Walsh in 2023/24 with the results from the program originally written by Kevin in 2022 for the creation of the UMF income predictor. These processes should be the same and as a result the final metrics should be comparable.
...
counties_new <- read_csv(here("01_financial-well-being", "data", "final", "metrics_income_county_all_longitudinal.csv")) %>% 
  select(year, state, county, pctl_20, pctl_50, pctl_80) %>% 
  filter(year == 2018)
...
counties_new %>% 
  ...
  filter(!is.na(pctl_20)) %>% 
  mutate(diff_20 = pctl_20 - pctl_income_20,
         diff_50 = pctl_50 - pctl_income_50,
         diff_80 = pctl_80 - pctl_income_80)%>% 
  reactable::reactable()
