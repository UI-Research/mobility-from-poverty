...
The largest difference is significant at 30 thousand dollars (this is troubling). However, on average the difference is between 120 and 648 dollars. 
```{r}
counties_new_race %>% 
  left_join(counties_old_race, by = c("year", "state", "county", "subgroup")) %>% 
  filter(!is.na(pctl_income_20), year == 2018) %>% 
...
The largest difference is 15 thousand dollars. However, on average the difference between the results are very small (etween 18 and 26 dollars) and the median results are almost exactly the same. 
```{r}
counties_new_race %>% 
  left_join(counties_old_race, by = c("year", "state", "county", "subgroup")) %>% 
  filter(!is.na(pctl_income_20), year == 2021) %>% 
