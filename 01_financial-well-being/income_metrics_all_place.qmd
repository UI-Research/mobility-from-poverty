...
*Program for the creation of the income metric, non-subgroup, place level*

ACS Code: Initially written by Tina Chelidze and updated by JP Walsh 2022-2023.

Primary data is an IPUMS extract for 2021 (1-year ACS)
...
Calculate the total place population by summing pop20. Pop20 is the population of a PUMA that overlaps with the place (i.e. total population times AFACT).
...
We also want to know particularly small places, defined by the bottom percentile of population. This starts at 110,629.
...
Crate an indicator of data quality based on these results. Create an indicator based on sum_product result (1 to 3 with 1 being best). Also create an indicator of small places (1 is small).
...
if (file.exists(here::here("01_financial-well-being", "data", "raw", "income_1year.dat.gz")) == FALSE) 
...
Rename the IPUMS raw files. Filter for .GZ and .XML files in the raw data folder and change the names to income_1year. Note that we filter out files that have income in the name before renaming, this is to prevent changing downloads from the income subgroup code. 
...
ipums_files <-
  list.files(paste0(here::here("01_financial-well-being", "data", "raw"))) %>%
  as_tibble() %>%
  filter(str_detect(value, "dat.gz|xml"),!str_detect(value, "income")) %>%
  mutate(value = paste0(here::here("01_financial-well-being", "data", "raw"), "/", value)) %>%
  pull()

file.rename(ipums_files, c(
  here::here("01_financial-well-being", "data", "raw", "income_1year.dat.gz"),
  here::here("01_financial-well-being", "data", "raw", "income_1year.xml")
))
...
ddi <-
  read_ipums_ddi(here::here("01_financial-well-being", "data", "raw", "income_1year.xml"))

micro_data <-
  read_ipums_micro(
    ddi,
    data_file = here::here("01_financial-well-being", "data", "raw", "income_1year.dat.gz")
  )
...
Drop labels and define variable formats. Household income takes an unknown format upon reading in the raw data, we redefine it as numeric.
...
acs_2021 <- acs_2021 %>%
  mutate(
    across(where(is.labelled), ~ lbl_clean(.x)),
...
Note there are no vacancy results by default of the structure of census data read in. Turn the missing/"not in universe" variable into NAs.
...
Look at distribution after adjustment. Some out liers will still exist but bunching around the missing value should not be present.
...
Remove group quarters, we want to keep only households (GQ 1 and 2).
...
Check that group quarters are only 1 and 2.
...
```{r}
write_csv(acs2021clean, here::here("01_financial-well-being", "data", "intermediate", "2021microdata.csv"))

```

## Create income metric

Create the income metric (non-subgroup).
...
write_csv(metrics_income, here::here("01_financial-well-being", "data", "final", "metrics_income_city_2021.csv"))
