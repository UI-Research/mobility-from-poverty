...
check_results <- results_all %>% 
  select(statefip, place, pctl_income_q20:pctl_income_q80_upp) %>%
  pivot_longer(-c(statefip, place), names_to = "income_results") %>% 
  filter(is.na(value)) %>% 
  select(statefip, place) %>% 
  unique() %>% 
  write_csv("na_cis_place.csv")
...
metrics_income_all <- left_join(results_all_expand, place_puma, by=c("crosswalk_period", "statefip","place"))
```
