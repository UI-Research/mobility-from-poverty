---
title: "UMF: Income metric, non-subgroup, counties"
author: "JP Walsh"
date: today
format:
  html:
...
Income metric, non-subgroup, counties.

ACS Code: Income metric, non-subgroup Based on processes developed by Paul Johnson and Kevin Werner in SAS.
...
Now have 7,054,190 observations. Run anti_join to see how many cases on the left did not have a match on the right.
...
6,679,068 obs to 2,826,718 obs.
...
metrics_income <- metrics_income %>% 
  select(statefip, county, pctl_20, pctl_20_quality, 
...
metrics_income %>% 
  ...
  theme_minimal() +
  ggtitle("Income percentiles, county") +
  ylab("Density")
...
write_csv(metrics_income, "data/final/metrics_income_county_2021.csv")
