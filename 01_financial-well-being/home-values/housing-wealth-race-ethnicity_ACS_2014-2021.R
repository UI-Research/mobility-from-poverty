...
## 3.1. Household Data
acs_hh <- acs_hh %>% 
  # if county isn't present in the ACS then use 
  mutate(county = ifelse(countyfip != "000", countyfip, county)) %>% 
  # adjust the weights to account for the allocation from PUMAs to counties
