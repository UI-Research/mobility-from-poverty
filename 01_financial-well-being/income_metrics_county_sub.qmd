...
results_all <- finalize_metric(results_all, .geo_level = county)
...
results_race_ethnicity <- finalize_metric(results_race_ethnicity, .geo_level = county)
...
Join on to county_puma which provides the quality of the crosswalk (`afact * afact2`).
```{r}

metrics_income_sub <- left_join(metric_income_sub, county_puma, by=c("crosswalk_period", "statefip","county"))
```

Now create the metric quality variable. The `pctl_income_quality` variable combines quality information on the quality of the crosswalk and the sample size (effective sample) to create a final quality flag. Note that any county with NA for the pctl_income_20 metric is being suppressed due to sample size and is given a quality flag of 3.  

```{r}
metrics_income_sub <- metrics_income_sub %>% 
         mutate(pctl_income_quality = case_when(is.na(pctl_income_20) ~ 3,
...
Look at distributions of income percentiles by county for all (2021).
```{r}
metrics_income_sub %>% 
...
```{r}
metrics_income_sub <- metrics_income_sub %>% 
  select(year, state, county, subgroup, subgroup_type, pctl_income_20, pctl_income_20_lb, 
         pctl_income_20_ub,
...
Export as CSV.
```{r}
write_csv(metrics_income_sub, here::here("01_financial-well-being", "data", "final", "metrics_income_county_subgroup_longitudinal.csv"))
```
