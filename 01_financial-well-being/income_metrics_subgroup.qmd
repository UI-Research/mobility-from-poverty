---
title: "UMF: Income metric, subgroup"
author: "JP Walsh"
date: today
format:
  html:
...
Using the API, read in the IPUMS micro data. To check on available surveys you can use the function get_sample_info("usa"). This version of the code is currently pulling from "2021b" which is 5-year 2021 ACS data.
...
filepath <- download_extract(usa_ext_umf_submitted, download_dir = paste0(here::here(), "/01_financial-well-being/data/raw"), progress = FALSE)
...
Limit the crosswalk to the Census Places that will be used in the final data set. Bring in the places crosswalk (place-populations.csv) to select only places included in the UMF data. Keep only places in 2020.
...
Rename Place population variables to prep for merge.
...
puma_place_2021 %>% 
  ...
  theme_minimal() +
  ggtitle("AFACT distribution, UMF place crosswalk") +
  ylab("Density") 
...
puma_place_2021 %>% 
  ...
  geom_density(color = "blue", fill = alpha("blue", 0.3)) +
  theme_minimal() +
  ggtitle("Population distribution in PUMAs, UMF place crosswalk") +
  ylab("Density") 
...
puma_place %>% 
  ungroup() %>% 
  summarise(avg_sum_product = mean(sum_products))
```
...
7.9 million obs from the microdata (makes sense since we limited to only PUMAs that are overlapping with Places of interest).
...
```{r}
write_csv(acs2021clean, "data/intermediate/2021_5year_microdata.csv")
...
```{r}
write_csv(place_subgroup, "data/intermediate/place_subgroup.csv")
```
...
Objective: get pctl_20 pctl_50 and pctl_80 per unique state+place for subgroups in the year 2021 (from the 5-year data) for the var HHINCOME.
...
income_all <- read_csv("data/final/metrics_income_city_2021.csv")
...
write_csv(metrics_income, "data/final/metrics_income_city_subgroup_2021.csv")
...
First read in the fips information for these places.
...
```{r}
metrics_income_place <- read_csv("data/final/metrics_income_city_subgroup_2021.csv") %>% 
...
metrics_income_county <- read_csv("data/final/metrics_income_subgroup_county_2021.csv") %>% 
  select(state, county, subgroup, pctl_20_county = pctl_20,
...
metrics_income_place %>% 
  left_join(metrics_income_county, by = c("state" = "state", "county" = "county", "subgroup" = "subgroup")) %>%
  mutate(diff_20 = pctl_20 - pctl_20_county,
         diff_50 = pctl_50 - pctl_50_county,
         diff_80 = pctl_80 - pctl_80_county) %>% 
  select(state, place, county, subgroup, diff_20, diff_50, diff_80) %>% 
  reactable(
