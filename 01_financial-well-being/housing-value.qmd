...
We load the 2014-2022 1 Year ACS microdata we downloaded from IPUMS above. IPUMS
...
# the previous chunk is called. In my case it is, "usa_00014.xml" but that will
...
Notice that missing values for "value of housing units" (`valueh`) are labelled
as 9999998, and 9999999. We fix this before doing any computations.
...
non-hispanic, other non-hispanic, and
...
# Check that only new planning regions are present in 2022 for CT
assertthat::assert_that(
  all.equal(acs1yr |>
    select(year, state, county) |>
    filter(state == "09", year == 2022) |>
    distinct(county) |>
    pull(), c("190", "000", "180", "170", "130", "140", "120"))
)
...
households_county <-
  ...
  # Remove old counties from 2022 Conneticut data
  filter(!(year == 2022 & state == "09" & str_starts(county, "0")))
...
We keep observations for homeowners only in order to calculate total aggregated
...
In particular,
  ...
  - Data quality is marginal (`2`): N is greater than 30 but only 35% to 75% of
    observations are from the county/place.
  - Data quality is poor (`3`): N is less than or equal to 30 **or** if less
...
We remove 2014, 2015 observations for places due to missing values in the place populations file for those years. 
...
export_metric <- function(data, file_type) {
  ...
  if (file_type == "subgroup") {
    if (geo_var == "county") {
      missing_geo <- tribble(
        ~state, ~county, ~state_name, ~county_name, ~subgroup_type, ~subgroup,
        "02", "261", "Alaska", "Valdez-Cordova Census Area", "age", "All",
        "02", "261", "Alaska", "Valdez-Cordova Census Area", "age", "Over or exactly 45 years old",
        "02", "261", "Alaska", "Valdez-Cordova Census Area", "age", "Under 45 years old"
      ) |> expand(nesting(state, county, state_name, county_name, subgroup_type, subgroup),
      ...
      data <-
        ...
        full_join(missing_geo, by = c(
          "year", "state", "county", "state_name", "county_name",
          "subgroup", "subgroup_type"
        )) |>
        arrange(year, state, county, subgroup_type, subgroup) |>
        relocate("subgroup_type", .before = subgroup) |>
        group_by(year, state, county) |>
        fill(c(state_name, county_name), .direction = "downup") |>
        ungroup()
    } else if (geo_var == "place") {
      missing_geo <- tribble(
        ~state, ~place, ~state_name, ~place_name, ~subgroup_type, ~subgroup,
        "13", "72122", "Georgia", "South Fulton city", "age", "All",
        "13", "72122", "Georgia", "South Fulton city", "age", "Over or exactly 45 years old",
        "13", "72122", "Georgia", "South Fulton city", "age", "Under 45 years old"
      ) |>
        expand(nesting(state, place, state_name, place_name, subgroup_type, subgroup),
          year = c(2016, 2017, 2018, 2019)
        )

      data <-
        data |>
        # Filter out South Fulton city, which is NA for all years for this metric
        # and then add the table with complete year-state-county-subgroup
        # combinations
        filter(place_name != "South Fulton city") |>
        mutate(subgroup_type = "age", .before = "subgroup") |>
        ...
        full_join(missing_geo, by = c("year", "state", "place", "state_name", "place_name", "subgroup", "subgroup_type")) |>
...
assertthat::assert_that(
  all.equal(
    ...
    county_all |>
      select(year, state, county) |>
      filter(state == "09", year %in% c(2022)) |>
      distinct(county) |>
      arrange(county),
...
```{r}
#| label: check-ct-counties-recent
county_recent <- read_csv(here::here("01_financial-well-being", "final", "households_house_value_race_ethnicity_2022_county.csv"))

assertthat::assert_that(
  all.equal(
    county_recent |> nrow(),
    county_file |> filter(year == 2022) |> nrow()
  )
)
...
place_subgroup <- read_csv(here::here(
  "01_financial-well-being", "final",
  "households_house_value_race_ethnicity_subgroup_city.csv"
))
```

**Check**: How many observations for Valdez-Cordova and South Fulton city?
