...
We compare the aggregated totals to the state total posted on [Wikipedia](https://web.archive.org/web/20230609222923/https://en.wikipedia.org/wiki/2016_United_States_presidential_election). This code scrapes the vote count: 

```{r}
# scrape data from Wikipedia with rvest
wiki <- read_html("https://web.archive.org/web/20230609222923/https://en.wikipedia.org/wiki/2016_United_States_presidential_election")
...
all_counties <- all_counties %>%
  mutate(year = 2016) %>%
  select(year, state, county, share_election_turnout, share_election_turnout_quality) 
