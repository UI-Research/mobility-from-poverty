...
We compare the aggregated totals to the state total posted on [Wikipedia](https://web.archive.org/web/20230629100426/https://en.wikipedia.org/wiki/2020_United_States_presidential_election). This code scrapes the vote count: 

```{r}
# scrape data from Wikipedia with rvest
wiki <- read_html("https://web.archive.org/web/20230629100426/https://en.wikipedia.org/wiki/2020_United_States_presidential_election")
...
joined_data <- joined_data %>%
  mutate(
    share_election_turnout_quality = case_when(
      is.na(share_election_turnout) ~ as.numeric(NA),
      cv >= 0.15 ~ 3,
      cv >= 0.05 ~ 2,
      TRUE ~ 1
    )
...
Missouri has inherent issues with their voting reporting data. Manually update the quality of these states to reflect poor quality.
...
turnout_combined <- turnout_combined %>%
  mutate(change = share_election_turnout_2020 - share_election_turnout_2016)

turnout_combined %>%
  ggplot(aes(share_election_turnout_2016, share_election_turnout_2020, color = population < 10000)) +
  geom_abline() +
  geom_point(alpha = 0.1) +
  labs(
...
```{r}
turnout_combined %>%
  count(share_election_turnout_quality_2016, share_election_turnout_quality_2020)
