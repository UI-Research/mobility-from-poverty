...
We need to crosswalk the pre-2023 counties in the MIT data to the updated Connecticut planning regions.

```{r}
cw_connecticut <- read_csv(here::here("geographic-crosswalks", "data", "connecticut_planning_regions_to_pre_2023_counties.csv"),
                           skip = 2, col_names = c("CTcounty", "county", "CountyName", "CTCountyName", "pop20", "afact2", "afact")) %>% 
  select(FIPS = county, FIPS_old = CTcounty, afact, afact2) %>% 
   mutate(FIPS = as.character(FIPS),
         FIPS = str_pad(FIPS, width = 5, side = "left", pad = "0"),
...
Create a data frame with just CT and join on the crosswalk.
```{r}
mit_ct <- mit_counties %>% 
  filter(state == "09") %>% 
  mutate(FIPS_old = paste0(state, county))

mit_ct_join <- mit_ct %>% 
  left_join(cw_connecticut, by = c("FIPS_old"))
```

Create a data frame without CT 
```{r}
mit_non_ct <- mit_counties %>% 
  filter(state != "09")

```

Sum the eligible voting population weighted by the AFACT in CT planning regions
```{r}
mit_ct_join <- mit_ct_join %>% 
  group_by(year, FIPS) %>% 
  summarise(votes = sum(votes * afact)) %>% 
  ungroup() %>% 
  mutate(state = str_sub(FIPS, start = 1, end = 2),
         county = str_sub(FIPS, start = 3, end = 5)) %>% 
  select(-FIPS)
```

Compare Connecticut voting sums.

```{r}
sum(pull(mit_ct, votes))
sum(pull(mit_ct_join, votes))
stopifnot(abs(sum(pull(mit_ct, votes)) - sum(pull(mit_ct_join, votes))) < 50)
```

Combine the CT data back onto the overall data.
```{r}
mit_counties <- bind_rows(mit_non_ct, mit_ct_join) 
```

Alaska shows voting districts which are not comparable to counties. To test our count we drop Alaska from the data and subtract 30 counties from our usual total.
```{r}
stopifnot(nrow(mit_counties %>% filter(state != "02")) == 3144-30)
```

#### Turnout comparisons

We compare the aggregated totals to the state total posted on [Wikipedia](https://web.archive.org/web/20260301183408/https://en.wikipedia.org/wiki/2024_United_States_presidential_election) and with totals published by the [Census](https://www.census.gov/data/tables/time-series/demo/voting-and-registration/p20-587.html). 
...
wiki <- read_html("https://web.archive.org/web/20260301183408/https://en.wikipedia.org/wiki/2024_United_States_presidential_election")
...
joined_data <- joined_data %>%
  mutate(
    share_election_turnout = votes / cvap,
...
joined_data <- joined_data %>%
  mutate(share_election_turnout = if_else(condition = share_election_turnout > 1, true = 1, false = share_election_turnout))
...
```{r}
stopifnot(
  max(joined_data$share_election_turnout, na.rm = TRUE) <= 1
)

stopifnot(
  min(joined_data$share_election_turnout, na.rm = TRUE) >= 0
)
...
```{r}
joined_data %>%
  ggplot(aes(votes, share_election_turnout)) +
...
joined_data %>%
  ggplot(aes(cvap, share_election_turnout)) +
  geom_point(alpha = 0.1) +
  scale_x_log10() +
  scale_y_continuous(limits = c(0, 1),
                     expand = expansion(mult = c(0, 0.1))) +
...
joined_data <- joined_data %>%
  mutate(
    share_election_turnout_quality = case_when(
      is.na(share_election_turnout) ~ NA_real_,
      cv >= 0.15 ~ 3,
      cv >= 0.05 ~ 2,
      TRUE ~ 1
    )
...
Connecticut and Missouri have inherent issues with their voting reporting data. Manually update the quality of these states to reflect poor quality.

```{r}
joined_data <- joined_data %>% 
  mutate(
    share_election_turnout_quality = ifelse(
      state %in% c("09", "29"), 
      pmax(2, share_election_turnout_quality), 
      share_election_turnout_quality
    )
  )

count(joined_data, share_election_turnout_quality)
```


```{r}
joined_data %>%
  ggplot(aes(factor(share_election_turnout_quality), share_election_turnout)) +
  geom_point(alpha = 0.1) +
  scale_y_continuous(limits = c(0, 1),
                     expand = expansion(mult = c(0, 0.1))) +
  scatter_grid() +
...
all_counties <- all_counties %>%
  mutate(year = 2024) %>%
  select(year, state, county, share_election_turnout, share_election_turnout_quality, population) 

```

Test if 'all_counties' is 3,144 rows

```{r}
stopifnot(nrow(all_counties) == 3144)
```
...
turnout_combined <- turnout_combined %>%
  mutate(change = share_election_turnout_2024 - share_election_turnout_2020)

turnout_combined %>%
  ggplot(aes(share_election_turnout_2024, share_election_turnout_2020, color = population < 10000)) +
  geom_abline() +
  geom_point(alpha = 0.1) +
  labs(
...
turnout_combined <- turnout_combined %>%
  mutate(change = share_election_turnout_2024 - share_election_turnout_2016)

turnout_combined %>%
  ggplot(aes(share_election_turnout_2024, share_election_turnout_2016, color = population < 10000)) +
  geom_abline() +
  geom_point(alpha = 0.1) +
  labs(
...
turnout_longitudinal <- turnout_longitudinal |>
  ...
  mutate(turnout_variation  = sum(abs(share_election_turnout - median(share_election_turnout)))) |>
  ungroup()

turnout_longitudinal |>
  ggplot(aes(share_election_turnout_quality, turnout_variation)) +
  geom_point(alpha = 0.1)
