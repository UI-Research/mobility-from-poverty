...
See [here](https://www.countyhealthrankings.org/health-data/community-conditions/social-and-economic-factors/safety-and-social-support/social-associations?year=2025)
for more
...
if(!file.exists(here::here("15_social-capital/data/cbp_zcta.csv"))){
  write_csv(cbp_zcta, "15_social-capital/data/cbp_zcta.csv")
}
...
```{r}
cbp_zcta <- read_csv(here("15_social-capital/data/cbp_zcta.csv"))
```
...
zcta_20_place_21_xwalk <-
  read_csv(here("15_social-capital/data/2020_ZCTA_2021_Census_Places_Crosswalk.csv"), skip = 1) %>%
  as_tibble() %>%
  clean_names() |>
  rename(
...
zcta_10_place_10_xwalk <-
  read_csv(here("15_social-capital/data/2010_ZCTA_2010_Census_Places_Crosswalk.csv"), skip = 1) %>%
  as_tibble() %>%
  clean_names() |>
  rename(
    zcta_code = zip_census_tabulation_area,
...
evaluate_final_data(here("99a_final-evaluation", "evaluation_form_social_cap1_overall_place.csv"), associations_city_data, "place", subgroups = FALSE, confidence_intervals = FALSE)
```
...
associations_city_data %>%
  ...
  write_csv(here("15_social-capital/final/social_associations_all_city.csv"))
```
