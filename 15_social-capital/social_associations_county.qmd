...
format:
  html:
    self-contained: true
    toc: true
    css: ../15_social-capital/www/web_report.css
editor_options:
...
source(here::here("15_social-capital", "R", "census_api_key.R"))
```
Below are the codes/associations included in the County Health Rankings metric. See [here](https://www.countyhealthrankings.org/health-data/community-conditions/social-and-economic-factors/safety-and-social-support/social-associations?year=2025) for more
...
data_dir <- here("15_social-capital", "data")
...
head(cbp_county)
write_csv(cbp_county, here("15_social-capital/data/cbp_county.csv"))
```

```{r}
cbp_county <- read_csv(here("15_social-capital/data/cbp_county.csv"))
```
...
This is due to a [methodological change](https://www.census.gov/programs-surveys/cbp/technical-documentation/methodology.html) from the Census Bureau beginning with the 2017 CBP release. In particular, the Census Bureau stopped publishing any county–industry cell that has fewer than three establishments. Before 2017, establishment counts were released even when other fields in that cell were suppressed; after 2017, those small cells are omitted entirely. 
...
evaluate_final_data(here("99a_final-evaluation", "evaluation_form_social_cap1_overall_county.csv"), merged_associations, "county", subgroups = FALSE, confidence_intervals = FALSE)
```
...
merged_associations %>%
  ...
  write_csv(here("15_social-capital/final/social_associations_all_county.csv"))
```
