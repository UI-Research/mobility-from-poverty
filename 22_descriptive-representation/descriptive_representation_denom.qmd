...
Note that if the 5-year data is not available this program will pull in the 1-year data and provide a notice that the 5-year is not out.
...
avail <- tryCatch(
  load_variables(update_years, "acs5", cache = TRUE), 
  error = function(e) e)

survey <- if (inherits(avail, "error")) "acs1" else "acs5"

if(survey == "acs1") print("Notice! Your latest selected year of ACS data is only availale at the 1-year level")

 vars <- c(
    total_people       = "B01003_001E",
  ...
  get_acs(
    ...
    dplyr::mutate(year = update_years)
...
```{r}
evaluate_final_data(exp_form_path = here::here("99a_final-evaluation", "evaluation_form_desc_rep_overall_place.csv"), data = place_pop_by_race, geography = "place", subgroups = FALSE, confidence_intervals = FALSE)
```

County data.

```{r}
evaluate_final_data(exp_form_path = here::here("99a_final-evaluation", "evaluation_form_desc_rep_overall_county.csv"), data = county_pop_by_race, geography = "county", subgroups = FALSE, confidence_intervals = FALSE)
```
...
### Export as CSV
```{r}
write_csv(county_pop_by_race, here::here("22_descriptive-representation", "data", "final",
                                         paste0("descriptive_rep_denominator_county_all.csv")))

write_csv(place_pop_by_race, here::here("22_descriptive-representation", "data", "final",
