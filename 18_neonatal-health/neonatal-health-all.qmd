...
# Create folders for the raw data if they do not exist
folder_paths <- c(here::here("18_neonatal-health", "data", "all"),
                  here::here("18_neonatal-health", "data", "subgroups"))

# Check if the folder exists, if not, create it
lapply(folder_paths, dir.create, recursive = TRUE, showWarnings = FALSE)
```
...
+ The code for this metric was previously written in Stata. Vincent Pancini rewrote it in R and simplified it to be more streamlined and intuitive in 2025.
+ This update includes the latest year of data available (2024) as well as all years back to 2016.
...
The raw data used to create this metric are available to Urban Institute researchers on Box at [Data/UMI Raw Data/Metrics_2026_round/18_neonatal-health/data](https://urbanorg.box.com/s/jsgyyt8ksok5h3t9x49s5jyo7ywfp2y9).
...
2. Under "Current Final Natality Data" select [Natality for 2007 - 2024](https://wonder.cdc.gov/natality-current.html). 
...
4.  Select birth characteristics:
  + Year: 2024 or relevant year (select only 1 year at a time)
...
-   Move the `.txt` file from your Downloads folder to the `mobility-from-poverty\18_neonatal-health\data\all` directory.
...
-   Move the `.txt` files from your Downloads folder to the `mobility-from-poverty\18_neonatal-health\data\subgroups` directory.
...
paths <- list.files(here::here("18_neonatal-health", "data", "all"), full.names = TRUE)
...
births_raw <- paths |> 
  ...
  mutate(
    ...
    birth_type = str_extract(filename, "^(all|lbw)"),  # Extract birth type from filename
  )
```
...
paths_subgroups <- list.files(here::here("18_neonatal-health", "data", "subgroups"),
...
births_subgroups_raw <- paths_subgroups |> 
  ...
  mutate(
    year = parse_number(filename),  # Extract year from filename
    birth_type = str_extract(filename, "^(all|lbw)"),  # Extract birth type from filename
    subgroup_type = str_extract(filename, "(race|momed)"),  # Extract subgroup type from filename
    subgroup = case_when(
...
For years 2022 and beyond the counties for Connecticut don't match between the CDC WONDER data and Urban's county population file.
...
In the 2022, 2023, and 2024 CDC WONDER data, Connecticut's outdated 8 counties are still used (09001, 09003, 09005, 09007, 09009, 09011, 09013, 09015), but in Urban's universe of counties the updated 9 planning regions are used (09110, 09120, 09130, 09140, 09150, 09160, 09170, 09180, 09190). CDC WONDER intends to move to the updated planning regions in the future, so we leave these years as missing until that transition takes place.
...
urban_counties_clean <- urban_counties |>
  # Remove CT counties in 2022, 2023, and 2024
  filter(!(state=="09" & year %in% c(2022, 2023, 2024)))
...
((3142-578)*4) + ((3143-578)*2) + ((3135-570)*3)
...
We removed Connecticut counties in 2022, 2023, and 2024 earlier in the file, because the counties for these years in the CDC WONDER data do not match our target counties. We add them back to the final file below
```{r add-ct-2022-2023}
...
lbw_rates <- bind_rows(lbw_rates, ct_2022_2024)
...
```{r evaluate-data-all}
evaluate_final_data(
  exp_form_path = "99a_final-evaluation/evaluation_form_low_birth_overall_county.csv",
  data = final_all,
  geography = "county",
  subgroups = FALSE,
...
evaluate_final_data(
  exp_form_path = "99a_final-evaluation/evaluation_form_low_birth_race-ethnicity_county.csv",
...
evaluate_final_data(
  exp_form_path = "99a_final-evaluation/evaluation_form_low_birth_mothers-education_county.csv",
