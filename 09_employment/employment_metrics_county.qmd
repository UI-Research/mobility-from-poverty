...
```{r}
acs_clean_samp <- acs_clean %>% 
  group_by(sample, county) %>% 
  slice_sample(n = 30) %>% 
  ungroup()

acs_clean_samp_state <- acs_clean_samp %>% 
  filter(statefip == "17")

acs_clean_samp_dupe <- acs_clean_samp %>% 
  filter(statefip == "17") %>% 
  bind_rows(acs_clean_samp_state)
...
```{r}
acs_survey_all <- acs_clean_samp %>%
  mutate(survey_id = paste0(year, crosswalk_period, statefip, county)) %>% 
...
acs_survey_all_acs <- acs_clean_samp %>%
  mutate(survey_id = paste0(year, crosswalk_period, statefip, county)) %>% 
  filter(sample_structure == "1year") %>% 
  left_join(repwts_person, by = c("unique_person_id")) %>% 
  as_survey_design(weights = perwt, 
...
acs_survey_subgroups <- acs_clean_samp %>%
...
Function for survey means.
```{r}

grouped_survey_mean <- function(id_list, df) {
  df %>%
    filter(survey_id == id_list) %>% 
    summarise(
    share_employed = survey_mean(employed, vartype = "ci")
    ) %>% 
    mutate(survey_id = id_list)
}
```
...
option1 <- end_time1 - start_time1

#Option 2


list_ids <- acs_clean_samp %>%
  filter(sample_structure == "1year") %>% 
  mutate(survey_id = paste0(year, crosswalk_period, statefip, county)) %>% 
  select(survey_id) %>% 
  unique() %>% 
...
metrics_employment_all <- map_df(list_ids, ~grouped_survey_mean(id_list = .x,
                                                      df = acs_survey_all)) %>% 
  mutate(year = as.integer(str_sub(survey_id, 1, 4)),
         crosswalk_period = str_sub(survey_id, 5, 12),
         statefip = str_sub(survey_id, 13, 14),
         county = str_sub(survey_id, 15, 18)
         )
end_time2 <- Sys.time()

option2 <- end_time2 - start_time2


option12 <- end_time1 - start_time1

start_time_check <- Sys.time()

metrics_employment_all_check <- acs_survey_all %>%
  filter(survey_id == list_ids[200]) %>% 
  summarise(
    share_employed = survey_mean(employed, vartype = "ci")
  )

end_time_check <- Sys.time()

start_time_check - end_time_check

acs_clean_samp %>% 
  select(statefip, county) %>% 
  unique() %>% 
  count()
#Option 3 Data table group (Bust so far)

# Convert the survey design to data.table
start_time1 <- Sys.time()
my_survey_dt <- as.data.table(acs_survey_all)

check <- my_survey_dt[, .(share_employed = survey_mean(employed, vartype = "ci", na.rm = TRUE)), by = .(year, crosswalk_period, statefip, county)]
end_time1 <- Sys.time()

dt_option <- end_time1 - start_time1

#Option 4 run state by state 


start_time1 <- Sys.time()
metrics_employment_all <- acs_survey_state2 %>%
  group_by(year, crosswalk_period, statefip, county) %>%
  summarise(
    share_employed = survey_mean(employed, vartype = "ci")
  ) %>% 
...
state2 <- end_time1 - start_time1

start_time1 <- Sys.time()
metrics_employment_all <- acs_survey_state1 %>%
  group_by(year, crosswalk_period, statefip, county) %>%
  summarise(
    share_employed = survey_mean(employed, vartype = "ci")
  ) %>% 
...
state1 <- end_time1 - start_time1

#Option 5 ACS format 

start_time1 <- Sys.time()
metrics_employment_all <- acs_survey_all_acs %>%
  group_by(year, crosswalk_period, statefip, county) %>%
  summarise(
    share_employed = survey_mean(employed, vartype = "ci")
  ) %>% 
...
Create share employed by disability at the county-level (5-year data) using the survey_mean function.
```{r}
