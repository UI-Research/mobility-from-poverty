...
```{r}
calculate_survey_mean_sub <- function(df) {
  df %>%
    as_survey_design(weights = perwt, 
                     repweights = matches("repwtp[0-9]+"),
                     type = "ACS",
...
```{r}
future::plan(future::multisession, workers=8)
metrics_employment_subgroup_disability <- furrr::future_map2_df(state_fips_sub, years_sub, ~calculate_survey_mean_sub(geoid_select = .x,
                                                                                                       year_select = .y,
