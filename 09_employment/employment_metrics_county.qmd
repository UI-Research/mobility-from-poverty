...
acs_clean_all <- acs_clean %>% 
  ...
  filter(effective_sample >= 30, sample_structure == "1year") %>% 
...
Create survey_mean calculation function for subgroups.
```{r}
...
Create survey_mean calculation function for subgroups with group_split method.
```{r}
calculate_survey_mean_sub <- function(geoid_select, year_select, df) {
  df %>%
    as_survey_design(weights = perwt, 
                     repweights = matches("repwtp[0-9]+"),
                     type = "ACS",
                     scale = 4/ 80,
                     rscales = rep(1, 80 ), mse = TRUE) %>%
    group_by(crosswalk_period, county, subgroup_disability)
    summarise(
      share_employed = survey_mean(employed, vartype = "ci")
    )
}
...
acs_clean_disability <- acs_clean %>% 
  ...
  filter(effective_sample >= 30, sample_structure == "5year") %>% 
  select(-effective_sample)

acs_clean_disability_split <- acs_clean_disability %>% 
  group_split(year, statefip)
```
...
metrics_employment_subgroup_disability <- furrr::future_map_dfr(~calculate_survey_mean_sub(df = acs_clean_disability_split)) %>% 
  ungroup() %>%
  mutate(subgroup_type = "disability") %>% 
  rename(subgroup = subgroup_disability)
...
```{r}
future::plan(future::multisession, workers=8)
metrics_employment_subgroup_disability <- furrr::future_map2_dfr(state_fips_sub, years_sub, ~calculate_survey_mean_sub(geoid_select = .x,
                                                                                                       year_select = .y,
                                                                                                       by_sub = "subgroup_disability",
