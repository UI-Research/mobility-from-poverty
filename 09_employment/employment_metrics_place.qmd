...
Create share employed for all individuals at the place-level.
...
Create share employed by race/ethnicity at the place-level.
...
```{r}
metrics_employment <- left_join(metrics_employment, place_puma, by=c("crosswalk_period", "statefip","place"))
```
...
metrics_employment %>% 
  ...
  theme_minimal() +
  ggtitle("Distribution of share employed, by place 2022") +
  ylab("Density")
...
```{r}
write_csv(metrics_employment, here::here("09_employment", "data", "final", "metrics_employment_city.csv"))
```
