...
acs_survey_all <- acs_clean %>%
  ...
  group_by(year, crosswalk_period, statefip, place, employed) %>% 
...
acs_survey_subgroups <- acs_clean %>%
  ...
  group_by(year, crosswalk_period, statefip, place, employed, subgroup) %>% 
  summarise(perwt = sum(perwt)) %>% 
  as_survey_design(weights = perwt) %>% 
  ungroup()
...
Objective: get the percent of individuals between the ages 25 and 54 that are employed in every place.
...
Merge the data quality flags from place_puma and effective sample onto the employment metric to create the final data quality metric. This metric is based on both size of place and the number of samples for that place.
```{r}
...
Create share employed by race/ethnicity at the place-level (5-year data).
```{r}
...
Create the share employed measure for all values in the 5-year data (non-subgroup). This value will be merged onto the subgroup data. 
```{r}
...
Look at distribution of employment share by place by race.
```{r}
