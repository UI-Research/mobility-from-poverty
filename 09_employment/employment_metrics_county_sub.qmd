...
Filter sample and create a list of the years included.
...
Create the employment metric for all counties. 
...
metric_employment_sub <-results_race_ethnicity_expanded %>% 
  bind_rows(results_gender_expanded, results_disability_expanded, results_all_expanded)
```

Add data quality. First, join on to county_puma which provides the quality of the crosswalk (`afact * afact2`).
```{r}
metric_employment_sub <- left_join(metric_employment_sub, county_puma, by=c("crosswalk_period", "statefip","county"))
```
...
Look at distribution of `share_employed` by county by race.
...
Look at the counts of the quality flag. For the 5-year data at the county level, when including subgroups, there is a high amount of quality "3" observations that we would suggest users disregard or use with caution. 
```{r}
metric_employment_sub %>% 
...
```{r}
write_csv(metric_employment_sub, here::here("09_employment", "data", "final", "metrics_employment_county_subgroups_longitudinal.csv"))
```
