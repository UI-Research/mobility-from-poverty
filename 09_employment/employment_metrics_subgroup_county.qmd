---
title: "Employment Opportunities Predictor, county, subgroups"
author: "JP Walsh"
date: today
format:
  html:
...
*Program for the creation of the Opportunity for Employment Opportunities Predictor, subgroups, county level*
...
if(!file.exists(here::here("09_employment", "data", "raw", "employment_opp_5year.dat.gz"))){
  
    usa_ext_umf <-
      define_extract_usa(
        description = "5-year ACS microdata pull for the Employment Opportunities Predictor at the county level, subgroups",
        samples = c("us2021c"),
         variables = list(
      "ADJUST",
      "STATEFIP",
...
Rename the IPUMS raw files. Filter for .GZ and .XML files in the raw data folder and change the names to employment_opp_5year. Note that we filter out files that have employment in the name before renaming, this is to prevent changing downloads from the employment non-subgroup code. 
...
if(!file.exists(here::here("09_employment", "data", "raw", "employment_opp_5year.dat.gz"))){
    ...
    file.rename(ipums_files, c(
      here::here("09_employment", "data", "raw", "employment_opp_5year.dat.gz"),
      here::here("09_employment", "data", "raw", "employment_opp_5year.xml")
    ))
  }
```
...
ddi <-
  read_ipums_ddi(here::here("09_employment", "data", "raw", "employment_opp_5year.xml"))

micro_data <-
  read_ipums_micro(
    ddi,
    data_file = here::here("09_employment", "data", "raw", "employment_opp_5year.dat.gz")
  )
...
Save as "microdata_county_subgroup.csv"

```{r}
write_csv(acs2021clean, here::here("09_employment", "data", "intermediate", "2021microdata_county_subgroup.csv"))

```

## Create employment metric

Create the employment metric (subgroup).

Objective: get the percent of individuals between the ages 25 and 54 that are employed from the EMPSTAT variable for all unique statefip and county pairs.

Aggregation should be weighted by PERWT (this is a person level statistic).

First, isolate the data set to 25-54 year-olds.
...
Look at distributions of employment share by county for each subgroup category.
...
Read in the employment share variable for all households in the county.

```{r}

employment_all <- read_csv(here::here("09_employment", "data", "final", "metrics_employment_county_2021.csv"))
...
write_csv(metrics_employment, here::here("09_employment", "data", "final", "metrics_employment_county_subgroup_2021.csv"))
